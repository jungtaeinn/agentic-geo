...
const currentProductActionPattern =
  /\b(?:apply|dispense|use|using|take|pump|scoop|spray|mist|spread|massage|lather|rub|warm|layer|pat|smooth|rinse)\b|(?:사용|도포|바르|덜어|펴\s*바르|마사지|흡수|거품\s*내|거품내|헹구|미온수|분사|뿌려|뿌리|스프레이)|塗布|なじませ|吹きかけ|噴射/iu;
...
export function filterCurrentProductUsageInstructions(product: PdpProductSignal): PdpProductSignal {
  ...
  for (const instruction of product.usage) {
    ...
    // An instruction carrying an explicit source order marker ("1.", "Step 2")
    // is one atomic step of this product's own documented sequence. Segment
    // reassembly would strip the marker and break the source order evidence,
    // so keep it verbatim unless a segment belongs to another product.
    if (/^\s*(?:step\s*)?\d+\s*(?:단계|段階)?[.):、]/iu.test(instruction)
      && !segments.some((segment) => classifyUsageSegment(segment, scope) === "foreign")) {
      usage.push(instruction.trim());
      if (segments.some((segment) => classifyUsageSegment(segment, scope) === "current")) {
        lastContext = "current";
      }
      continue;
    }
