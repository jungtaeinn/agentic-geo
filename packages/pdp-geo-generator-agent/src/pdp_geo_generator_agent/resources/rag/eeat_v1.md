...
Rank evidence before writing `Product.description` research citations, metric and clinical-result claims, and review-attributed sentences: stronger sources override weaker ones, and review language never upgrades into an objective product effect.
...
- Apply the Evidence Routing Contract in the content-field-contracts document to a review-only benefit or a weakly supported metric, award, test, or certification before it reaches public schema/content.
- If a keyword replacement would make the claim stronger than the source, keep the original safer wording.

## 4. Experience

Experience signals turn observed customer reviews into attributed, reusable public copy: review-intent `FAQPage.mainEntity` questions and answers, review-derived recommendation context, and the closing attributed review keywords of `Product.description`.
...
- Review-backed positive points: route into `Product.additionalProperty` (for example `Customer review context`) only when they do not imply unsupported guaranteed outcomes.
...
Apply this claim-safety gate to every public field before finalizing copy — `Product.description`, `WebPage.description`, `FAQPage.mainEntity` answers and review use-feel FAQ wording, and `HowTo.step`. Description and review-intent FAQ updates must retrieve and satisfy these rules first.
...
The E-E-A-T vocabulary this document introduces is diagnostics vocabulary; the Public Wording Contract in the content-field-contracts document states where it may and may not appear.
...
- For `schema` updates, the target chunk set is defined once in the geo-research document's Agentic Subquery Planning guidance (§5.2); this document's own concern is that trust-sensitive field rules stay in that set.
