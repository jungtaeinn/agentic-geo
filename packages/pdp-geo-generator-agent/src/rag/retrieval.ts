...
export interface RetrievePdpGeoRagChunksOptions {
  ...
  /**
   * Optional real embedding adapter for local-versioned RAG. When provided,
   * semantic similarity uses provider embeddings (multilingual models close
   * the ko/ja query vs. English policy-doc gap) while the deterministic hash
   * embedding remains the reproducible fallback. Results are cached by text
   * hash for determinism and cost control.
   */
  customEmbedder?: PdpGeoRagEmbedder;
}

/** Provider-neutral embedding adapter (e.g. OpenAI, Gemini, bge-m3). */
export interface PdpGeoRagEmbedder {
  /**
   * Vector-space identifier. Cached vectors are only reusable inside one
   * space, so embedders that declare the same model share a cache namespace
   * and embedders that declare none are isolated per instance.
   */
...
/**
 * Final-context budget. `selectFinalRagChunks` seats one chunk per policy
 * family from `coverageRagKindOrder` out of this budget, so a budget smaller
 * than the number of families silently drops whichever families sit last —
 * measured 2026-08-28: at 8 the schema target scored 0.542 because
 * `official-docs` and `terminology` never got a seat, and at 10 it reached
 * 1.000. Fourteen seats every family with discretionary slots to spare; the
 * measured curve is flat from 14 to 20, so wider budgets buy context size
 * rather than recall.
 */
export const DEFAULT_PDP_GEO_RAG_MAX_CHUNKS = 14;

/**
 * Settings after resolution. Every field the resolver always fills is required
 * here, so callers read the resolved value instead of repeating a default —
 * duplicated `?? 8` fallbacks were how a stale budget survived a change to the
 * real default.
 */
export type ResolvedPdpGeoRagSettings =
  Required<Pick<PdpGeoRagSettings, "mode" | "provider" | "embeddingProvider" | "rerankerProvider" | "maxChunks" | "scoreThreshold" | "rewriteQuery">>
  & PdpGeoRagSettings;

/** Resolves user/runtime RAG settings into explicit provider choices. */
export function resolvePdpGeoRagSettings(settings: PdpGeoRagSettings = {}): ResolvedPdpGeoRagSettings {
  ...
  return {
    ...
    embeddingProvider,
    rerankerProvider,
    maxChunks: settings.maxChunks ?? DEFAULT_PDP_GEO_RAG_MAX_CHUNKS,
...
export async function retrievePdpGeoRagChunks(
  ...
  return new LocalVersionedRagRetriever(options.customEmbedder).retrieve({
    ...request,
    documents
  });
}
 ...
 *
 * Queries name the contracts and policy concepts they need; they never restate
 * the rules themselves. A query that spells out a contract's own wording turns
 * retrieval into a self-fulfilling match — the documents that paraphrase the
 * contract score highest, so canonicalizing a rule into one document lowers its
 * retrievability (measured 2026-08-27: removing four CEP restatements while the
 * query still carried the description stage order dropped webPageDescription
 * claim recall by 0.458). Product signals carry the query; contract names are
 * the routing anchors.
 */
...
/**
 * Full generation diverges into per-field subqueries by default (IF-GEO-style
 * multi-intent planning): distinct field intents retrieve distinct guidance
 * instead of relying on one general query plus coverage backfill.
 */
const defaultFullGenerationUpdateTargets: PdpGeoRagUpdateTarget[] = [
  "productDescription",
  "webPageDescription",
  "faq",
  "howToUse",
...
export function createPdpGeoRagQueryPlan(
  ...
  const explicitUpdateTargets = uniqueUpdateTargets([
    ...hintUpdateTargets,
    ...(settings.queryPlanning?.updateTargets ?? [])
  ]);
  const planningEnabled = settings.queryPlanning?.enabled ?? true;
  ...
  const updateTargets = explicitUpdateTargets.length > 0
    ? explicitUpdateTargets
    : defaultFullGenerationUpdateTargets;
...
function createTargetSubquery(
  ...
  const map: Record<PdpGeoRagUpdateTarget, PdpGeoRagSubquery> = {
    ...
    webPageDescription: {
      ...
      target,
      query: `${baseFacts} Update only WebPage.description as the page-scope counterpart of the product-entity narrative. Need the content field contracts for description separation, PDP field mapping for schema fields, CEP guidance for the supported target customer, customer situation, and review preference, page-scope wording for the product page and source-backed brand, schema role separation between the page resource and the product entity, and public wording safety. ${benefitText} ${ingredientText} ${reviewText}`,
    ...
    benefits: {
      id: "target-benefits",
      target,
      query: `${baseFacts} Update benefit/effect PDP sections and Product.additionalProperty benefit fields with source-backed claim wording and overclaim filtering. ${benefitText}`,
      intents: ["claims", "evidence", "customer"],
      fieldTargets: ["PDP.content", "Product.description"],
    ...
    howToUse: {
      id: "target-howto",
      target,
      query: `${baseFacts} Update only HowTo.step and how-to-use PDP content from direct source usage. Need the HowTo contract, source-faithful step eligibility, direct source action versus routine note or review anecdote, step count and order fidelity, routine CEP field mapping, and schema.org HowTo node compatibility. ${usageText}`,
    ...
    schema: {
      id: "target-schema",
      target,
      query: `${baseFacts} Update JSON-LD schema graph fields only: Product, WebPage, FAQPage, HowTo, BreadcrumbList, additionalProperty, offer/review compatibility, and validation constraints.`,
    ...
    reviews: {
      id: "target-reviews",
      target,
      query: `${baseFacts} Update review-led product copy, review-intent FAQ use-feel answers, review summaries, and review-backed points routed to Product.additionalProperty. Need the evidence routing contract for review signals, review sentiment boundaries for public FAQ intent, attribution requirements for customer experience, and the separation between rating metadata and reviewer wording. ${reviewText}`,
      intents: ["review", "faq", "customer", "evidence"],
      fieldTargets: ["FAQPage.mainEntity", "Product.description", "PDP.content"],
...
export class LocalVersionedRagRetriever implements PdpGeoRetriever {
  constructor(private readonly embedder?: PdpGeoRagEmbedder) {}
  ...
  async retrieve(request: PdpGeoRetrieverRequest): Promise<PdpGeoRetrievedChunk[]> {
    ...
    // Cross-lingual lexical bridge: terminology maps define per-locale
    // preferred wordings for the same concept, so a Korean/Japanese query can
    // match the English policy corpus (and vice versa) deterministically.
    const terminologyExpansions = buildTerminologyExpansionMap(request.documents);
    const queryTerms = expandTokensWithTerminology(tokenize(request.query), terminologyExpansions);
    const expandedQueryText = queryTerms.join(" ");
    const contextualTexts = chunks.map((chunk) => createContextualRetrievalText(chunk));
    const semanticScores = await this.computeSemanticScores(request.query, expandedQueryText, contextualTexts);
    const scoredCandidates = chunks.map((chunk, index) => {
      const contextualText = contextualTexts[index] ?? "";
      const candidateTerms = expandTokensWithTerminology(tokenize(contextualText), terminologyExpansions);
      const lexicalScore = lexicalSimilarity(queryTerms, candidateTerms);
    ...
    const eligible = reranked.filter((chunk) => chunk.score >= (request.settings.scoreThreshold ?? 0.08));

    // Per-document candidate cap: without it a single lexically dominant
    // document can monopolize the candidate pool (measured 2026-07-31: one
    // brand best-practice file held 15/24 candidate slots on a schema
    // subquery, crowding schema/official docs out entirely — schema-target
    // claim recall 0.25). The cap scales with how many documents are in the
    // request, and single-document requests are never constrained.
    const maxChunks = request.settings.maxChunks ?? DEFAULT_PDP_GEO_RAG_MAX_CHUNKS;
    const distinctSources = new Set(eligible.map((chunk) => chunk.source)).size;
    const perSourceCap = distinctSources <= 1
      ? Number.POSITIVE_INFINITY
      : Math.max(4, Math.ceil(maxChunks / distinctSources));
    const selected: typeof eligible = [];
    const perSourceCounts = new Map<string, number>();
    for (const chunk of eligible) {
      const count = perSourceCounts.get(chunk.source) ?? 0;
      if (count >= perSourceCap) {
        continue;
      }
      perSourceCounts.set(chunk.source, count + 1);
      selected.push(chunk);
  ...
  /**
   * Provider embeddings when configured (cached by text hash); deterministic
   * hash-bag fallback otherwise.
   *
   * The two spaces want different query text. The hash embedding is a bag of
   * words, so it needs `lexicalQueryText` — the terminology-expanded token
   * stream that bridges a Korean query to the English corpus. A provider model
   * reads prose, and the candidate side is prose, so feeding it the flattened
   * token stream would compare two different registers; it gets `queryText`,
   * the query as written.
   */
  private async computeSemanticScores(queryText: string, lexicalQueryText: string, candidateTexts: string[]): Promise<number[]> {
    const scoreWithHashEmbedding = (): number[] => {
      const queryEmbedding = embedText(lexicalQueryText);
      return candidateTexts.map((text) => cosineSimilarity(queryEmbedding, embedText(text)));
    };

    if (!this.embedder) {
      return scoreWithHashEmbedding();
    }
    try {
      const [queryEmbedding, ...candidateEmbeddings] = await embedWithCache(this.embedder, [queryText, ...candidateTexts]);
      if (!queryEmbedding) {
        throw new Error("empty query embedding");
      }
      return candidateEmbeddings.map((embedding) => cosineSimilarity(queryEmbedding, embedding));
    } catch {
      // Provider failure must not break generation; fall back deterministically.
      return scoreWithHashEmbedding();
    }
  }
}
...
const maxEmbeddingCacheEntries = 4096;
const embedderNamespaces = new WeakMap<PdpGeoRagEmbedder, string>();
let embedderNamespaceSequence = 0;

/**
 * Cache namespace for one vector space. A cosine similarity is only meaningful
 * between vectors from the same model, so a cached vector must never be served
 * to a different embedder: declared models share a namespace, undeclared ones
 * fall back to per-instance isolation.
 */
function resolveEmbedderNamespace(embedder: PdpGeoRagEmbedder): string {
  if (embedder.model) {
    return `model:${embedder.model}`;
  }
  const existing = embedderNamespaces.get(embedder);
  if (existing) {
    return existing;
  }
  embedderNamespaceSequence += 1;
  const namespace = `instance:${embedderNamespaceSequence}`;
  embedderNamespaces.set(embedder, namespace);
  return namespace;
}

async function embedWithCache(embedder: PdpGeoRagEmbedder, texts: string[]): Promise<number[][]> {
  const namespace = resolveEmbedderNamespace(embedder);
  const missing: Array<{ index: number; text: string; key: string }> = [];
  const results: Array<number[] | undefined> = texts.map((text, index) => {
    const key = `${namespace}|${stableHash(text)}:${text.length}`;
  ...
  if (missing.length > 0) {
    const embedded = await embedder.embed(missing.map((item) => item.text));
    missing.forEach((item, position) => {
      const vector = embedded[position];
      if (Array.isArray(vector) && vector.length > 0) {
        results[item.index] = vector;
        if (embeddingCache.size < maxEmbeddingCacheEntries) {
          embeddingCache.set(item.key, vector);
        }
      }
    });
  }

  // A missing vector would score 0, below anything the hash space can produce,
  // so a partial provider result must demote the whole batch rather than
  // silently sink the items the provider dropped.
  const unresolved = results.filter((vector) => !vector || vector.length === 0).length;
  if (unresolved > 0) {
    throw new Error(`Embedder returned ${unresolved}/${texts.length} empty vector(s); falling back to deterministic local embeddings.`);
  }
...
/**
 * Builds a token expansion map from every terminology-map JSON document in the
 * request: all preferred wordings of one concept (across locales) plus the
 * concept id become one synonym group.
 */
function buildTerminologyExpansionMap(documents: PdpGeoRetrieverRequest["documents"]): Map<string, string[]> {
  const expansions = new Map<string, Set<string>>();

  for (const document of documents) {
    if (!/terminology/i.test(document.name) || !document.content.trim().startsWith("{")) {
      continue;
    }
    let parsed: { concepts?: Array<{ concept?: string; preferred?: Record<string, string[]> }> };
    try {
    ...
    for (const concept of parsed.concepts ?? []) {
      ...
      for (const token of tokenize(concept.concept ?? "")) {
        group.add(token);
      }
      for (const terms of Object.values(concept.preferred ?? {})) {
      ...
      for (const token of group) {
        const existing = expansions.get(token) ?? new Set<string>();
        for (const synonym of group) {
          existing.add(synonym);
        }
        expansions.set(token, existing);
      }
    }
  }

  return new Map(Array.from(expansions.entries()).map(([token, group]) => [token, Array.from(group)]));
}

function expandTokensWithTerminology(tokens: string[], expansions: Map<string, string[]>): string[] {
  if (expansions.size === 0) {
    return tokens;
  }
  const expanded = new Set(tokens);
  for (const token of tokens) {
    for (const synonym of expansions.get(token) ?? []) {
...
export class OpenAiVectorStoreRetriever implements PdpGeoRetriever {
  ...
  async retrieve(request: PdpGeoRetrieverRequest): Promise<PdpGeoRetrievedChunk[]> {
    ...
    const response = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        query: request.query,
        max_num_results: request.settings.maxChunks ?? DEFAULT_PDP_GEO_RAG_MAX_CHUNKS,
...
function chunkDocument(name: string, content: string, version = "v1"): PdpGeoRagChunk[] {
  ...
  return sections.map((section, index) => {
    ...
    const indexedSection = findPdpGeoRagSectionEntry(name, section.title, section.headingPath);
...
function splitMarkdownSections(content: string): Array<{ title?: string; headingPath?: string; text: string }> {
  ...
  for (const line of lines) {
    ...
    if (heading) {
      pushSection(sections, title, headingPath, buffer);
...
/**
 * Pushes a buffered section, skipping heading-only sections (a heading line
 * immediately followed by a child heading). Those produced ~30 noise chunks
 * across the corpus that competed for retrieval slots while carrying no body;
 * the parent heading context is preserved in every child chunk's headingPath.
 */
function pushSection(
  sections: Array<{ title?: string; headingPath?: string; text: string }>,
  ...
  sections.push({ title, headingPath, text });
}
...
function kindFromName(name: string): PdpGeoRagChunk["kind"] {
  ...
  // Checked before the topic heuristics below: a contract document names the
  // fields it governs, so "schema"/"cep"/"best" would otherwise claim it.
  if (/field-contract|content-field/i.test(name)) {
...
function inferRagChunkRouting(
  ...
  if (/claim|evidence|source-supported|reported result|clinical|metric|award|certification|trust|trustworthiness|expertise|authoritativeness|additionalproperty|propertyvalue|효능|효과|근거/.test(haystack)) {
    intents.add("claims");
...
function retrievalBoost(chunk: PdpGeoRagChunk, locale: PdpGeoLocale, market?: string): number {
  ...
  const indexedSection = findPdpGeoRagSectionEntry(
    chunk.source,
    chunk.title,
  ...
  if (chunk.kind === "field-contracts" || chunk.kind === "best-practice" || chunk.kind === "geo-research") {
    boost += 0.08;
  }
...
function lightweightRerankBoost(chunk: PdpGeoRagChunk, request: PdpGeoRetrieverRequest): number {
  ...
  // Typed subquery alignment: applied before the candidate cut so that
  // target-relevant guidance survives lexically dominant brand/product chunks.
  // (Benchmark evidence: schema-target claim recall 0.25 when this alignment
  // only happened after retrieval — see evals/baseline.json 2026-07-31.)
  if (request.queryIntents?.length && chunk.intents?.some((intent) => request.queryIntents?.includes(intent))) {
    boost += 0.06;
  }
  if (request.queryFieldTargets?.length && chunk.fieldTargets?.some((fieldTarget) => request.queryFieldTargets?.includes(fieldTarget))) {
    boost += 0.05;
  }
  ...
  // No field-contracts clause here on purpose: the contracts document declares
  // every generation intent and field target in rag-index, so it already earns
  // the generic intent/fieldTarget alignment boosts above, on top of the 0.08
  // kind boost in retrievalBoost. A kind clause too would double-count it.
...
export function lexicalSimilarity(queryTerms: string[], candidateTerms: string[]): number {
...
/**
 * Offline tooling exports: the ingestion scripts (embedding precompute,
 * rag-index skeleton generation) must chunk documents and build contextual
 * retrieval texts exactly like the runtime retriever so snapshot keys match.
 */
export const chunkPdpGeoRagDocument = chunkDocument;
export const createPdpGeoContextualRetrievalText = createContextualRetrievalText;
