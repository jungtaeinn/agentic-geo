...
- FAQ question shape is defined once in the EXAMPLEDERMA best-practice document; this overlay adds only the Korean surface. A question that points at the product with `이 제품`, `이 크림`, or `본 제품` instead of naming it cannot be attributed to this product in an AI answer. `상품 근거` and `근거 자료` are internal evidence labels, not words a shopper types. `공개된`/`발표된` overstates a PDP-reported study in Korean the way `published` does in English.
