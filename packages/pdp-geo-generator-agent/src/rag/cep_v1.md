...
Wherever this guidance and the content-field-contracts document state the same field rule, that contract holds the authoritative wording. What this document adds is which category entry points belong in which field and which causal links may be stated at all; the description stage order that sections 4.2 and 5.1 reason about is defined once in that contract.
...
CEP dimensions translate product facts and customer-review keywords into the natural buyer questions and situations behind `FAQPage.mainEntity` questions, review-intent FAQ answers, `Product.description` targeting, and `HowTo.step` routine context. Select only dimensions the product source supports.
...
- A source-stated property that rules out an ingredient's plain form is itself a selection condition, and the named formula answering it is the CEP `constraint`.
...
Use this rule set when composing the ordered `Product.description` and `WebPage.description` narrative flow, and when deciding whether an ingredient-to-benefit link may be stated at all.

- Prefer a compact causal path that can answer a buyer question end to end. Every stage the path keeps must rest on an explicitly supported link, and an official measurement or a positive/neutral review pattern joins the path only when the source supplies one.
...
- Where a formulation constraint and the named technology answering it are both stated, open the path with that pair. It answers the buyer question a formula name alone leaves open, and the outcome then follows as the resolution rather than as an unrelated fact.
...
- `WebPage.description`: identify the product page and source-backed brand, then carry only page-level CEPs — the customer situation and category-entry framing that make the page's scope legible to a shopper.
- `Product.description`: use product-specific CEPs — the target customer's concrete concern, the category-entry situation the product answers, and the representative review preference that shows how customers already use it. Missing evidence is omitted rather than replaced with a general category association.
...
- `HowTo.step`: a routine CEP is a reason to look for a usage step, not a step in itself. A general usage moment or a customer-review usage anecdote stays a CEP until the source supplies a direct product action for it, and the resulting steps keep the source's own count and order.

### 5.2 HTML Content

When HTML content rendering is enabled (it is currently disabled by default), the rendered HTML content block follows the same field mapping as the structured fields above:
...
- Update `WebPage.description`, `Product.description`, and `additionalProperty` only for fields touched by the new source evidence.
