    ...
    const visualAltHtml = `
      <main>
        ...
        <img
          ...
          alt="ExampleLuxe Gentle Cleansing Foam, facial cleanser, model applying product to face"
        />
      </main>
    `;
          ...
          <script type="application/ld+json">
            {
              ...
              "description": "Essential Activating Serum is a different serum for first-step hydration.",
...
elasticity and resilience helping to diminish visible signs of aging"
  ...
  it("keeps image OCR scoped to product-evidence sections instead of related commerce imagery", async () => {
    ...
    const routineTileImage = "https://cdn.example.com/product-card/essential-activating-serum.png";
      ...
      <main>
        ...
        <section class="routine-builder">
          ...
          <div class="product-tile product-tile--slider" role="group" aria-label="product">
            <img src="${routineTileImage}" alt="Essential Care Activating Serum routine step" />
          </div>
        </section>
  ...
  it("keeps clinical use timelines and untreated controls out of actionable usage", async () => {
    const usageImage = "https://assets.example.com/upload/editor/hydro-soothing-usage.png";
    const clinicalImage = "https://assets.example.com/upload/editor/hydro-soothing-clinical.png";
    const usageInstruction = "아침과 저녁 세안 후 크림 단계에서 적당량을 덜어 캡슐을 부드럽게 녹이듯 펴 바르고 가볍게 두드려 흡수시킵니다.";
    const clinicalTimeline = "사용 전 사용 직후 사용 12시간 후 깊고 오래 가는 속수분 충전 10층 속수분 +12시간 지속 1회 사용 후 72시간 수분 지속 제품 사용 무도포 사용 전 사용 직후 24시간 후 72시간 후 3일, 7일 사용 후 누적 증가하는 얼굴 수분량 글로벌의학연구센터 성인 여성 30명 대상 인체적용시험 결과";
    const fetchMock = vi.fn(async (_url: string | URL, init?: RequestInit) => {
      const body = JSON.parse(String(init?.body ?? "{}"));
      const content = body.input?.[0]?.content;
      const imageParts = Array.isArray(content) ? content.filter((part: { type?: string }) => part.type === "input_image") : [];
      ...
      if (imageParts.length > 0) {
        return new Response(JSON.stringify({
          output_text: JSON.stringify({
            images: [
              { imageUrl: usageImage, text: usageInstruction },
              { imageUrl: clinicalImage, text: clinicalTimeline }
            ]
          })
        }), { status: 200 });
      }
      ...
      return new Response(JSON.stringify({
        output_text: JSON.stringify({
          keywords: [
            { keyword: "아침과 저녁", category: "usage", confidence: 0.94, source: "llm" },
            { keyword: "무도포", category: "usage", confidence: 0.9, source: "llm" },
            { keyword: "72시간", category: "metric", confidence: 0.9, source: "llm" }
          ],
          sentenceInsights: [
            { text: usageInstruction, category: "usage", keywords: ["아침과 저녁", "적당량", "흡수"], confidence: 0.94, source: "llm" },
            { text: clinicalTimeline, category: "usage", keywords: ["제품 사용", "무도포", "72시간"], confidence: 0.9, source: "llm" }
          ],
          semanticFacts: {
            usageSteps: [usageInstruction, clinicalTimeline],
            metricClaims: [{ sentence: clinicalTimeline, sourceText: clinicalTimeline }]
          },
          summary: "classified"
        })
      }), { status: 200 });
    ...
    expect(result.geoProduct.usage).toEqual([usageInstruction]);
    expect(result.geoProduct.semanticFacts?.usageSteps).toEqual([usageInstruction]);
    expect(result.geoProduct.ocr.keywords.usage).not.toContain("무도포");
    expect(result.geoProduct.contentAnalysis.sections.find((section) => section.text.includes("무도포"))?.category).toBe("effect");
  });
