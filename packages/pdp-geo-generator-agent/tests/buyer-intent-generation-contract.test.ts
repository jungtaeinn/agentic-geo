...
import { ensurePdpGeoFaqPlanCoverage, generatePdpGeoArtifacts } from "../src/generate";
...
describe("buyer-intent generation contracts", () => {
  ...
  it("publishes one source usage instruction as exactly one HowTo step", async () => {
    ...
    expect(run.result.content.sections.howToUse.trim()).toBe("Apply one pump to the face and gently pat until absorbed");
  });

  it("gives a HowTo step a distinct name instead of repeating the full step text", async () => {
    const run = await generatePdpGeo({
      product: {
        name: "Hydra Serum",
        description: "A hydrating face serum for dry-feeling skin.",
    ...
    // A step "name" that is just a truncated (or exact) copy of "text" is not
    // a real label -- it duplicates the same sentence under a second field.
    expect(steps[0]?.name).not.toBe(steps[0]?.text);
    expect(String(steps[0]?.name)).not.toContain("Apply one pump");
  });
  ...
  it("normalizes a Korean 단계 marker to canonical numbering without reading a dose as one", () => {
    // English "Step 1:" already survives normalization as "1. …". The Korean
    // suffix form must reach the same canonical shape, so ordering does not
    // depend on the raw page text happening to be captured alongside it.
    const numbered = normalizePdpProduct({
      name: "리페어 크림",
      description: "건조한 피부를 위한 크림입니다.",
      category: "크림",
      usage: ["1단계 적당량을 손에 덜어냅니다.", "2단계 얼굴 전체에 펴 바릅니다."]
    }, { hints: { locale: "ko-KR" } }).product;

    expect(numbered.usage).toEqual([
      "1. 적당량을 손에 덜어냅니다.",
      "2. 얼굴 전체에 펴 바릅니다."
    ]);

    // A bare number before a space is a dose, not a step marker. Treating it
    // as one deleted the amount the instruction told the customer to use.
    const dose = normalizePdpProduct({
      name: "리페어 크림",
      description: "건조한 피부를 위한 크림입니다.",
      category: "크림",
      usage: ["2 펌프를 손에 덜어 얼굴 전체에 펴 바릅니다."]
    }, { hints: { locale: "ko-KR" } }).product;

    expect(dose.usage).toEqual(["2 펌프를 손에 덜어 얼굴 전체에 펴 바릅니다."]);
  });

  it("keeps a Korean 단계-numbered procedure at its source step count", async () => {
    const run = await generatePdpGeo({
      product: {
        name: "리페어 크림",
        brand: "테스트랩",
        ...
        category: "크림",
        benefits: ["보습"],
        // "1단계" ends at whitespace rather than punctuation. The marker is
        // stripped from the instruction either way, so a marker form that goes
        // unrecognised silently loses the ordering the source stated.
        usage: [
          "1단계 손끝으로 적당량을 덜어 얼굴 전체에 펴 바릅니다.",
          "2단계 손바닥으로 감싸 흡수시킵니다."
        ]
      },
    ...
    expect(steps.map((step) => step.position)).toEqual([1, 2]);
    expect(steps.map((step) => String(step.text))).toEqual([
      "손끝으로 적당량을 덜어 얼굴 전체에 펴 바릅니다",
      "손바닥으로 감싸 흡수시킵니다"
    ]);
    // The ordinal suffix must not survive as literal copy in the step text.
    expect(JSON.stringify(steps)).not.toMatch(/"text":\s*"단계/u);
  });

  it("labels HowTo steps by position instead of guessing a descriptive label", async () => {
    const run = await generatePdpGeo({
      product: {
        name: "Hydra Cleanser",
        description: "A cleanser for dry-feeling skin.",
        category: "Cleanser",
        benefits: ["hydration"],
        usage: [
          "Step 1: Lather with water and massage onto damp skin.",
    ...
    expect(steps.map((step) => String(step.name))).toEqual(["Step 1", "Step 2"]);
    // A keyword-guessed label contradicted the step it labelled: an unanchored
    // "warm" matched inside "lukewarm" and titled a rinse step "Warm and apply".
    expect(JSON.stringify(steps)).not.toMatch(/Warm and apply|Press to absorb|Apply after toner/u);
  });
  ...
  it("recovers the first step of a numbered procedure whose leading marker was stripped", async () => {
    const run = await generatePdpGeo({
      product: {
        ...
        // The source numbered its steps "사용법 1 … 2 …", but extraction split
        // the "사용법 1" heading away, leaving only the "2" marker embedded in
        // one usage string. That surviving marker still proves source order,
        // so the text before it must stay its own step instead of collapsing
        // both actions into a single merged note.
        usage: [
          "연약하고 건조해진 피부 부위에 미세 분사하고, 피부에 건조함이 느껴질 때 수시로 뿌려 사용한다.",
          "연약하고 건조해진 피부 부위에 미세 분사를 합니다. 2 피부에 건조함이 느껴질 때 수시로 뿌려줍니다"
        ]
      },
      hints: { locale: "ko-KR", market: "KR" }
    });
    ...
    expect(steps.map((step) => step.position)).toEqual([1, 2]);
    expect(steps.map((step) => String(step.text))).toEqual([
      "연약하고 건조해진 피부 부위에 미세 분사를 합니다",
      "피부에 건조함이 느껴질 때 수시로 뿌려줍니다"
    ]);
    // The recovered marker must not survive as literal copy in the step text.
    expect(JSON.stringify(steps)).not.toMatch(/"text":\s*"2\s/u);
  ...
  it("turns label-heavy OCR citation metadata into natural Korean prose", async () => {
    ...
    for (const type of ["Product", "WebPage"]) {
      const description = String(nodes.find((node) => (Array.isArray(node["@type"]) ? node["@type"].includes(type) : node["@type"] === type))?.description);
  ...
  it("does not infer a reverse-FAQ concern from ingredient or general source-text co-occurrence", async () => {
    ...
    // The composition and benefit-only anchors both ask "what does this
    // product do" and are merged into one canonical composition+benefit
    // question rather than publishing both as near-duplicate FAQ entries.
    expect(questions.join("\n")).toMatch(/글로우 세럼의 구성 성분과 효능·효과는 무엇인가요/u);
    expect(questions.join("\n")).not.toMatch(/상품 근거|인체적용시험/u);
    expect(questions.join("\n")).not.toMatch(/(?:건조|민감)한 피부가 고민인 고객/u);
  });

  it("merges the composition and plain benefit FAQ anchors into one entry instead of two near-duplicate questions", async () => {
    const run = await generatePdpGeo({
      product: {
        name: "하이드라 세럼",
    ...
    expect(questions).not.toContain("하이드라 세럼의 주요 효능·효과는 무엇인가요?");
    expect(questions.filter((question) => /효능|효과/u.test(question))).toHaveLength(1);
  });

  it("keeps the composition and clinical-evidence benefit FAQ anchors separate because they answer distinct questions", async () => {
    const run = await generatePdpGeo({
      product: {
        name: "하이드라 세럼",
        description: "건조한 피부에 수분을 공급하는 세럼입니다.",
        ...
        ingredients: ["세라마이드"],
        benefits: ["수분 공급"],
        metrics: ["인체적용시험에서 32명을 대상으로 사용 4주 후 수분량이 30% 개선되었습니다."],
        semanticFacts: {
          metricClaims: [{
            label: "수분량 개선",
            subject: "수분량",
            ...
            method: "인체적용시험",
            institution: "㈜테스트연구소",
            sentence: "인체적용시험에서 32명을 대상으로 사용 4주 후 수분량이 30% 개선되었습니다.",
            sourceText: "인체적용시험에서 32명을 대상으로 사용 4주 후 수분량이 30% 개선되었습니다."
          }]
        }
      },
      hints: { locale: "ko-KR", market: "KR" }
  ...
  it("drops a model-planned plain benefit question that the composition FAQ already answers", () => {
    ...
    const plainBenefitQuestion = "하이드라 세럼의 주요 효능·효과는 무엇인가요?";
    // A plain "what does this product do" question carries no ingredient,
    // audience, evidence, usage, or review angle of its own, so the
    // composition-and-benefits anchor answers it in full. Coverage merging
    // must drop it instead of publishing both.
    const plan: PdpGeoContentPlan = {
      ...
      faq: [{
        include: true,
        question: plainBenefitQuestion,
        answer: "하이드라 세럼은 수분 케어를 돕습니다.",
    ...
    const questions = ensurePdpGeoFaqPlanCoverage({
      plan,
      product,
      locale: "ko-KR",
      market: "KR",
    ...
    expect(questions).toContain("하이드라 세럼의 구성 성분과 효능·효과는 무엇인가요?");
    expect(questions).not.toContain(plainBenefitQuestion);
  });
