...
export interface ValidateAndRepairInput {
  ...
  locale?: PdpGeoLocale;
  /** Optional normalized source facts used by read-only semantic audits. */
  sourceProduct?: PdpProductSignal;
}
...
const allowedGraphTypes = new Set(["WebPage", "ItemPage", "CollectionPage", "AboutPage", "Product", "FAQPage", "HowTo", "BreadcrumbList", "Question", "Answer", "HowToStep", "Offer", "AggregateRating", "Review", "Rating", "PropertyValue", "ItemList", "ListItem", "Brand", "Person"]);
...
export interface SafePublicCopyRepairResult {
  schemaMarkup: PdpGeoSchemaMarkup;
  content: PdpGeoContentArtifact;
  appliedRepairs: PdpGeoValidationRepair[];
}

/**
 * Applies only the deterministic, meaning-preserving sentence-QA repair class
 * to public copy: text-artifact normalization (repairGeneratedText) and Usage
 * step deduplication. Structural graph repairs, property renames/removals,
 * and trust rewrites are deliberately NOT applied — they change claim content
 * or graph shape, so they stay read-only findings from
 * validatePdpGeoArtifacts. Run this BEFORE the read-only validator so its
 * findings describe the published artifacts.
 */
export function applySafePublicCopyRepairs(input: ValidateAndRepairInput): SafePublicCopyRepairResult {
  const locale = input.locale ?? "en-US";
  const warnings: string[] = [];
  const appliedRepairs: PdpGeoValidationRepair[] = [];
  const jsonLd = JSON.parse(JSON.stringify(input.schemaMarkup.jsonLd)) as JsonObject;
  const graph: Array<Record<string, unknown>> = Array.isArray(jsonLd["@graph"])
    ? jsonLd["@graph"].flatMap((node) => isRecord(node) ? [node] : [])
    : [];

  for (const node of graph) {
    const isProduct = schemaNodeHasType(node, "Product");
    if (typeof node.description === "string" && (isProduct || schemaNodeHasType(node, "WebPage"))) {
      node.description = repairGeneratedText(
        node.description,
        locale,
        isProduct ? "Product.description" : "WebPage.description",
        warnings,
        appliedRepairs
      );
    }
    if (isProduct && Array.isArray(node.additionalProperty)) {
      node.additionalProperty = node.additionalProperty.flatMap((property, index) => {
        if (!isRecord(property) || typeof property.name !== "string" || typeof property.value !== "string") {
          return [property];
        }
        let value = repairGeneratedText(property.value, locale, `Product.additionalProperty[${index}].value`, warnings, appliedRepairs);
        if (property.name === "Usage") {
          const deduped = repairUsagePropertyValue(value, locale);
          // A trailing-punctuation-only difference is a style preference, not
          // a repair — only apply when dedup/marker stripping changed content.
          if (deduped && deduped !== value && deduped !== value.replace(/[.。]+\s*$/gu, "").trim()) {
            addRepair(warnings, appliedRepairs, {
        ...
        // A PropertyValue whose text collapses to empty (artifact-only value,
        // or already-empty input) carries no claim — publishing value: ""
        // breaks the property contract, so the entry is dropped instead.
        if (value.trim().length === 0) {
          addRepair(warnings, appliedRepairs, {
            field: `Product.additionalProperty.${property.name}`,
            source: "sentence-qa",
            issue: "PropertyValue text was empty after artifact-only content was removed.",
            action: "Removed the additionalProperty entry instead of publishing an empty value.",
            before: property.value,
            after: null,
            evidence: [`Product.additionalProperty.${property.name}`, "PropertyValue content contract", locale]
          }, `Empty ${property.name} PropertyValue was removed during final sentence QA.`);
          return [];
        }
        return [{ ...property, value }];
      });
    }
    if (schemaNodeHasType(node, "FAQPage") && Array.isArray(node.mainEntity)) {
      node.mainEntity = node.mainEntity.map((item) => {
        if (!isRecord(item)) {
          return item;
        }
        const next: Record<string, unknown> = { ...item };
        if (typeof next.name === "string") {
          next.name = repairGeneratedText(next.name, locale, "FAQPage.mainEntity.name", warnings, appliedRepairs);
        }
        if (isRecord(next.acceptedAnswer) && typeof next.acceptedAnswer.text === "string") {
          next.acceptedAnswer = {
            ...next.acceptedAnswer,
            text: repairGeneratedText(next.acceptedAnswer.text, locale, "FAQPage.mainEntity.acceptedAnswer.text", warnings, appliedRepairs)
          };
        }
        return next;
      });
    }
    if (schemaNodeHasType(node, "HowTo") && Array.isArray(node.step)) {
      node.step = node.step.map((item) => isRecord(item) && typeof item.text === "string"
        ? { ...item, text: repairGeneratedText(item.text, locale, "HowTo.step.text", warnings, appliedRepairs) }
        : item);
    }
  }

  // Content sections mirror the public copy (they feed the eval surface), so
  // they get the same text normalization — but not the structural section
  // contracts, which can drop or move content.
  const sections: PdpGeoContentSections = {
    ...input.content.sections,
    productName: repairGeneratedText(input.content.sections.productName, locale, "content.sections.productName", warnings, appliedRepairs),
    description: repairGeneratedText(input.content.sections.description, locale, "content.sections.description", warnings, appliedRepairs),
    quickFacts: repairMultilineGeneratedText(input.content.sections.quickFacts, locale, "content.sections.quickFacts", warnings, appliedRepairs),
    benefits: repairMultilineGeneratedText(input.content.sections.benefits, locale, "content.sections.benefits", warnings, appliedRepairs),
    ingredients: repairMultilineGeneratedText(input.content.sections.ingredients, locale, "content.sections.ingredients", warnings, appliedRepairs),
    howToUse: repairMultilineGeneratedText(input.content.sections.howToUse, locale, "content.sections.howToUse", warnings, appliedRepairs),
    faq: repairMultilineGeneratedText(input.content.sections.faq, locale, "content.sections.faq", warnings, appliedRepairs)
  };
...
function collectPublicWordingFindings(jsonLd: JsonObject, locale: PdpGeoLocale, sourceProduct?: PdpProductSignal): PdpGeoValidationFinding[] {
  ...
  graph.forEach((node, nodeIndex) => {
    const type = stringValue(node["@type"])
      ?? (Array.isArray(node["@type"]) ? stringValue(node["@type"][0]) : undefined)
      ?? `node[${nodeIndex}]`;
    if (typeof node.description === "string") entries.push({ field: `${type}.description`, text: node.description });
    if (Array.isArray(node.additionalProperty)) {
      node.additionalProperty.forEach((property, index) => {
        if (!isRecord(property) || typeof property.value !== "string") return;
        entries.push({ field: `Product.additionalProperty[${index}].value`, text: property.value });
      });
    }
    if (type === "FAQPage" && Array.isArray(node.mainEntity)) {
      node.mainEntity.forEach((item, index) => {
        if (!isRecord(item)) return;
        const answer = isRecord(item.acceptedAnswer) ? item.acceptedAnswer : undefined;
        if (typeof item.name === "string") entries.push({ field: `FAQPage.mainEntity[${index}].name`, text: item.name });
        if (answer && typeof answer.text === "string") entries.push({ field: `FAQPage.mainEntity[${index}].acceptedAnswer.text`, text: answer.text });
      });
    }
    if (type === "HowTo" && Array.isArray(node.step)) {
      node.step.forEach((item, index) => {
        if (isRecord(item) && typeof item.text === "string") entries.push({ field: `HowTo.step[${index}].text`, text: item.text });
      });
    }
  });

  const findings: PdpGeoValidationFinding[] = [];
  const hasQualitativeReviewEvidence = Boolean(sourceProduct && (
    sourceProduct.reviews.items.some((item) => item.body.trim().length >= 20)
  ...
  for (const entry of entries) {
    ...
    if (/\((?:timing|sample|method|baseline|comparator|period)\s+[^)]*(?:,\s*(?:timing|sample|method|baseline|comparator|period)\s+[^)]*)+\)/iu.test(entry.text)) {
      findings.push({
        field: entry.field,
        source: "sentence-qa",
        issue: "Public copy exposed structured metric metadata as a label/value parenthetical instead of a complete sentence.",
        suggestedAction: "Render the metric through the locale-specific structured-result formatter.",
        before: entry.text,
        evidence: ["semanticFacts.metricClaims", "public sentence contract"]
      });
    }
    if (/(?:^|[.!?]\s+)(?:improvement\b|[Ii]nstrumental\s+result\b|timing\b|sample\b|method\b)/u.test(entry.text)) {
      findings.push({
        field: entry.field,
        source: "sentence-qa",
        issue: "English public copy contained a report-style or lowercase dependent sentence fragment.",
        suggestedAction: "Rewrite the fragment as a complete customer-facing sentence.",
        before: entry.text,
        evidence: ["English sentence completeness contract"]
      });
    }
    // Claim-modality contract (E-E-A-T §3.2): self-assessment agreement must
    // never be published as an objective improvement reading.
    if (/(?:self[-\s]?assessment|자가\s*평가)(?:(?!instrumental|기기\s*측정|임상|clinical\s+stud)[^.!?;]){0,120}(?:\b(?:showed|demonstrated|confirmed|proved|saw|exhibited)\b[^.!?;]{0,60}\bimprovement|\bimproved\s+by\b|개선(?:을|이)?\s*(?:확인|입증|보였))/iu.test(entry.text)) {
      findings.push({
        field: entry.field,
        source: "trust-field-validator",
        issue: "Self-assessment agreement wording was upgraded to an objective improvement claim.",
        suggestedAction: "Preserve the source modality for self-assessment results (e.g. \"96% agreed skin felt smoother\", not \"96% showed improvement\").",
        before: entry.text,
        evidence: ["semanticFacts.metricClaims.method", "claim-modality contract"]
      });
    }
    if (/clinical\s+self[-\s]?assessment|self[-\s]?assessment\s+(?:from|of|via|in)\s+(?:a\s+)?clinical|임상\s*(?:시험)?\s*(?:기반|의)?\s*자가\s*평가/iu.test(entry.text)) {
      findings.push({
        field: entry.field,
        source: "trust-field-validator",
        issue: "Self-assessment was attributed to a clinical method in the same phrase.",
        suggestedAction: "Use one accurate method label per claim; a self-assessment must not borrow clinical status.",
        before: entry.text,
        evidence: ["claim-modality contract"]
      });
    }
    if (sourceProduct && !hasQualitativeReviewEvidence && /(?:customer|positive)\s+reviews?\s+(?:mention|highlight|use|describe)|customers?\s+highlight/iu.test(entry.text)) {
      findings.push({
        field: entry.field,
        source: "trust-field-validator",
  ...
  const faqPage = graph.find((node) => stringValue(node["@type"]) === "FAQPage");
  if (sourceProduct && faqPage && Array.isArray(faqPage.mainEntity)) {
    faqPage.mainEntity.forEach((item, index) => {
      ...
      if (/clinical\s+stud(?:y|ies)/iu.test(item.name) && /instrumental\s+(?:result|assessment)/iu.test(item.acceptedAnswer.text)) {
        findings.push({
          field: `FAQPage.mainEntity[${index}].name`,
          source: "trust-field-validator",
          issue: "FAQ wording upgraded an instrumental result to a clinical study.",
          suggestedAction: "Use the source method label, such as instrumental assessment, without strengthening it.",
          before: item.name,
          evidence: ["semanticFacts.metricClaims.method", "claim-modality contract"]
        });
      }
  ...
  const productNode = resolvePrimaryProductNode(graph);
  const webPageNode = graph.find((node) => schemaNodeHasType(node, "WebPage"));

  if (productNode && Array.isArray(productNode.additionalProperty)) {
    const propertyEntries = productNode.additionalProperty.flatMap((property, index) =>
      isRecord(property) && typeof property.name === "string" && typeof property.value === "string"
        ? [{ index, name: property.name, value: property.value }]
        : []);

    // Method-accurate property naming: a summary built on self-assessment
    // evidence must not carry the "Clinical result summary" label.
    for (const property of propertyEntries) {
      if (/^Clinical result summary$/i.test(property.name)
        && /(?:self[-\s]?assessment|자가\s*평가)/iu.test(property.value)
        && !/(?:instrumental|clinical\s+(?:study|trial|test)|인체\s*적용\s*시험|임상\s*(?:시험|평가))/iu.test(property.value)) {
        findings.push({
          field: `Product.additionalProperty[${property.index}].name`,
          source: "trust-field-validator",
          issue: "A self-assessment-based summary was labeled \"Clinical result summary\".",
          suggestedAction: "Rename the property to \"Reported assessment summary\" or keep only clinically sourced results under the clinical label.",
    ...
    // Key ingredients must stay source-backed ingredient names: broken tokens
    // ("What") and cross-product bleed ("Retinol" absent from this product's
    // evidence) both undermine the entity density engines cite.
    const keyIngredientProperties = propertyEntries.filter((property) => property.name === "Key ingredients");
    const evidenceCorpus = sourceProduct && keyIngredientProperties.length > 0 ? normalizeIngredientEvidence([
      sourceProduct.name,
      sourceProduct.originalName ?? "",
      sourceProduct.description ?? "",
      ...sourceProduct.ingredients,
      ...sourceProduct.benefits,
      ...sourceProduct.effects,
      ...sourceProduct.usage,
      ...sourceProduct.metrics,
      ...sourceProduct.options,
      ...sourceProduct.sourceTexts,
      ...(sourceProduct.semanticFacts?.ingredients ?? []),
      ...(sourceProduct.semanticFacts?.ingredientBenefitLinks ?? []).flatMap((link) => [link.ingredient ?? "", link.sentence ?? ""]),
      ...sourceProduct.reviews.keywords,
      ...sourceProduct.faq.flatMap((item) => [item.question, item.answer])
    ].join(" ")) : undefined;
    const corpusHasHangul = evidenceCorpus ? /[가-힣]/.test(evidenceCorpus) : false;
    for (const property of keyIngredientProperties) {
      for (const rawToken of property.value.split(listSeparatorCommaPattern)) {
        const token = rawToken.trim();
        if (!token) continue;
        // Localized surfaces (ko source → en token or vice versa) cannot be
        // substring-checked against the source corpus; skip the source-backed
        // check across scripts to avoid flagging every translated ingredient.
        const tokenHasHangul = /[가-힣]/.test(token);
        if (/^(?:what|which|how|why|when|where|who|it|this|that|these|those|other|others|etc|and|or|more|misc|unknown|n\/a|무엇|어떤|기타|그\s*외|등|외)$/i.test(token)) {
          findings.push({
            field: `Product.additionalProperty[${property.index}].value`,
            source: "trust-field-validator",
            issue: `Key ingredients contained a non-ingredient token ("${token}").`,
        ...
        } else if (evidenceCorpus && tokenHasHangul === corpusHasHangul && token.length >= 3 && !evidenceCorpus.includes(normalizeIngredientEvidence(token))) {
          findings.push({
            field: `Product.additionalProperty[${property.index}].value`,
            source: "trust-field-validator",
            issue: `Key ingredients listed "${token}", which does not appear in this product's source evidence.`,
            suggestedAction: "Keep only ingredients backed by this product's own source data; cross-product ingredient bleed is misleading.",
            before: property.value,
            evidence: ["product source evidence", "E-E-A-T evidence hierarchy"]
          });
        }
      }
    }

    // Entity consistency: "all skin types" and an enumerated recommended
    // skin-type list must not coexist in the same public graph.
    const skinScopeEntries = [
      ...propertyEntries.map((property) => ({ field: `Product.additionalProperty[${property.index}].value`, text: property.value })),
      ...entries.filter((entry) => entry.field.startsWith("FAQPage."))
    ];
    const broadScopePattern = /all\s+skin\s+types|모든\s*피부(?!\s*고민)/iu;
    const broadScope = skinScopeEntries.find((entry) => broadScopePattern.test(entry.text));
    // Only a *recommendation* field narrows the scope; a sentence like
    // "all skin types, including dry skin" is inclusion, not contradiction.
    const enumeratedScope = skinScopeEntries.find((entry) =>
      /Recommended skin type|Target customer/i.test(entry.field.includes("additionalProperty")
        ? (propertyEntries.find((property) => entry.field === `Product.additionalProperty[${property.index}].value`)?.name ?? "")
        : "")
      && !broadScopePattern.test(entry.text)
      && !/including|such as|포함|비롯/iu.test(entry.text)
      && (/(?:dry|normal|combination|oily|sensitive)\s+skin[^.!?]{0,80}(?:dry|normal|combination|oily|sensitive)\s+skin/iu.test(entry.text)
        || /(?:건성|중성|지성|복합성|민감성)\s*피부[^.!?]{0,40}(?:건성|중성|지성|복합성|민감성)/u.test(entry.text)));
    if (broadScope && enumeratedScope && broadScope.field !== enumeratedScope.field) {
      findings.push({
        field: broadScope.field,
        source: "trust-field-validator",
        issue: "The graph claims \"all skin types\" while another field enumerates a narrower recommended skin-type list.",
        suggestedAction: "Align the skin-type scope to the source data and use the same scope in every field.",
        before: broadScope.text,
        evidence: [enumeratedScope.field, "entity consistency contract"]
      });
    }
  }

  // Role separation (geo-research §5.3): a long claim sentence should live in
  // one description, not be repeated verbatim across WebPage and Product.
  if (productNode && webPageNode
    && typeof productNode.description === "string" && typeof webPageNode.description === "string") {
    const productSentences = splitPublicSentences(productNode.description)
      .map((sentence) => ({ raw: sentence.trim(), normalized: normalizeDescriptionForRoleComparison(sentence) }))
      .filter((sentence) => sentence.normalized.length >= 90);
    const webPageSentences = new Set(splitPublicSentences(webPageNode.description)
      .map(normalizeDescriptionForRoleComparison)
      .filter((sentence) => sentence.length >= 90));
    const duplicated = productSentences.find((sentence) => webPageSentences.has(sentence.normalized));
    if (duplicated) {
      findings.push({
        field: "WebPage.description",
        source: "field-contract-validator",
        issue: "A long claim sentence is duplicated verbatim across WebPage.description and Product.description.",
        suggestedAction: "Keep the full claim in Product.description and summarize the page-side coverage instead of repeating it.",
        before: duplicated.raw,
        evidence: ["WebPage.description", "Product.description", "WebPage/Product role separation"]
      });
    }
  }
...
function normalizeIngredientEvidence(value: string): string {
  return value
    .toLowerCase()
    .replace(/[™®©]/g, "")
...
function repairJsonLd(
  ...
  const product = resolvePrimaryProductNode(graph) as Record<string, unknown> | undefined;
  if (!product) {
    const addedProduct = {
      "@type": "Product",
      "@id": `urn:agentic-geo:pdp:${slug(fallbackProductName)}#product`,
...
function repairSchemaEvidenceConsistency(
  ...
  for (const node of graph) {
    const type = schemaNodeHasType(node, "Product") ? "Product" : schemaNodeHasType(node, "WebPage") ? "WebPage" : undefined;
    if (!type) {
      continue;
    }
...
function repairSchemaDescriptionRoleSeparation(
  ...
  const product = resolvePrimaryProductNode(graph) as Record<string, unknown> | undefined;
  const webPage = graph.find((node) => schemaNodeHasType(node, "WebPage"));
  if (!product) {
    return;
  }
...
function repairGraphNode(
  ...
  const typeValues = typeof rawType === "string" ? [rawType] : Array.isArray(rawType) ? rawType.filter((item): item is string => typeof item === "string") : [];
  for (const unsupported of typeValues.filter((item) => !allowedGraphTypes.has(item))) {
    addRepair(warnings, repairs, {
      field: `${unsupported}.@type`,
      source: "schema-validator",
      issue: `Unsupported schema.org node type "${unsupported}" was detected.`,
      action: "Flagged the unsupported type for review; the node was preserved.",
      before: unsupported,
      evidence: ["allowedGraphTypes"]
    }, `Unsupported schema.org type "${unsupported}" was flagged.`);
  }
  ...
  if (schemaNodeHasType(node, "WebPage") && typeof node.description === "string") {
    node.description = dedupeRedundantMetricClausesWithRepair(
...
function normalizeOfferNode(value: unknown): Record<string, unknown> | Record<string, unknown>[] | undefined {
  ...
  if (Array.isArray(value)) {
    const normalized = value
      .map((item) => normalizeSingleOfferNode(item))
  ...
  return normalizeSingleOfferNode(value);
}

function normalizeSingleOfferNode(value: unknown): Record<string, unknown> | undefined {
  const offer = isRecord(value) ? value : undefined;
  if (!offer) {
    return undefined;
  }
...
function repairHowToUseSectionLines(lines: string[], _locale: PdpGeoLocale): string[] {
  ...
  // A numbered list implies a real source-ordered sequence; a single
  // surviving step has no order to show, so it stays a plain sentence.
  return results.length === 1 ? results : results.map((step, index) => `${index + 1}. ${step}`);
}
...
function shouldRepairSchemaString(path: string[], value: string): boolean {
  const key = path.at(-1);
  if (!key || ["@context", "@type", "@id", "url", "item", "priceCurrency", "datePublished", "dateModified"].includes(key)) {
    return false;
  }
...
function normalizePunctuationArtifacts(value: string): string {
  const normalized = value
    ...
    .replace(/,\s*\.\s*$/g, ".")
    .replace(/,\s*\.\s*/g, ", ")
    // 문장 결합 아티팩트: ";," 이중 구두점과 "and, and" 중복 접속사는
    // 모델 재작성/절 병합 과정에서 생기는 손상이다.
    .replace(/;\s*,/g, ";")
    .replace(/\band,?\s+and\b/gi, "and")
    .replace(/[ \t]{2,}/g, " ")
    .trim();
  return stripDanglingEnglishTail(normalized);
}

/**
 * 절 병합 절단으로 남은 영어 기능어 꼬리("... felt diminished in a")를
 * 제거한다. 종결 문장부호로 끝나는 온전한 문장에는 적용하지 않는다.
 */
function stripDanglingEnglishTail(value: string): string {
  if (/[.!?。！？]$/.test(value)) {
    return value;
  }
  const danglingTailPattern = /\s+(?:a|an|the|and|or|but|in|on|of|to|for|with|from|at|as)$/i;
  let next = value;
  while (danglingTailPattern.test(next)) {
    next = next.replace(danglingTailPattern, "").replace(/[,;:]$/g, "").trim();
  }
  return next;
}
...
function dedupeCommaListValue(value: string): string {
  const parts = value.split(listSeparatorCommaPattern);
...
function repairFunctionalCertificationPropertyValue(value: string, locale: PdpGeoLocale): string {
  ...
  // Same atomic list the renderer publishes from — publish and validate can
  // never disagree on which certification spellings are recognized.
  const signals = selectAtomicFunctionalCertificationValues(value, locale);
...
function repairGeneratedText(
  ...
  let next = value
    ...
    .replace(/\\[rn]/g, " ")
    .replace(/\u00a0/g, " ");
  // URL/image artifact sentences must be removed while URLs are still intact:
  // the sentence-punctuation spacing below would split a URL at its internal
  // periods and leave non-removable fragments such as "com/upload/\u2026".
...
function repairUsagePropertyValue(value: string, locale: PdpGeoLocale): string {
  ...
  if (steps.length === 1) {
    const step = steps[0] ?? value;
    // 끝 문장부호만 제거된 결과는 수리가 아니다 — 원문을 그대로 유지해
    // 문장부호 차이만으로 Usage 경고/수리가 발생하지 않게 한다.
    return step === value.trim().replace(/[.。]+$/gu, "") ? value.trim() : step;
  }
  if (locale === "ko-KR") {
...
function shouldRemoveKoreanMetaQuestionFragments(path: string): boolean {
  return /(?:description|quickFacts|benefits|ingredients|faq|acceptedAnswer|additionalProperty|reviewBody)/i.test(path)
...
function shouldRepairKoreanPublicEvidencePresentationPath(path: string): boolean {
  ...
  return /(?:description|additionalProperty|reviewBody|content\.sections\.(?:description|quickFacts|benefits)|value)$/u.test(path)
...
function isUrlArtifactRemainder(value: string): boolean {
  return /(?:exampleshop|fileupload|format\s*=\s*webp|\.?\s*jpe?g\s*\?|\bwebp\b|\bpng\b|\bgif\b)/i.test(value);
}
...
function isShortKoreanLabelValue(path: string, value: string): boolean {
  return /(?:additionalProperty\.\d+\.value)$/.test(path)
    && value.length <= 32
    && !/[.!?。！？\n]/.test(value)
...
function addSentencePunctuationSpacing(_match: string, punctuation: string, offset: number, input: string): string {
  ...
  if (punctuation === "." && isDottedInitialismSeparator(input, offset)) {
    return punctuation;
  }
...
function isDottedInitialismSeparator(input: string, offset: number): boolean {
  ...
  // A chain requires a neighbouring single-character group, so the token reads
  // as an abbreviation rather than a missing sentence break.
  return input[offset - 2] === "." || input[offset + 2] === ".";
}
