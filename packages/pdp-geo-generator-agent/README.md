...
flowchart TD
  ...
  M --> N["13. Final Proofreading (optional)<br/>gpt-5.6-terra 권장"]
...
| RAG 문서 로드 | `src/rag`의 계약 정본, schema.org, E-E-A-T, CEP, GEO 연구, official docs, locale 문서를 읽습니다. |
...
| Strategic Coverage 보강 | 10개 정책 가족의 정본 문서가 최종 후보에서 빠지면 해당 문서만 보충 검색하고, 가족마다 좌석 하나를 예약합니다. |
...
src/rag/
  ...
  content-field-contracts_v1.md
  schema-org-product_v2.md
  eeat_v1.md
  cep_v1.md
  best-practice_v1.md
  geo-research_v3.md
  evidence/
    geo-research-cards_v1.md
  ...
  brands/
    exampleluxe/
      ...
      best-practice_v2.md
      locale-expression-guidelines_v2.md
      locale-terminology-map_v2.json
    examplederma/
      brand-identity_v1.md
      best-practice_v2.md
      locale-expression-guidelines_v2.md
...
| 문서 | 역할 |
| --- | --- |
| `content-field-contracts_v1.md` | **계약 정본.** description 구성/분리, FAQ, HowTo, schema safety, public wording, evidence routing 계약의 유일한 권위 문안 |
| `analysis-prompt_v1.md` | 전체 생성 원칙과 RAG orchestration 기준. 계약 정본에 종속되며, 어떤 근거 역할이 어떤 필드로 흐르는지의 오케스트레이션 관점만 담당 |
| `schema-org-product_v2.md` | Product, WebPage, FAQPage, HowTo, BreadcrumbList 구조화 기준 |
| `eeat_v1.md` | 신뢰, 근거, claim safety, 리뷰 표현, 과장 방지 기준 |
| `cep_v1.md` | 고객이 상품을 찾는 상황, 루틴, 고민, 리뷰 질문을 표현하는 기준 |
| `geo-research_v3.md` | GEO 연구, answer readiness, citation readiness, AI 검색 최적화 원칙 |
| `evidence/geo-research-cards_v1.md` | 외부 연구 근거를 오프라인 증류한 evidence card (Source/Status/Checked/Claim/Do-not-use) |
...
| `brands/{brand}/best-practice_v2.md` | 기본 `best-practice_v1.md` 위에 얹는 브랜드별 GEO 생성 품질 오버레이(대체 아님, 병행 주입) |
| `brands/{brand}/locale-expression-guidelines_v2.md` | 기본 `locale-expression-guidelines_v1.md` 위에 얹는 브랜드별 언어/톤 오버레이 |
| `brands/{brand}/locale-terminology-map_v2.json` | 기본 `locale-terminology-map_v1.json` 위에 얹는 브랜드별 선호/회피 용어 오버레이 |

코퍼스는 **정본(canon) 하나 + 관점 문서 여럿** 구조입니다. 필드 계약의 권위 문안은 `content-field-contracts_v1.md`에만 있고, 나머지 문서는 같은 계약을 자기 관점(schema 호환성, E-E-A-T, CEP, GEO 연구, 브랜드, 오케스트레이션)에서 *적용*할 뿐 경쟁하는 문안으로 재진술하지 않습니다. 각 관점 문서는 자기가 기대는 계약을 산문으로 지목하고("...는 content-field-contracts 문서의 Description Composition Contract를 따른다"), 그 위에 자기 관점이 추가하는 것만 규칙으로 씁니다. 같은 원칙이 연구 수치에도 적용되어, `geo-research_v3.md`는 외부 연구의 수치·표본·출처를 직접 적지 않고 `evidence/geo-research-cards_v1.md`의 카드를 단일 출처로 지목합니다.

이 구조는 문서를 읽기 좋게 만드는 것 이상의 효과가 있습니다. 정책 컴파일러는 문서 간 규칙을 전역 dedupe하는데, 정본이 문서 목록 맨 앞에 있으므로 같은 계약이 여러 문서에 흩어져 있을 때 권위 문안이 dedupe에서 살아남습니다. 또한 주입 예산이 압박받을 때 `kind` 순위(`field-contracts` > `evidence-cards`·전략 문서 > `orchestration` > 참조 문서 > 브랜드 톤)에 따라 계약이 먼저 살아남고 톤 가이드가 먼저 축출됩니다. `evidence-cards`가 전략 문서와 같은 순위인 것은 카드가 연구 수치의 정본이고 전략 문서가 그 수치를 카드에 위임하기 때문입니다 — 인용하는 쪽만 남고 인용되는 쪽이 축출되면 근거 없는 주장이 남습니다.

브랜드 문서는 상품 브랜드와 매칭될 때만 생성 RAG 후보에 포함되며, 기본(공통) 문서를 대체하지 않고 그 위에 병행 주입되는 오버레이입니다. 예를 들어 예시더마 상품 요청에는 공통 `best-practice_v1.md`, `locale-expression-guidelines_v1.md`, `locale-terminology-map_v1.json`에 더해 `brands/examplederma/brand-identity_v1.md`, `brands/examplederma/best-practice_v2.md`, `brands/examplederma/locale-expression-guidelines_v2.md`, `brands/examplederma/locale-terminology-map_v2.json`가 함께 포함되고, 예시럭셔리 브랜드 문서는 제외됩니다. 브랜드를 식별할 수 없거나 해당 브랜드별 문서가 없으면 공통 문서만 사용되고 `brands/` 하위 문서는 제외됩니다(공통 문서는 이 경우에도 항상 로드되므로 별도 fallback 전환이 필요 없습니다). 정책 컴파일러는 문서 간 규칙을 전역 dedupe하고 소스별 주입 한도를 적용해, 공통 문서와 브랜드 오버레이가 함께 로드되어도 중복 주입을 흡수합니다.

`rag-index.ts`는 단순한 파일 목록이 아니라 RAG 문서의 source of truth입니다. 각 문서와 주요 섹션에 다음 metadata를 붙입니다.

- 문서 종류: field-contracts, orchestration, schema, eeat, cep, geo-research, evidence-cards, best-practice, official-docs, locale, terminology 등
...
코퍼스는 10개 정책 가족(`field-contracts`, `geo-research`, `evidence-cards`, `eeat`, `cep`, `schema`, `best-practice`, `locale`, `terminology`, `official-docs`)으로 나뉩니다. 일반 검색에서 어느 가족이든 밀릴 수 있으므로, agent는 1차 선택 결과에 각 가족의 **정본 문서**가 빠졌는지 확인하고 빠진 문서만 별도로 작게 검색해 보강합니다.

누락 판정은 `kind`가 아니라 **문서 단위**입니다. `kind`로 판정하면 같은 kind를 쓰는 문서가 하나라도 검색된 순간 보호가 조용히 풀립니다 — 브랜드 오버레이가 공통본을 가리거나, 연구 수치를 인용하는 `geo-research_v3.md`가 정작 수치의 정본인 evidence 카드를 가리는 식입니다.

최종 선택에서는 가족마다 좌석 하나가 예약되며, 그 좌석은 **그 가족의 정본 문서** 몫입니다. 브랜드 오버레이는 자기 브랜드의 델타만 담고 있어서 좌석을 가져가면 그 델타가 기대는 공통 규칙이 도달하지 않으므로, 오버레이는 좌석 경쟁 대신 별도 보호 슬롯을 받습니다(공통본과 오버레이 병행 주입). 선언한 field target이 `retrieval`·`diagnostics`뿐인 청크 — 무엇을 쓸지가 아니라 무엇을 가져올지를 정하는 섹션 — 는 규칙을 담은 형제 청크에서 좌석을 빼앗지 못합니다.

좌석은 `maxChunks` 예산에서 나오므로 예산이 가족 수보다 작으면 순서상 뒤쪽 가족이 조용히 탈락합니다. 기본값 14는 10개 가족을 모두 앉히고 재량 슬롯을 남기도록 정한 값입니다(§RAG 설정 예시).
...
생성 품질은 문장을 많이 만드는 것보다, 상품 근거가 올바른 필드에 들어가는지에 크게 좌우됩니다. 이 agent는 RAG 문서와 validation 단계에서 같은 field contract를 사용합니다. 계약의 권위 문안은 `src/rag/content-field-contracts_v1.md`에 있고, 아래 표는 그 계약을 필드별로 읽는 요약입니다.
...
## Commerce Identity Contract (GEO-128)

입력 JSON의 상거래 식별 필드는 구조 보존으로 JSON-LD에 매핑됩니다. 모든 값은 fail-closed입니다 — 정규화에 실패한 값은 추정 없이 생략되고, availability 누락은 `diagnostics.recommendations`(`field: "offers.availability"`)로 알립니다.

| 입력 필드 | 출력 | 정규화 규칙 |
| --- | --- | --- |
| `skuId` / `sku` / `productCode` | `Product.sku` | 명시값이 URL/본문 추론보다 우선. 공백 제거, 4-64자 |
| `gtin` / `barcode` / `ean` / `upc` | `Product.gtin` + `gtin8/12/13/14` | 8/12/13/14자리 + GS1 체크 디지트 검증 통과 값만 |
| `availability` / `stockStatus` / `soldOut`·`available`(boolean) | `Offer.availability` | schema.org `ItemAvailability` 12종 canonical URL로만 출력. ko/ja/en 문구(판매중, 품절, sold out 등) 매핑 지원 |
| `itemCondition` / `condition` | `Offer.itemCondition` | `OfferItemCondition` enum으로만 출력 |
| `variants[]` (id/sku/gtin/title/options/price/currency/availability/url/image) | variant별 `offers: Offer[]` (Tier 2) | 신뢰 가능한 price+currency를 가진 variant가 2개 이상이고 가격/재고/식별자가 실제로 구분될 때만 배열 발행. 발행 시 평탄화된 `Options`/`Variant comparison` additionalProperty는 억제 |
...
## Final Proofreading (`gpt-5.6-terra` 권장)
...
const run = await generatePdpGeo({
  ...
  rag: {
    mode: "local-versioned-rag",
    // 기본값 14. 정책 가족 10개의 예약 좌석이 이 예산에서 나오므로 10 미만으로
    // 낮추면 순서상 뒤쪽 가족(terminology, official-docs)이 컨텍스트에서 빠진다.
    maxChunks: 14,
    scoreThreshold: 0,
    queryPlanning: {
      enabled: true,
...
| `src/final-proofreader.ts` | `gpt-5.6-terra` final fluency-only 교정, strict output, 불변성 gate, fail-safe |
...
이 패키지는 두 개의 상호보완 벤치마크 티어를 가집니다.

| 티어 | 명령어 | 측정 대상 | LLM/비용 | CI |
| --- | --- | --- | --- | --- |
| RAG retrieval 벤치마크 | `pnpm rag:eval` | 생성 직전 RAG chunk 선택 품질 (claimRecall 등). 커버리지 패스를 포함한 실제 조립 경로를 그대로 실행합니다 | 없음 (결정적) | 회귀 게이트 포함 |
| 인용 가시성 벤치마크 | `pnpm geo:benchmark` (apps/geo-generator에서 실행) | 모의 생성엔진 답변에서 생성 PDP의 인용 점유율(wordpos/word/pos)과 유용성 가드레일(KPR/KPC, citation precision) | LLM 사용 (opt-in) | 지표 순수 함수만 CI |

인용 가시성 벤치마크는 AutoGEO(ICLR 2026)의 GEO/GEU 2축 평가를 이식한 것으로, 같은 고객 질문·같은 경쟁 문서에서 원본 PDP와 생성 PDP를 치환 비교(counterfactual)해 "생성 결과가 실제로 더 인용되는가"를 paired delta로 측정합니다. 인용 점유율이 크게 갈린 결과는 `pnpm geo:extract-rules`로 엔진 선호 규칙 초안을 채굴해(사람 리뷰 후) RAG 코퍼스 개선으로 되돌립니다. 평가 로직은 독립 패키지 [`packages/pdp-geo-eval-agent`](../pdp-geo-eval-agent/README.md)로 분리되어 있고, 이 패키지와의 연결(산출물 생성→주입)은 `apps/geo-generator/scripts/run-geo-benchmark.ts`가 담당합니다.
...
# 평가
pnpm rag:eval                                  # 결정적 retrieval 벤치마크 (이 패키지)
# 인용 가시성 벤치마크/규칙 추출은 apps/geo-generator에서:
#   pnpm geo:benchmark -- --provider azure-openai
#   pnpm geo:extract-rules -- --provider azure-openai --results <json>
```
