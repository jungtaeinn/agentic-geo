...
test("raw proxy preserves NDJSON and correlation semantics without forwarding untrusted headers", async () => {
  ...
  try {
    ...
    assert.equal(receivedUrl, "http://python-agent.test/base/extract?batch=two");
...
test("extractor profile proxy replaces a spoofed console selector with the private selector", async () => {
  const originalFetch = globalThis.fetch;
  const previousBaseUrl = process.env.AGENTIC_GEO_API_URL;
  let receivedHeaders = new Headers();
  ...
  try {
    await proxyPythonAgent(
      new Request("http://next-console.test/api/rag-profile", {
    ...
    assert.equal(receivedHeaders.get("x-neo-console"), "extractor");
  } finally {
    globalThis.fetch = originalFetch;
    if (previousBaseUrl === undefined) {
      delete process.env.AGENTIC_GEO_API_URL;
...
test("proxy preserves a browser cancellation signal for the upstream request", async () => {
  const originalFetch = globalThis.fetch;
  const previousBaseUrl = process.env.AGENTIC_GEO_API_URL;
  ...
  try {
    const request = new Request("http://next-console.test/api/generate", {
      method: "POST",
      body: "{}",
      signal: controller.signal
    });
    await proxyPythonAgent(request, "/generate");
