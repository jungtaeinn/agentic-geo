...
export function selectProductNormalizationRagDocuments(
  ...
  /**
   * The family slot belongs to the family's common document. A brand overlay
   * shares its family but carries only that brand's deltas, so letting it take
   * the slot on priority leaves the overlay referring to base rules that never
   * arrived — the same invariant `selectFinalRagChunks` holds on the retrieval
   * side. Overlays still reach the prompt through the same-family fill below.
   */
  const familySlotDocument = (family: string) => {
    const inFamily = ranked.filter((item) => productNormalizationRagFamily(item.document.name) === family);
    return (inFamily.find((item) => !isBrandScopedRagDocumentName(item.document.name)) ?? inFamily[0])?.document;
  };
  ...
  for (const family of ["geo", "eeat", "cep"]) {
    add(familySlotDocument(family));
  }
  // Remaining families are seated by priority, but each family's slot still
  // goes to its common document rather than whichever member ranks highest.
  for (const item of ranked) {
    add(familySlotDocument(productNormalizationRagFamily(item.document.name)));
  }
...
export function productNormalizationRagPriority(name: string): number {
  const path = name.toLocaleLowerCase().replace(/\\/g, "/");
  if (/brands\/[^/]+\/brand-identity/.test(path)) return 120;
  if (!/(?:analysis-prompt|schema-org|geo-research|eeat|cep|best-practice|official-ai-search|locale-expression|locale-terminology|content-field-contracts)/.test(path)) return 115;
  ...
  // Lowest of the recognized policy documents, deliberately. The field
  // contracts govern what generated output may contain; normalization-stage
  // evidence-role routing is analysis-prompt §3.1's job. Without this branch the
  // document falls through to the 115 catch-all meant for unrecognized custom
  // documents and outranks analysis-prompt itself.
  if (/content-field-contracts/.test(path)) return 79;
  return 70;
}

/** True for a `brands/<slug>/…` overlay, which carries only that brand's deltas. */
function isBrandScopedRagDocumentName(name: string): boolean {
  return /(?:^|\/)brands\/[^/]+\//.test(name.toLocaleLowerCase().replace(/\\/g, "/"));
}
...
/**
 * A review body must be customer language. The product name, original name, and
 * brand all live in the source corpus, so an echoed identity string passes the
 * source-backed check while carrying no review evidence at all.
 */
function isCustomerReviewBody(body: string, bootstrapProduct: PdpProductSignal): boolean {
  const compact = body.trim().toLowerCase().replace(/\s+/g, " ");
  if (compact.length < 16) {
    return false;
  }

  return ![bootstrapProduct.name, bootstrapProduct.originalName, bootstrapProduct.brand]
    .filter((value): value is string => typeof value === "string" && value.length > 0)
    .some((value) => value.trim().toLowerCase().replace(/\s+/g, " ") === compact);
}
...
function applyProductNormalization(
  ...
  const bootstrapUsage = dedupePdpUsageInstructions(filterValuesByEvidenceRole(bootstrapProduct.usage, "usage"));
  ...
  next.usage = dedupePdpUsageInstructions(filterValuesByEvidenceRole(next.usage, "usage"));
  ...
  if (incoming.reviews) {
    const items = Array.isArray(incoming.reviews.items)
      ? incoming.reviews.items.flatMap((item) => {
          ...
          return body && isCustomerReviewBody(body, bootstrapProduct) ? [{ ...item, body }] : [];
        })
      : [];
    const hasAuthoritativeKeywords = Array.isArray(incoming.reviews.keywords);
    const keywords = hasAuthoritativeKeywords
    ...
    if (items.length > 0 || keywords.length > 0 || typeof incoming.reviews.rating === "number" || typeof incoming.reviews.reviewCount === "number") {
      next.reviews = {
        ...
        // An explicit empty array is an authoritative request to clear
        // qualitative review language while retaining aggregate rating data.
        keywords: hasAuthoritativeKeywords ? keywords : bootstrapProduct.reviews.keywords
      };
