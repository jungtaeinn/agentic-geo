...
## 4. Long Page Evidence Candidates
...
- `ingredient`: formula terms such as niacinamide, peptide, retinol, hyaluronic acid, ginseng, BotanicalComplex, 인삼 펩타이드, 전성분.
- `usage`: actionable customer directions — sentences that tell the customer when, where, how, or how often to use the product. Classify by the directive function of the sentence, not by a fixed verb list; every application method a dosage form requires (spreading, spraying, rinsing, wiping, and so on) qualifies equally. An explicitly numbered sequence (사용법 1 ... 2 ...) stays one step per source instruction in source order. Before/after-use measurements, clinical timelines, and treatment/control labels such as `제품 사용`/`무도포` belong to `metric` or `effect`, not `usage`.
...
## 7. Sentence Insight Output

- Sentence insights should preserve the source sentence or a compact source-backed clause.
- If one sentence links an ingredient/technology to outcomes, classify it by the strongest downstream field and keep related keywords together, for example ingredient keywords plus firmness/elasticity effect terms.
- Do not retain scene-description phrases; keep only citation-ready product facts, metrics, ingredients, benefits, effects, usage, FAQ, or review evidence.
