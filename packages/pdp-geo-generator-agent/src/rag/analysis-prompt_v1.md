...
- A benefits FAQ question asks, in buyer language, about the product's main benefits together with whether disclosed test results exist. Preferred example: add `공개된 인체적용시험 결과는 어떻게 나타났나요?` or `what do the reported clinical study results show?` only when finished-product clinical evidence exists; otherwise ask only about the main benefits. Do not phrase the question as a request for the internal evidence record itself — that exposes the generation pipeline's sourcing apparatus rather than asking what a buyer would ask; avoid `what product evidence supports them?`. Reserve `published` or `peer-reviewed` for an actual bibliographic citation.
