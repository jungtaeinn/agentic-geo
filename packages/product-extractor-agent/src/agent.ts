...
export async function extractProductFromHtml(
  ...
  const name =
    stringValue(productNode?.name) ??
    clientStateData.name ??
    meta($, "og:title") ??
    cleanText($("h1").first().text()) ??
    cleanText($("title").first().text()) ??
    "Untitled product";
  evidence.push({ field: "product.name", source: productNode?.name ? "jsonLd" : "dom", value: name });
  ...
  const description =
    stringValue(productNode?.description) ??
    clientStateData.description ??
    meta($, "description") ??
    meta($, "og:description") ??
    firstLongText(bodyText);
  if (description) {
    evidence.push({ field: "product.description", source: productNode?.description ? "jsonLd" : "meta", value: description });
  }
...
function extractClientStateProductData($: ReturnType<typeof load>, source: string): ClientStateProductData {
  ...
  const sections = uniqueContentSections(states.flatMap(extractKeyedProductSections));
  ...
  return {
    ...
    reviews,
    images: unique(productRecords.flatMap((record) => readClientStateImages(record, source))).slice(0, 28),
    options: unique(productRecords.flatMap(readClientStateOptions)).filter(isProductOptionText).slice(0, 12),
    name: firstStringFromRecords(productRecords, ["onlineProdName", "productName", "prodName", "name", "title"], isLikelyProductName),
    description: firstStringFromRecords(productRecords, ["linePromoDesc", "description", "desc", "summary", "shortDescription"], (text) =>
      text.length >= 20 && hasProductCareSignal(text) && !isReviewEvidenceText(text)
    ),
    price: firstClientStatePrice(productRecords),
    currency: firstClientStateCurrency(productRecords)
  };
}
