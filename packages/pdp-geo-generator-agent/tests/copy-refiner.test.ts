...
function createRefinementRequest(
  ...
  const productDescription = overrides.productDescription
    ?? "배리어케어365 캡슐 토너는 건조하고 민감한 피부를 위한 장벽보습 캡슐 토너입니다.";
  const webPageDescription = overrides.webPageDescription
    ?? "배리어케어365 캡슐 토너 상품 페이지는 민감 피부 고객을 위한 장벽보습 정보를 소개합니다.";
  const faq = overrides.faq ?? [
    {
      question: "배리어케어365 크림에 함유된 캡슐과 동일한 캡슐인가요?",
      answer: "배리어케어365 캡슐 토너의 캡슐은 PHA 워터에 띄워진 고밀도 세라마이드 캡슐입니다."
    },
    {
      question: "예시더마 배리어케어365 캡슐 토너는 어떤 고객에게 추천할 수 있나요?",
      answer: "예시더마 배리어케어365 캡슐 토너는 건조하고 민감한 피부 고객에게 적합한 장벽보습 캡슐 토너입니다."
    },
    {
      question: "예시더마 배리어케어365 캡슐 토너의 주요 성분과 효능은 무엇인가요?",
      answer: "예시더마 배리어케어365 캡슐 토너는 PHA와 고밀도 세라마이드 캡슐을 담은 장벽보습 캡슐 토너입니다."
    }
  ];
  ...
  return {
    ...
    product: {
      ...
      sourceTexts: [
        ...
        "동일한 고밀도 세라마이드 캡슐이 배리어케어365 크림과 캡슐 토너에 사용된다."
      ]
    },
    schemaMarkup: {
      jsonLd: {
...
describe("copy refinement description gates", () => {
  it("rejects refined descriptions that expose analysis labels", async () => {
    ...
    const result = await refinePdpGeoCopy(request, createOptions(() => ({
      schemaDescriptions: {
        product: "예시더마 배리어케어365 캡슐 토너는 민감 피부용 캡슐 토너입니다. 사용 직후 시점 기준 평가 지표: 사용 직후는 93% 회복되었습니다."
      }
    })));
  ...
  it("rejects refined descriptions that enumerate raw volume strings", async () => {
    ...
    const result = await refinePdpGeoCopy(request, createOptions(() => ({
      schemaDescriptions: {
        webPage: "배리어케어365 캡슐 토너 상품 페이지는 장벽 보습 맥락과 촉촉한 사용감 중심의 리뷰 맥락, 10.14 fl. oz. / 300 mL 용량을 함께 살펴볼 수 있습니다."
      }
    })));
...
describe("corrective refinement pass", () => {
  it("rewrites editorial caveat narration as a direct customer-facing FAQ ending", async () => {
    const question = "예시더마 배리어케어365 캡슐 토너의 쿨링 결과에는 개인 차가 있나요?";
    const directAnswer = "예시더마 배리어케어365 캡슐 토너는 사용 직후 시원한 쿨링감을 줄 수 있으며, 해당 쿨링 결과에는 개인 차가 있을 수 있습니다.";
    const request = createRefinementRequest({
      ...
      return {
        faqAnswers: [{
          ...
          answer: calls.length === 1
            ? "예시더마 배리어케어365 캡슐 토너는 사용 직후 시원한 쿨링감을 줄 수 있으며, 해당 쿨링 결과에는 개인 차가 있을 수 있다는 단서가 붙습니다."
            : directAnswer
        }]
      };
    }));
  ...
  it("retries Korean Product descriptions that retain OCR, predicate, test-note, or passive-review artifacts", async () => {
    ...
    const badDescription = "예시더마 배리어케어365 캡슐 토너는 민감 피부용 토너입니다. 주요 성분은 PHA이며, 하는 기술이 적용되어 있고 ☑ 장벽 지표 93% ※ 시험 결과입니다. 완료된 테스트는 참고할 수 있는 시험 정보입니다. 실제 고객 리뷰에서는 촉촉한 사용감이 언급됩니다.";
    const cleanDescription = "예시더마 배리어케어365 캡슐 토너는 건조하고 민감한 피부 고객을 위한 장벽보습 캡슐 토너로, 고밀도 세라마이드 캡슐이 장벽 보습을 돕고 세정에 의한 장벽 손상은 사용 직후 93% 회복되었습니다.";
    const calls: PdpGeoCopyRefinementRequest[] = [];
  ...
  it("triggers the corrective pass when unrefined fallback copy keeps analysis labels", async () => {
    const request = createRefinementRequest({
      productDescription: "예시더마 배리어케어365 캡슐 토너는 민감 피부용 캡슐 토너입니다. 사용 직후 시점 기준 평가 지표: 사용 직후는 93% 회복되었습니다."
    });
    const cleanDescription = "예시더마 배리어케어365 캡슐 토너는 건조하고 민감한 피부 고객을 위한 장벽보습 캡슐 토너로, 세정에 의한 장벽 손상은 사용 직후 93% 회복되었습니다.";
    const calls: PdpGeoCopyRefinementRequest[] = [];
  ...
  it("falls back with a warning when the corrective pass also fails", async () => {
    const badDescription = "예시더마 배리어케어365 캡슐 토너는 민감 피부용 캡슐 토너입니다. 사용 직후 시점 기준 평가 지표: 사용 직후는 93% 회복되었습니다.";
    const request = createRefinementRequest();
    let callCount = 0;
  ...
  it("does not reorder FAQ items when the corrective pass returns only a partial faqAnswers list", async () => {
    ...
    const result = await refinePdpGeoCopy(request, createOptions((incoming) => {
      ...
      if (calls.length === 1) {
        return {
          ...
          faqAnswers: [
            {
              sourceQuestion: "예시더마 배리어케어365 캡슐 토너는 어떤 고객에게 추천할 수 있나요?",
              question: "민감하고 건조한 피부에는 어떤 토너를 추천하나요?",
              answer: "예시더마 배리어케어365 캡슐 토너는 건조하고 민감한 피부 고객에게 추천할 수 있는 장벽보습 캡슐 토너로, 고밀도 세라마이드 캡슐이 장벽 보습을 돕습니다."
            },
            {
              sourceQuestion: "예시더마 배리어케어365 캡슐 토너의 주요 성분과 효능은 무엇인가요?",
              question: "예시더마 배리어케어365 캡슐 토너의 주요 성분과 효능은 무엇인가요?",
              answer: "예시더마 배리어케어365 캡슐 토너는 PHA와 고밀도 세라마이드 캡슐을 담은 장벽보습 캡슐 토너로, 장벽 보습과 피부결 정돈을 돕습니다."
            },
            {
              sourceQuestion: "배리어케어365 크림에 함유된 캡슐과 동일한 캡슐인가요?",
              question: "배리어케어365 크림에 함유된 캡슐과 동일한 캡슐인가요?",
              answer: "네, 동일한 고밀도 세라마이드 캡슐입니다. 배리어케어365 크림과 캡슐 토너에 같은 고밀도 세라마이드 캡슐이 사용됩니다."
            }
          ]
        };
      }
      return {
        ...
        faqAnswers: [
          {
            sourceQuestion: "예시더마 배리어케어365 캡슐 토너의 주요 성분과 효능은 무엇인가요?",
            question: "예시더마 배리어케어365 캡슐 토너에는 어떤 성분이 들어 있나요?",
            answer: "예시더마 배리어케어365 캡슐 토너에는 PHA와 고밀도 세라마이드 캡슐이 담겨 있으며, 장벽 보습과 피부결 정돈에 도움을 줍니다."
          }
        ]
      };
    }));
    ...
    expect(mainEntity[0]!.name).toBe("예시더마 배리어케어365 캡슐 토너는 어떤 고객에게 추천할 수 있나요?");
    expect(mainEntity[1]!.name).toBe("예시더마 배리어케어365 캡슐 토너에는 어떤 성분이 들어 있나요?");
    expect(mainEntity[1]!.acceptedAnswer.text).toBe(
      "예시더마 배리어케어365 캡슐 토너에는 PHA와 고밀도 세라마이드 캡슐이 담겨 있으며, 장벽 보습과 피부결 정돈에 도움을 줍니다."
    );
    expect(mainEntity[2]!.name).toBe("배리어케어365 크림에 함유된 캡슐과 동일한 캡슐인가요?");
  });
...
describe("FAQ generative-intent recomposition", () => {
  it("reorders FAQ, rewrites questions, and applies yes-leading comparison answers", async () => {
    ...
    expect(result.content.sections.faq.indexOf("예시더마 배리어케어365 캡슐 토너는 어떤 고객에게")).toBeLessThan(
    ...
    const result = await refinePdpGeoCopy(request, createOptions(() => ({
      faqAnswers: [
        ...
        },
        {
          sourceQuestion: "예시더마 배리어케어365 캡슐 토너는 어떤 고객에게 추천할 수 있나요?",
          question: "민감하고 건조한 피부에는 어떤 토너를 추천하나요?",
          answer: "예시더마 배리어케어365 캡슐 토너는 건조하고 민감한 피부 고객에게 추천할 수 있는 장벽보습 캡슐 토너입니다."
        }
      ]
    })));
...
describe("approved copy contract preservation", () => {
  it("retains the approved Product.description when refinement drops an existing core role", async () => {
    const baseDescription = [
      "예시더마 배리어케어365 캡슐 토너는 장벽보습 캡슐 토너입니다.",
  ...
  it("accepts a page-scoped WebPage refinement that connects supported buyer-decision facts", async () => {
    ...
    const webPageDescription = "EXAMPLEDERMA의 예시더마 배리어케어365 캡슐 토너 상품 페이지는 상품 정보와 브랜드 정보, 사용법 및 FAQ를 제공합니다.";
    const request = createRefinementRequest({ productDescription, webPageDescription });
    const productClone = [
      "EXAMPLEDERMA의 예시더마 배리어케어365 캡슐 토너 상품 페이지는 건조하고 민감한 피부 고객을 위한 장벽보습 토너를 소개합니다.",
  ...
  it("retains a product-specific FAQ answer when refinement becomes generic", async () => {
    const request = createRefinementRequest();
    const sourceQuestion = "예시더마 배리어케어365 캡슐 토너는 어떤 고객에게 추천할 수 있나요?";
    ...
    expect(targetFaq?.acceptedAnswer.text).toContain("예시더마 배리어케어365 캡슐 토너");
