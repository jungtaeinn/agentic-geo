...
The field contracts this document works from are stated once, in their authoritative wording, in the content-field-contracts document. This document does not restate them; each section below names the contract it builds on. What it supplies is the tone, architecture, and product-page judgement that turn those contracts into finished copy.
...
Public JSON-LD values and PDP content should read like customer-facing product information, not internal optimization notes. The Public Wording Contract in the content-field-contracts document decides which wording is barred and from which fields; the guardrails below decide what the sentence should sound like instead.
...
The Description Separation Contract in the content-field-contracts document fixes which entity each description describes and why schema.org forces that split. GEO adds a further constraint on top of it: each description should be easy for a generative engine to cite, verify, and connect to the correct entity without collapsing page context and product facts into the same sentence. The two subsections below cover what that means for each field in practice.
...
- Interrupting the buyer narrative with standalone certification, test-method, disclosure, or report-style sentences. One natural measured-outcome sentence may stay when it directly answers the concern; `Reported details` and evidence FAQ take the rest.
...
Compose the field under the Description Composition Contract in the content-field-contracts document, which supplies the evidence arc and the stage-skipping rule. It is an arc, not a surface template: CEP determines transitions and sentence boundaries, and missing evidence shortens it.

Narrative connection principles (apply in every target locale):

- Prefer subordinate clauses, appositions, and cause-neutral connectors over sentence-per-field enumeration. Merge adjacent stages into one sentence when the grammar stays natural and the claim scope of each clause is unchanged.
- Keep each proof element at the stage whose scope it actually matches. Preserved modality, caveats, and study qualifiers are what carry trust, and moving a proof element earlier or later silently widens the claim it makes.
...
The `Product` node should be dense but verifiable. How the two descriptions differ, and which report-style material belongs in dedicated properties instead, is settled by the Description Separation Contract in the content-field-contracts document.
...
- A customer situation such as "skin feels dry and tight" belongs in `value` under a stable name like `Review-derived recommendation context`; a full question normally belongs in `FAQPage.mainEntity`, or under `Indirect customer question` or `Direct product question` when it is retained as product context.
...
Apply the Evidence Routing Contract in the content-field-contracts document to a compressed OCR string before anything else: it decides when the string is not one atomic metric and how far a shared study context may reach. The intent taxonomy below is what this document adds on top of that.
...
Avoid a Product description that only says the product is a "hydration serum": naming the type and stopping there skips every later stage of the composition contract, which is where a buyer finds a reason to choose this product over the next one. Never fabricate a citation to fill a stage the source left empty.
...
How many items may appear, which intents qualify, and when a question must be dropped instead of answered are settled by the FAQ Contract in the content-field-contracts document. What follows is the question repertoire and answer craft this document adds.
...
Whether a `HowTo` may exist at all — a concrete goal plus at least one direct source usage action — and how those source instructions map onto `HowTo.step` entries, their count, and their order, is settled by the HowTo Contract in the content-field-contracts document. This section covers only what a good usage step looks like once the contract has admitted it.
...
- Benefit and effect evidence goes to `Product.description`, `additionalProperty`, FAQ answers, and visible benefit sections.
