...
describe("adversarial evidence-bound content planning", () => {
  it("does not treat a repeated cross-language corrective pass as semantic proof", async () => {
    ...
    const unsupportedPlan = planPayload({ locale: "ko-KR",
  ...
  it("retains audited Korean product FAQs despite natural inflection and separated numeric question context", async () => {
    ...
    const ids = (...roles: string[]) => request.evidenceLedger
      ...
      .map((item) => item.id);
    const candidate = planPayload({ locale: "ko-KR",
      faq: [
        {
          include: true,
  ...
  it("does not let semantic step labels add order to unmarked source usage notes", async () => {
    ...
    // The semantic "Step 1/Step 2" labels must not create an ordered procedure
    // the source never marked, so both actions publish as one unordered step.
    expect(result.plan.howTo.ordered).toBe(false);
    expect(result.plan.howTo.steps).toHaveLength(1);
  });

  it("keeps one direct product usage instruction as one step despite an unrelated source sequence", async () => {
    const source = product({
    ...
    expect(result.plan.howTo.steps.map((step) => step.text)).toEqual(["Apply one pump evenly to the face."]);
  });
  ...
  it("does not publish a raw wrong-locale description when the plan omits it", async () => {
    ...
    const run = await generatePdpGeo({
      ...
      customContentPlanner: {
        planContent: () => ({ plan: planPayload({ locale: "ko-KR" }) })
      }
    });
