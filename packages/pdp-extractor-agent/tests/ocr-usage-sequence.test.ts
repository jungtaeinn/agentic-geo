...
// Real merged OCR blob shape from https://example.com/web/product/view.do?prdSeq=1027
// (editor detail image, 7 slices joined): package label noise + "사용법 1 ... 2 ..." sequence.
const mergedUsageBlob = "크림 Step 5 배리어케어365 크림 미스트 EXAMPLEDERMA BARRIERCARE365 CREAM MIST Ceramide 10,000 ppm Moisturizing & strengthening skin's moisture barrier For dry & weakened skin 4.05 fl.oz / 120 mL 사용법 1 연약하고 건조해진 피부 부위에 미세 분사를 합니다. 2 피부에 건조함이 느껴질 때 수시로 뿌려줍니다. EXAMPLEDERMA BARRIERCARE365 CREAM MIST Ceramide 10,000 ppm Moisturizing & strengthening skin's moisture barrier For dry & weakened skin 4.05 fl.oz / 120 mL 철저히 검증한 피부 안전성 테스트";

const html = `
<!doctype html>
<html>
  <head>
    <title>배리어케어365 크림 미스트</title>
    <meta name="description" content="10,000ppm 함유된 고함량 세라마이드 미세분사로 피부장벽을 보호하는" />
  </head>
  <body>
    <main>
      <h1>배리어케어365 크림 미스트</h1>
...
describe("explicit numbered usage sequence rescue from merged OCR blocks", () => {
  it("extracts spray-type usage steps with their explicit source numbering preserved", async () => {
    const { result } = await extractProductFromHtml(html, "https://example.com/web/product/view.do?prdSeq=1027");
