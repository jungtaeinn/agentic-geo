...
export function ensurePdpGeoFaqPlanCoverage(input: {
  ...
  const planned = input.plan.faq
    .filter((item) => item.include)
    .map((item) => normalizePlannedAssessmentFaqAnswer(item, input.product, input.locale));
  ...
  const merged = [
    ...
    .filter((item, index, items) => {
      const semanticKeys = createFaqSemanticDedupeKeys(item, input.locale);
      if (semanticKeys.length === 0) return true;
      return !items.slice(0, index).some((candidate) =>
        hasFaqSemanticDedupeConflict(
          semanticKeys,
...
function normalizePlannedAssessmentFaqAnswer(
  item: PdpGeoPlannedFaqItem,
  product: PdpProductSignal,
  locale: PdpGeoLocale
): PdpGeoPlannedFaqItem {
  const measurementIntent = /(?:clinical|metric|assessment|evidence|measurement|study)/iu.test(item.intent)
    || createFaqSemanticDedupeKeys(item, locale).includes("evidence-measurement");
  if (!measurementIntent) return item;
  const formatted = selectStructuredReportedMetricDetails(product, locale, 3);
  return formatted.length > 0
    ? { ...item, answer: compactRepeatedStudyContextText(formatted.join(" ")) }
    : item;
}
...
export function generatePdpGeoArtifacts(input: GenerateArtifactsInput): GenerateArtifactsOutput {
  ...
  const plannedHowToSteps = input.contentPlan?.howTo.eligible
    ? normalizePlannedHowToSteps(input.contentPlan.howTo.steps.map((step) => step.text), input.locale)
    : [];
  ...
  const inferredProductDescription = normalizeInferencePublicText(modelPlanned
    ? plannedDescriptionOrSource(
      ...
      "product",
      () => applyAvoidTerms(
        createGeoDescription(input.product, productName, input.locale, localizedTerms, guidance, optimizedUsageSteps),
        terminologyConcepts,
        input.locale,
        terminology
      )
  ...
  const sections: PdpGeoContentSections = {
    ...
    howToUse: normalizeInferencePublicText(applyAvoidTerms(
      createHowToUseSection(input.product, input.locale, publicUsageSteps, Boolean(input.contentPlan)),
      terminologyConcepts,
      input.locale,
      terminology
    ), input.locale),
  ...
  const schemaMarkup = createSchemaMarkup({
    ...
    webPageDescription,
    faq,
    usageInstructions: publicUsageSteps,
...
function plannedDescriptionOrSource(
  ...
  const brandPrefix = product.brand && !productName.toLocaleLowerCase().includes(product.brand.toLocaleLowerCase())
    ? product.brand
    : "";
  if (locale === "ko-KR") {
    return kind === "product"
      ? `${productName} 제품입니다.`
      : `${brandPrefix ? `${brandPrefix}의 ` : ""}${productName} 상품 페이지입니다.`;
  }
  if (locale === "ja-JP") {
    return kind === "product"
      ? `${productName}の商品です。`
      : `${brandPrefix ? `${brandPrefix}の` : ""}${productName}の商品ページです。`;
  }
  return kind === "product"
    ? `${productName} is a product.`
    : `This ${productName} product page${brandPrefix ? ` from ${brandPrefix}` : ""} presents the supplied product information.`;
}
...
function isDescriptionLocaleCompatible(value: string, product: PdpProductSignal, locale: PdpGeoLocale): boolean {
  ...
  if (locale === "ko-KR") return isKoreanNarrativeLocaleCompatible(sample);
  if (locale === "ja-JP") return kana >= 2;
  return latin >= 3 && hangul === 0 && kana === 0;
}

function isSafeEvidenceBackedProductDescription(value: string, product: PdpProductSignal, locale: PdpGeoLocale): boolean {
  return Boolean(value)
    && isDescriptionLocaleCompatible(value, product, locale)
...
/**
 * Brand identity taken from the page's own address.
 *
 * A PDP served from the brand's own site already carries one official,
 * dereferenceable identity URL, and the caller has no reason to send what the
 * request already states. Nothing is invented here: the value is the origin of
 * the canonical product URL the caller supplied.
 *
 * The host has to earn it by carrying the brand name, because the same PDP
 * shape arrives from marketplaces. Without that check a retailer's domain would
 * be published as the brand's identity — a false claim, and the exact kind of
 * unverified value the rest of this pipeline refuses to emit. A brand whose
 * name shares no ASCII token with any host (a Korean-only brand name, say)
 * yields nothing rather than a guess; explicit `hints.brandSameAs` covers it.
 */
function deriveBrandSameAsFromSourceUrl(brand?: string, sourceUrl?: string): string[] | undefined {
  if (!brand || !sourceUrl) {
    return undefined;
  }
  const brandToken = brand.toLowerCase().replace(/[^a-z0-9]/g, "");
  // Two characters match too many hosts by accident to count as evidence.
  ...
  try {
    ...
    const hostToken = parsed.hostname.toLowerCase().replace(/[^a-z0-9]/g, "");
    return hostToken.includes(brandToken) ? [`${parsed.protocol}//${parsed.host}/`] : undefined;
  } catch {
    return undefined;
  }
}

function dedupeValidBrandSameAsUrls(urls?: string[]): string[] | undefined {
  if (!urls || urls.length === 0) {
    return undefined;
  }
  const valid = Array.from(new Set(urls
    .map((url) => url.trim())
    .filter((url) => {
      try {
...
/**
 * Keeps a finished-product benefit in Product.description even when the
 * source has no ingredient, study, or metric detail. Existing ingredient
 * copy may already state the same product-level benefit; in that case this
 * fallback stays silent to avoid a duplicated claim.
 */
function createStandaloneProductBenefitDescription(
  locale: PdpGeoLocale,
  context: DescriptionContext,
  precedingParts: Array<string | undefined>
): string | undefined {
  const benefitPhrase = cleanSignal(context.benefitPhrase ?? "");
  if (!benefitPhrase || precedingParts.some((part) => part && context.benefits.some((benefit) => descriptionPartContainsBenefit(part, benefit)))) {
    return undefined;
  }

  return fallback(locale, {
    "ko-KR": `이 제품의 주요 효능·효과는 ${benefitPhrase}입니다`,
    "ja-JP": `本品の主なベネフィットは${benefitPhrase}です`,
    "en-US": `The product's documented benefit is ${benefitPhrase}`,
    "en-GB": `The product's documented benefit is ${benefitPhrase}`
  });
}

function descriptionPartContainsBenefit(part: string, benefit: string): boolean {
  const partKey = signalEntityKey(part);
  const benefitKey = signalEntityKey(benefit);
  return Boolean(partKey && benefitKey)
    && (descriptionFactMatches(part, benefit)
      || semanticFacetsOverlap(benefitSemanticFacets(part), benefitSemanticFacets(benefit))
      || benefitKey.length >= 2 && partKey.includes(benefitKey));
}
...
function createWebPageDescription(
  ...
  localizedTerms: string[],
  optimizedUsageSteps: string[] = [],
  sourcePlanIsAuthoritative = false
): string {
...
function createKoreanWebPageCoverageScopeDescription(product: PdpProductSignal, productName: string, context: DescriptionContext): string | undefined {
  ...
  return coverage ? `이 페이지는 ${appendKoreanObjectParticle(coverage)} 함께 다룹니다` : undefined;
}

/**
 * Describes concrete, independently supported facts available on the page.
 ...
 * rendered as "100%%" in public copy. Mirrors the evidence-ledger atom
 * serializer in content-planner.ts; every metric narrative must use this
 * instead of concatenating value and unit directly.
 */
function joinMetricValueWithUnit(value: string | undefined, unit: string | undefined): string {
  const cleanedValue = cleanSignal(value ?? "");
  const cleanedUnit = cleanSignal(unit ?? "");
  if (!cleanedValue) {
    return cleanedUnit;
  }
...
function createKoreanWebPageMetricSentences(product: PdpProductSignal, preferredEvidence?: string): string[] {
  ...
  const outcomes = (product.semanticFacts?.metricClaims ?? [])
    ...
    .filter((claim) => {
      const source = cleanSignal(claim.sentence ?? claim.sourceText ?? "");
      const direction = cleanSignal(claim.direction ?? "");
      const value = joinMetricValueWithUnit(claim.value, claim.unit);
      return Boolean(direction && value && source.includes(direction) && source.replace(/\s+/g, "").includes(value.replace(/\s+/g, "")));
    })
...
function formatKoreanWebPageMetricOutcome(claim: PdpSemanticMetricClaim): string | undefined {
  ...
  const labelWithTiming = (timing && !label.includes(timing)
    ...
    ? `${timing} ${label}`
    : label).replace(/^한번만/u, "한 번만");
  const baseline = cleanSignal(claim.baseline ?? claim.comparator ?? "");
  const baselinePhrase = baseline ? `${baseline}${/(?:대비|비교)/u.test(baseline) ? "" : " 대비"} ` : "";
  if (/지속/u.test(direction)) {
    const durationLabel = labelWithTiming.replace(/\s*지속$/u, " 효과");
    return `${appendKoreanSubjectParticle(durationLabel)} ${value} 동안 지속되는 것으로 확인되었습니다`;
  }
...
function createKoreanWebPageSafetyOfferSentence(product: PdpProductSignal, productName: string): string | undefined {
  const safetyTests = unique((product.semanticFacts?.safetyTests ?? [])
    ...
    .filter(Boolean))
    .slice(0, 3);
  const safetyPhrase = formatKoreanNaturalList(safetyTests);
  const options = formatDescriptionList(product.options.map(formatKoreanWebPageOptionLabel).filter(Boolean), "ko-KR", 2);
...
function createJapaneseWebPageCoverageLead(product: PdpProductSignal, productName: string, context: DescriptionContext): string {
  const pageEntity = product.brand && !productName.toLocaleLowerCase().includes(product.brand.toLocaleLowerCase())
    ? `${product.brand}の${productName}`
    : productName;
  return `${pageEntity}の商品ページでは、${context.productType}の商品概要を紹介します`;
}
...
function createJapaneseWebPageCoverageScopeDescription(product: PdpProductSignal, context: DescriptionContext): string | undefined {
  ...
  return coverage ? `このページでは${coverage}も扱います` : undefined;
}
...
function createEnglishWebPageCoverageLead(product: PdpProductSignal, productName: string, context: DescriptionContext): string {
  const brand = product.brand && !productName.toLocaleLowerCase().includes(product.brand.toLocaleLowerCase())
    ? ` from ${product.brand}`
    : "";
  return `This ${productName} product page${brand} introduces the ${lowercaseEnglishProductType(context.productType)}`;
}
...
function createEnglishWebPageCoverageScopeDescription(product: PdpProductSignal, context: DescriptionContext): string | undefined {
  ...
  return coverage ? `The page covers ${coverage}` : undefined;
}
...
function createEnglishWebPageEvidenceCoverageDescription(context: DescriptionContext): string | undefined {
  ...
  const factSentence = ingredientPhrase && context.benefitPhrase
    ? `It lists ${stripPatentApplicationQualifier(ingredientPhrase)} as highlighted formula components and separately documents ${context.benefitPhrase} as product benefits`
    : ingredientPhrase
      ? `It lists ${stripPatentApplicationQualifier(ingredientPhrase)} as highlighted formula components`
      : context.benefitPhrase
        ? `It documents ${context.benefitPhrase} as product benefits`
        : undefined;
...
function createEnglishWebPageDecisionSupportDescription(product: PdpProductSignal, context: DescriptionContext): string | undefined {
  ...
  const metricSentence = (product.semanticFacts?.metricClaims ?? [])
    .filter(hasSemanticClinicalClaimContext)
    .filter((claim) => !isIngredientPerformanceOnlyMetricClaim(claim))
    .map((claim) => cleanSignal(formatSemanticMetricClaim(claim, "en-US") ?? ""))
    .find((value) => value && isNarrativeLocaleCompatible(value, "en-US"));
  const safetyTests = unique((product.semanticFacts?.safetyTests ?? [])
    .map((value) => cleanSignal(value).replace(/\b(?:completed|tested)\b/giu, "test").replace(/\s+/g, " ").trim())
    .filter((value) => isNarrativeLocaleCompatible(value, "en-US")))
    .slice(0, 3);
  const safetyPhrase = formatDescriptionList(safetyTests, "en-US", 3);
  const optionPhrase = formatDescriptionList(product.options, "en-US", 2);
  const price = typeof product.price?.amount === "number" && Number.isFinite(product.price.amount)
    ? new Intl.NumberFormat("en-US", { style: "currency", currency: product.price.currency ?? "USD" }).format(product.price.amount)
    : cleanSignal(product.price?.raw ?? "") || undefined;
  return compactSentence([
    usageSentence,
...
function selectWebPageIngredientTerms(values: string[], locale: PdpGeoLocale): string[] {
  const atoms = values.flatMap((value) => cleanSignal(value)
    .split(/\s*(?:(?<!\d)[,，]|[,，](?!\d))\s*/u)
    .map(cleanSignal)
    .filter(Boolean));
  const deduped = dedupeIngredientSignals(dedupePublicListValues(atoms));
  const hasDenseCeramideCapsule = deduped.some((value) => /고밀도\s*세라마이드\s*캡슐|high[-\s]?density\s+ceramide\s+capsules?/iu.test(value));
  return deduped
    .filter((value) => !hasDenseCeramideCapsule
      || /고밀도\s*세라마이드\s*캡슐|high[-\s]?density\s+ceramide\s+capsules?/iu.test(value)
      || !/(?:세라마이드|ceramide)[^,，]{0,30}(?:캡슐|capsule)|(?:캡슐|capsule)[^,，]{0,30}(?:세라마이드|ceramide)/iu.test(value))
    .filter((value) => isNarrativeLocaleCompatible(value, locale)
      || /^[A-Za-z0-9™®+()\-\s]+$/u.test(value)
      || locale === "ko-KR" && /(?:[A-Za-z][^,，]{0,40}(?:기술|포뮬러)|(?:기술|포뮬러)[^,，]{0,40}[A-Za-z])/u.test(value))
    .slice(0, 3);
}
...
function createDescriptionContext(
  ...
  return {
    ...
    usage: sourcePlanIsAuthoritative
...
function isNarrativeLocaleCompatible(value: string, locale: PdpGeoLocale): boolean {
  ...
  if (locale === "ko-KR") {
    return isKoreanNarrativeLocaleCompatible(text);
  }
  if (locale === "ja-JP") {
...
/**
 * Korean public narrative may contain brand names and INCI tokens, but a
 * complete English clause is not Korean copy merely because a short Korean
 * token appears beside it.
 */
function isKoreanNarrativeLocaleCompatible(value: string): boolean {
  const text = cleanSignal(value);
  if ((text.match(/[가-힣]/gu) ?? []).length < 3 || /[ぁ-んァ-ン]/u.test(text)) {
    return false;
  }

  return text.split(/(?<=[.!?。！？])\s+|\n+/u).every((segment) => {
    const latinWords = segment.match(/\b[A-Za-z][A-Za-z'’-]*\b/g) ?? [];
    const hangulCount = (segment.match(/[가-힣]/gu) ?? []).length;
    if (latinWords.length >= 4 && hangulCount < 3) {
      return false;
    }
    const hasEnglishClause = /(?:^|\s)(?:i|we|you|they|this|that|it|the\s+(?:product|formula|serum|cream)|customers?)\s+(?:am|is|are|was|were|has|have|had|help(?:s|ed)?|support(?:s|ed)?|contain(?:s|ed)?|include(?:s|d)?|provide(?:s|d)?|improve(?:s|d)?|target(?:s|ed)?|use(?:s|d)?|feel(?:s|t)?|love(?:s|d)?|recommend(?:s|ed)?)\b|^(?:is|are|was|were|helps?|supports?|contains?|includes?|provides?|improves?)\b/iu.test(segment);
    return !(latinWords.length >= 3 && hasEnglishClause);
  });
}
...
function createEnglishIngredientTechnologyProductSentence(
  ...
  const detailRepeatsSupportedRelation = Boolean(detailSentence && relationSupported && benefitPhrase
    && descriptionFactMatches(detailSentence, ingredient)
    && semanticFacetsOverlap(benefitSemanticFacets(detailSentence), benefitSemanticFacets(benefitPhrase)));
  const detailClause = detailSentence && !detailRepeatsSupportedRelation
    ? `. ${capitalizeFirst(trimTrailingSentencePunctuation(detailSentence))}`
    : "";
...
function createProductEvidenceDescription(locale: PdpGeoLocale, evidence: string): string {
  ...
  if (assessment) {
    ...
    switch (locale) {
      ...
      case "en-US":
      default: {
        // The source-stated outcome keeps the measured value and its reporting
        // modality. Regex topic categories are a last resort because they drop
        // the value and can assert an outcome the study never measured.
        const reportedOutcome = cleanSignal(assessment[2] ?? "");
        if (reportedOutcome) {
          return `In ${indefiniteArticle(context)} ${context}, ${lowercaseFirst(reportedOutcome)}`;
        }
...
function normalizeEvidenceClaimPhrase(value: string): string {
  ...
  const hasOcrNoise = /[A-Z]{4,}.*[A-Z]{4,}|(?:serum|cream|exampleluxe|sérum|activateur|premiers soins)/i.test(cleanValue);
...
function createLocalizedReviewDescription(locale: PdpGeoLocale, context: DescriptionContext): string | undefined {
  ...
  if (context.representativeReviewPhrase) {
    switch (locale) {
      case "ja-JP":
        return `代表的なレビューでは${context.representativeReviewPhrase}のように語られ${context.reviewPhrase ? `、${context.reviewPhrase}などの関連表現も確認できます` : ""}`;
      case "en-GB":
      case "en-US":
      default:
        return context.reviewBodies.length === 1
          ? `One customer review ${formatEnglishRepresentativeReviewClause(context.representativeReviewPhrase)}`
          : `Customer reviews ${formatEnglishRepresentativeReviewClause(context.representativeReviewPhrase)}`;
    }
  }
...
/**
 * @internal Exported for the population-labeling regression test: the gender
 * capture is optional, so this is where an unstated demographic could be
 * invented, and the surrounding suitability-evidence path is only reached when
 * the structured clinical summary declines the same evidence.
 */
export function extractTargetAudienceAgeSample(value: string, locale: PdpGeoLocale): string | undefined {
  const text = cleanSignal(value);
...
/**
 * Labels the study population named in a source sample string. The gender
 * capture that feeds this is optional, so a source that states only an age
 * range and a headcount ("만 20~59세 30명") arrives here as `undefined`.
 * Defaulting that to 여성 published a demographic the source never stated —
 * the neutral label is the only one the evidence supports, and it matches what
 * {@link normalizeEnglishAudienceLabel} already returns for the same gap.
 */
...
function inferRecommendedSkinType(product: PdpProductSignal, locale: PdpGeoLocale): string | undefined {
  ...
  const semanticSkinType = formatRecommendedSkinTypeList(product.semanticFacts?.skinTypes ?? [], locale, 5);
  if (semanticSkinType) {
...
function selectExplicitSuitabilitySkinTypes(product: PdpProductSignal, locale: PdpGeoLocale): string | undefined {
  ...
  for (const candidate of candidates) {
    ...
    if (!/(?:suitable\s+for|recommended\s+for|ideal\s+for|designed\s+for|formulated\s+for|developed\s+for|speciali[sz]ed\s+for|for\s+(?:all|most)\s+skin\s+types|\bfor\s+(?:(?:sensitive|dry|oily|combination|normal|mature)(?:-feeling)?\s+(?:and|or)\s+)?(?:sensitive|dry|oily|combination|normal|mature)(?:-feeling)?\s+skin\b|적합|추천|위한|특화|맞춤)/i.test(positiveClause)) {
      continue;
    }
    ...
    const types = unique([
      ...Array.from(positiveClause.matchAll(/\b(sensitive|dry|oily|combination|normal|mature)(?:-feeling)?\s+(?:and|or)\s+(sensitive|dry|oily|combination|normal|mature)(?:-feeling)?\s+skin\b/gi))
        .flatMap((match) => [`${match[1]} skin`, `${match[2]} skin`]),
      ...Array.from(positiveClause.matchAll(/\b(sensitive|dry|oily|combination|normal|mature)(?:-feeling)?\s+skin\b/gi)).map((match) => `${match[1]} skin`),
...
function formatRecommendedSkinTypeList(values: string[], locale: PdpGeoLocale, limit: number): string | undefined {
  ...
  const englishSkinTypes = localized.map((value) => value.replace(/\s+skin$/i, ""));
  return localized.every((value) => /\s+skin$/i.test(value))
    ? englishSkinTypes.length === 2
      ? `${englishSkinTypes[0]} or ${englishSkinTypes[1]} skin`
      : `${englishSkinTypes.slice(0, -1).join(", ")}, or ${englishSkinTypes.at(-1)} skin`
    : localized.length === 2
...
/**
 * An ingredient role is not a finished-product efficacy claim. When the only
 * structured support for a candidate outcome is an ingredient-benefit link,
 * keep it available to the ingredient explanation but do not promote it into
 * Product benefits or a suitability answer about the finished product.
 */
function isIngredientLinkOnlyBenefitSignal(value: string, product: PdpProductSignal): boolean {
  const linkOutcomes = (product.semanticFacts?.ingredientBenefitLinks ?? [])
    .flatMap((link) => [link.benefit, link.effect])
    .filter((candidate): candidate is string => Boolean(cleanSignal(candidate ?? "")));
  if (!linkOutcomes.some((outcome) => descriptionFactMatches(value, outcome)
    || semanticFacetsOverlap(benefitSemanticFacets(value), benefitSemanticFacets(outcome)))) {
    return false;
  }

  const finishedProductClaims = [
    ...product.benefits,
    ...product.effects,
    ...(product.semanticFacts?.benefits ?? []),
    ...(product.semanticFacts?.effects ?? [])
  ];
  return !finishedProductClaims.some((claim) => descriptionFactMatches(value, claim)
    || semanticFacetsOverlap(benefitSemanticFacets(value), benefitSemanticFacets(claim)));
}
...
function selectBenefitEvidenceSourceTexts(product: PdpProductSignal): string[] {
  ...
  ].map((item) => signalEntityKey(item)).filter(Boolean));
  return product.sourceTexts.slice(0, 60)
    .map(cleanSignal)
    .filter((value) => value.length >= 3 && value.length <= 320)
    .filter((value) => !reviewKeys.has(signalEntityKey(value)))
    .filter((value) => !usageKeys.has(signalEntityKey(value)))
    .filter((value) => !isUsageInstruction(value))
...
function selectLocalizedKeyIngredients(product: PdpProductSignal, locale: PdpGeoLocale, limit: number): string[] {
  const sourceCorpusLines = [product.description ?? "", ...product.sourceTexts];
  const localized = dedupePublicListValues(selectKeyIngredients(product, Math.max(limit * 2, limit))
    ...
    .filter((value): value is string => Boolean(value))
    .filter((value) => isCredibleIngredientSurface(value, sourceCorpusLines)));
  const selected: string[] = [];
...
const nonIngredientSurfaceLeadPattern = /^(?:the|a|an|this|that|these|those|it|its|what|which|how|why|when|where|who|and|or|etc)\b/i;

/**
 * Rejects ingredient surfaces that are extraction artifacts rather than
 * ingredient names: article/interrogative-led sentence shards, single broken
 * tokens, and candidates that only occur in the product's own source as a
 * truncated prefix of a longer word run.
 */
function isCredibleIngredientSurface(value: string, sourceCorpusLines: string[]): boolean {
  const text = cleanSignal(value);
  if (!text) {
    return false;
  }
  if (nonIngredientSurfaceLeadPattern.test(text) || nonIngredientTokenPattern.test(text)) {
    return false;
  }
  return !isTruncatedSourceFragment(text, sourceCorpusLines);
}

/**
 * True when the candidate occurs in the source texts only as a prefix of a
 * longer word run — the signature of a clipped extraction ("SCUTELLARIA
 * BAICALENS" ⊂ "SCUTELLARIA BAICALENSIS ROOT EXTRACT"). Two occurrence kinds
 * do NOT count as standalone evidence: an occurrence at the very end of a
 * source line (that is exactly where clipping happens) and a line that merely
 * echoes the candidate itself (normalized field values are folded back into
 * sourceTexts, so the broken candidate is its own echo).
 */
function isTruncatedSourceFragment(candidate: string, sourceCorpusLines: string[]): boolean {
  // Mid-word clipping only exists inside space-delimited Latin words. Hangul,
  // Kana, and Han candidates are excluded: agglutinative particles attach
  // letters directly to the noun ("유도체가"), which is normal morphology,
  // not a longer word the candidate was clipped from.
  if (candidate.length < 6 || sourceCorpusLines.length === 0 || !/[A-Za-z]$/.test(candidate)) {
    return false;
  }
  const escaped = candidate.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const extendedPattern = new RegExp(`${escaped}[A-Za-z]`, "i");
  const standalonePattern = new RegExp(`${escaped}[^A-Za-z]`, "i");
  const identity = cleanSignal(candidate).toLocaleLowerCase();
  const evidenceLines = sourceCorpusLines.filter((line) => cleanSignal(line).toLocaleLowerCase() !== identity);
  if (!evidenceLines.some((line) => extendedPattern.test(line))) {
    return false;
  }
  return !evidenceLines.some((line) => standalonePattern.test(line));
}
 ...
 * entity key.
 */
function ingredientAliasFamilyKey(value: string): string {
  const cleaned = value
    .toLocaleLowerCase()
    .replace(/[™®©]/gu, "")
    .replace(/\([^)]*\)/gu, " ");
  const tokens = (cleaned.match(/[\p{L}\p{N}%]+/gu) ?? [])
    .filter((token) => !/^(?:extracts?|추출물|エキス)$/u.test(token));
  const numeric = tokens.filter((token) => /\p{N}/u.test(token));
  const head = tokens[tokens.length - 1];
  if (numeric.length === 0 || !head || /\p{N}/u.test(head)) {
    return "";
  }
  return [...numeric, head].join("|");
}
...
function selectAdjacentUsageInstructionSourceTexts(sourceTexts: string[]): string[] {
  ...
  sourceTexts.forEach((text, index) => {
    ...
    for (let offset = 1; offset <= 4; offset += 1) {
      ...
      // A stitched page/OCR dump after the usage heading means we left the
      // clean instruction list — stop instead of promoting the dump.
      if (!next || hasOfficialUsageInstructionSectionCue(next) || isLikelyNonUsageSectionBoundary(next) || isRawPageTextBlock(next)) {
        break;
      }
...
function selectPublicReviewKeywords(product: PdpProductSignal, locale: PdpGeoLocale): string[] {
  return dedupePublicListValues(selectReviewKeywords(product)
    ...
    .map((value) => value ? restoreSpecificReviewKeywordSurface(value, product, locale) : undefined)
    .filter((value): value is string => Boolean(value))
    .filter(isUsefulPublicListValue))
    .slice(0, 8);
}

function restoreSpecificReviewKeywordSurface(
  value: string,
  product: PdpProductSignal,
  locale: PdpGeoLocale
): string {
  if ((locale !== "en-US" && locale !== "en-GB") || !/^(?:absorbs?|absorption)$/i.test(value)) {
    return value;
  }
  ...
  return /\b(?:quick\s+absorption|absorbs?\s+quickly)\b/i.test(reviewEvidence)
    ? "quick absorption"
    : value;
}
...
function hasPublicReviewEvidence(product: PdpProductSignal, locale: PdpGeoLocale): boolean {
  return selectReviewItems(product, locale).length > 0
    || selectPublicReviewKeywords(product, locale).length > 0;
}
...
function selectGroundedExpressionPhrases(product: PdpProductSignal, locale: PdpGeoLocale, limit: number): string[] {
  ...
  const supportedLinks = (product.semanticFacts?.ingredientBenefitLinks ?? [])
    .flatMap((link) => {
      const linkedIngredient = cleanSignal(link.ingredient ?? "");
      const linkedBenefit = localizePublicBenefitSignal(cleanSignal(link.benefit ?? link.effect ?? ""), locale);
      const provenance = cleanSignal(link.sentence ?? link.sourceText ?? "");
      return linkedIngredient && linkedBenefit && provenance
        ? [{ ingredient: linkedIngredient, benefit: linkedBenefit }]
        : [];
    })
    .slice(0, 2);
  ...
  if (locale === "ko-KR") {
    return unique([
      ...supportedLinks.map((link) => `${link.ingredient} ${link.benefit} 포인트`),
      ingredientPhrase ? `${formatKoreanListForSentence(ingredientPhrase)} 함유` : undefined,
  ...
  if (locale === "ja-JP") {
    return unique([
      ...supportedLinks.map((link) => `${link.ingredient}と${link.benefit}`),
      ingredientPhrase ? `${ingredientPhrase}配合` : undefined,
  ...
  return unique([
    ...supportedLinks.map((link) => `${link.ingredient} for ${link.benefit}`),
    ingredientPhrase ? `formula ingredients: ${ingredientPhrase}` : undefined,
...
function createKoreanReviewUseFeelSentence(productType: string, reviewPhrase: string, outcomePhrase: string): string {
  void outcomePhrase;
  return `고객 리뷰의 ${reviewPhrase} 표현은 ${productType}의 사용감을 판단할 때 참고할 수 있습니다`;
}
...
function createEnglishReviewUseFeelSentence(productType: string, reviewPhrase: string, outcomePhrase: string): string {
  void outcomePhrase;
  return `Customer reviews mention ${reviewPhrase} when describing the comfort and finish of the ${lowercaseEnglishProductType(productType)}`;
}
...
/**
 * Results from one study each carry the full population clause, so joining them
 * restarts "In a home usage test survey of 600 women..." per metric. The shared
 * context is kept on the first result only and dropped from later sentences.
 */
function compactRepeatedStudyContextText(value: string): string {
  const seenContexts = new Set<string>();
  return value.replace(/(^|(?<=[.!?]\s))In an? ([^,]{4,120}),\s*/giu, (match, lead: string, context: string) => {
    const key = signalEntityKey(context);
    if (!seenContexts.has(key)) {
      seenContexts.add(key);
      return match;
    }
    return lead;
  }).replace(/([.!?]\s+)(\p{Ll})/gu, (_match, boundary: string, letter: string) => `${boundary}${letter.toLocaleUpperCase()}`);
}
...
function formatKoreanStructuredMetricTimeline(claim: PdpSemanticMetricClaim): string | undefined {
  ...
  const timeline = timings
    .map((timing, index) => `${timing} ${joinMetricValueWithUnit(values[index], unit)}`)
    .join(", ");
...
function formatEnglishStructuredMetricTimeline(claim: PdpSemanticMetricClaim, locale: "en-US" | "en-GB"): string | undefined {
  ...
  const timings = cleanSignal(claim.timing ?? "")
    ...
    .filter(Boolean);
  if (values.length === 1 && timings.length <= 1) {
    return formatEnglishSingleStructuredMetricNarrative(claim, locale, values[0] ?? "", timings[0]);
  }
  if (values.length < 2 || timings.length !== values.length) {
    return undefined;
  }
  ...
  const metricLabel = direction && !metric.toLocaleLowerCase().includes(direction.toLocaleLowerCase())
    ...
    : metric;
  const unit = cleanSignal(claim.unit ?? "");
  const timeline = formatEnglishList(timings.map((timing, index) => `${joinMetricValueWithUnit(values[index], unit)} ${timing}`));
  if (!timeline) {
    return undefined;
  }
...
/** Renders one structured result as buyer-facing prose instead of key/value metadata. */
function formatEnglishSingleStructuredMetricNarrative(
  claim: PdpSemanticMetricClaim,
  locale: "en-US" | "en-GB",
  rawValue: string,
  formattedTiming?: string
): string | undefined {
  // Source values frequently already carry the unit ("100%" + unit "%") —
  // appending unconditionally rendered "100%%" in public copy.
  const value = joinMetricValueWithUnit(rawValue, claim.unit);
  const label = cleanSignal(claim.label ?? claim.subject ?? claim.metric ?? "");
  if (!value || !label) return undefined;

  const rawMethod = cleanSignal(claim.method ?? "");
  const method = normalizeEnglishStudyMethod(rawMethod);
  const sample = cleanSignal(claim.sample ?? "");
  const dailyUse = /(?:with|using|used)\s+(?:the\s+product\s+)?daily|daily\s+use/iu.test(`${rawMethod} ${claim.sentence ?? ""} ${claim.sourceText ?? ""}`);
  const timing = formattedTiming || formatEnglishMetricTiming(cleanSignal(claim.timing ?? ""));
  // Labels frequently carry their own direction stem, with or without a
  // modality modifier ("Visible improvement in fine lines", "Reduction in
  // wrinkle depth"). The stem must be parsed out — templating a direction verb
  // around the raw label nests the stem ("showed improvement in Visible
  // improvement in fine lines").
  const outcomeStemMatch = label.match(/^(?:(visible|visibly|noticeable|noticeably|significant|significantly|clear|clearly)\s+)?(improvement(?:\s+rate)?|reduction|decrease|increase)\s+(?:in|of)\s+(.+)$/iu);
  const stemKind = outcomeStemMatch
    ? /^improvement/iu.test(outcomeStemMatch[2] ?? "") ? "improvement"
    : /^(?:reduction|decrease)/iu.test(outcomeStemMatch[2] ?? "") ? "reduction"
    : "increase"
    : undefined;
  const modality = outcomeStemMatch?.[1]
    ? outcomeStemMatch[1].toLocaleLowerCase().replace(/ibly$/u, "ible").replace(/ably$/u, "able").replace(/ly$/u, "")
    : undefined;
  const modalityPrefix = modality ? `${modality} ` : "";
  // The improvement subject is what improved — the label (or its direction-stem
  // tail). claim.subject is the study population ("32 women") and must never
  // fill this slot; the old fallback rendered "improvement in 32 women".
  const subject = cleanSignal(outcomeStemMatch?.[3] ?? label);
  const improvementSubject = subject
    ? formatEnglishList(subject.split(/\s*,\s*|\s+and\s+/iu).map(cleanSignal).filter(Boolean))
    : undefined;
  const outcomeSubject = improvementSubject ?? subject;
  const direction = cleanSignal(claim.direction ?? "").toLocaleLowerCase();
  // A percentage paired with a study sample is a share-of-participants claim.
  // Rendering it as a change magnitude ("fine lines decreased by 100%") would
  // distort claim strength, so participant framing takes precedence.
  const participantShare = Boolean(sample) && /%$/.test(value);

  const outcome = improvementSubject && (stemKind === "improvement" || (!stemKind && /improv/iu.test(direction)))
    ? `${value}${sample ? " of participants" : ""} showed ${modalityPrefix}improvement in ${improvementSubject}`
    : stemKind === "reduction" || /(?:decreas|reduc)/iu.test(direction)
      ? participantShare
        ? `${value} of participants saw ${indefiniteArticle(`${modalityPrefix}reduction`)} ${modalityPrefix}reduction in ${outcomeSubject}`
        : `${outcomeSubject} decreased by ${value}`
      : stemKind === "increase" || /(?:increas|improv)/iu.test(direction)
        ? participantShare
          ? `${value} of participants saw ${indefiniteArticle(`${modalityPrefix}increase`)} ${modalityPrefix}increase in ${outcomeSubject}`
          : `${outcomeSubject} improved by ${value}`
        : `${label} was reported at ${value}`;
  // A timing clause spliced mid-sentence must not keep its sentence-start
  // capital ("...fine lines After one bottle..."). Acronym-led clauses
  // ("UV exposure...") are left untouched.
  const inlineTiming = timing && /^[A-Z][a-z]/u.test(timing) ? lowercaseFirst(timing) : timing;
  const timedOutcome = inlineTiming && !outcome.toLocaleLowerCase().includes(inlineTiming.toLocaleLowerCase())
    ? `${outcome} ${inlineTiming}`
    : outcome;

  if (method) {
    const article = /^[aeiou]/iu.test(method) ? "an" : "a";
    const population = sample ? ` of ${sample}${dailyUse ? " who used the product daily" : ""}` : "";
    return ensurePublicSentence(`In ${article} ${method}${population}, ${timedOutcome}`, locale);
  }
  if (sample) {
    return ensurePublicSentence(`Among ${sample}${dailyUse ? " using the product daily" : ""}, ${timedOutcome}`, locale);
  }
  return ensurePublicSentence(capitalizeFirst(timedOutcome), locale);
}
...
function normalizeEnglishStudyMethod(value: string): string | undefined {
  const method = cleanSignal(value)
    ...
    .replace(/instrumental\s+result/giu, "instrumental assessment")
    .replace(/(?:,?\s*)with\s+daily\s+use\b/giu, "")
    .replace(/^(?:an?|the)\s+/iu, "")
    .trim();
...
/**
 * Removes source-attribution parentheticals that reference the page itself
 * ("(PDP clinical infographic)", "(상세 페이지 인포그래픽)"). They are internal
 * provenance notes rather than public product copy, and a page-reference
 * token inside Product.description trips the entity-role contract.
 */
function stripPageReferenceAnnotation(value: string): string {
  return value
    .replace(/\s*\((?:[^()]*\b(?:PDP|product[-\s]detail\s+page|product\s+page|detail\s+page)\b[^()]*|[^()]*(?:상세\s*페이지|상품\s*페이지|제품\s*페이지)[^()]*)\)/giu, "")
    .replace(/\s+/g, " ")
    .trim();
}

function normalizeReportedDetail(value: string): string | undefined {
  const text = stripPageReferenceAnnotation(stripSourceSectionLabel(value))
    .replace(/\*/g, "")
    .replace(/([+\-−]?\d+)\.\s+(\d+%)/g, "$1.$2")
...
function isConciseStandaloneUsageStep(value: string): boolean {
  ...
  return (
    ...
    && /\b(?:water|face|skin|hands?|palms?|neck|product|amount|absorbed|absorption)\b/i.test(text)
  ) || (
...
function allProductEvidenceText(product: PdpProductSignal): string {
  return allProductEvidenceValues(product).join(" ");
}

/**
 * Per-value evidence view for selectors that classify text function. Joining
 * values first lets a comma-split fragment span two unrelated source values
 * (e.g. page prose + vendor + price), producing chimera "facts" no source
 * ever stated; value boundaries are the entailment boundary.
 */
function allProductEvidenceValues(product: PdpProductSignal): string[] {
  return [
    product.name,
    product.originalName,
    product.description,
  ...
  ].filter((value): value is string => Boolean(value)).map(cleanSignal);
}
...
function isLowQualityPublicEvidenceText(value: string): boolean {
  ...
  if (containsSerializedMetadata(text)) {
    return true;
  }
...
function publicListDedupeKey(value: string): string {
  ...
  if (/(?:피부\s*)?장벽/u.test(key) || /\b(?:skin\s+)?barrier\b/.test(key)) {
    return "skin barrier";
  }
...
function createQuickFacts(
  ...
  const reportedMetricFact = guidance.useEvidenceBackedClaims
    ? createStructuredClinicalEvidenceSummary(product, locale)
      ?? first(selectStructuredReportedMetricDetails(product, locale, 1))
      ?? createEvidenceMetricFact(publicReportedDetail, locale)
    : undefined;
...
function createOptimizedUsageSteps(
  product: PdpProductSignal,
  locale: PdpGeoLocale,
  _guidance: GeoOptimizationGuidance
): string[] {
  ...
  // Preserve the cardinality of source usage items. Splitting is allowed only
  // when a single source value explicitly contains multiple numbered steps;
  // generic RAG guidance must not manufacture extra actions.
  const shouldSplitUsage = usage.some(hasExplicitMultiStepUsageMarkers);
...
/**
 * Keeps only the step text. A planned step label is never carried into public
 * markup, because the schema renderer states each step's position instead of
 * republishing a label that was not verified against the step's own action.
 */
function normalizePlannedHowToSteps(steps: string[], locale: PdpGeoLocale): string[] {
  const results: string[] = [];
  for (const step of steps) {
    const text = rewriteUsageStep(step, locale);
    const key = usageStepKey(text);
    if (!key
      || !isNarrativeLocaleCompatible(text, locale)
      || !(isUsageInstruction(text) || isComposedUsageInstruction(text))
      || results.some((item) => usageStepsAreSemanticallyEquivalent(item, text))) {
      continue;
    }
    results.push(text);
  }
...
/**
 * A step that merges usage directions the source never ordered exceeds the
 * single-instruction length budget. It stays a valid step when every sentence
 * it merges is itself a usage instruction.
 */
function isComposedUsageInstruction(value: string): boolean {
  const sentences = value
    .split(/(?<=[.!?。！？])\s+/u)
    .map((sentence) => sentence.trim())
    .filter(Boolean);
  return sentences.length >= 2 && sentences.every(isUsageInstruction);
}

function hasExplicitMultiStepUsageMarkers(value: string): boolean {
  const markers = Array.from(cleanSignal(value).matchAll(/(?:^|\s)(?:step\s*)?(\d+)\s*(?:단계|段階)?[.):、]/giu))
    .map((match) => Number(match[1]));
  return markers.includes(1) && markers.includes(2);
}
...
function createHowToUseSection(
  ...
  const sourceUsage = sourcePlanIsAuthoritative
    ? optimizedUsageSteps
    : optimizedUsageSteps.length > 0
      ? optimizedUsageSteps
      : selectUsageInstructions(product);
  const usage = uniqueUsageSteps(sourceUsage)
    .filter((value) => isNarrativeLocaleCompatible(value, locale));
  return formatUsageStepLines(usage);
}

/**
 * A numbered "1. / 2." list communicates a real source-ordered sequence.
 * A single usage direction has no order to show, so it renders as a plain
 * sentence instead of an always-"1."-prefixed one-item list.
 */
function formatUsageStepLines(usage: string[]): string {
  const [only] = usage;
  if (usage.length === 0) return "";
  if (usage.length === 1) return only ?? "";
  return usage.map((value, index) => `${index + 1}. ${value}`).join("\n");
}
...
function ensureFaq(
  ...
  const canonicalQuestionKeys = new Set<string>();
  const pushCanonicalFaq = (item: PdpGeoFaqItem): void => {
    faq.push(item);
  ...
  // The composition+benefit anchor below and the benefit-only candidate
  // further down both answer "what does this product do"; track the anchor
  // so the two can be merged instead of publishing two near-duplicate FAQ
  // entries whenever a product has both an ingredient and a benefit signal.
  let compositionBenefitCandidate: PdpGeoFaqItem | undefined;
  if (ingredient && compositionBenefit) {
    compositionBenefitCandidate = {
    ...
    pushCanonicalFaq(compositionBenefitCandidate);
  } else if (ingredient) {
    pushCanonicalFaq({
      question: fallback(locale, {
  ...
  if (benefit) {
    ...
    const hasExplicitEnglishClinicalMethod = (product.semanticFacts?.metricClaims ?? [])
      .some((claim) => /\bclinical(?:\s+study|\s+test)?\b/iu.test(claim.method ?? "")
    ...
    // A benefit question that also asks about published clinical/assessment
    // results covers a distinct evidence-seeking intent the composition
    // question does not, so it stays a separate FAQ entry. The plain
    // "what are the benefits" phrasing has no such distinct angle and
    // duplicates the composition+benefit anchor.
    if (compositionBenefitCandidate && !hasReportedFinishedProductClinicalEvidence) {
      // Same topic, two questions: keep the single composition+benefit
      // anchor rather than adding a second "what are the benefits" entry,
      // but do not silently discard whichever candidate actually explains
      // more -- carry over the more complete answer regardless of which
      // branch built it.
      if (preferMoreDetailedFaqAnswer(benefitCandidate.answer, compositionBenefitCandidate.answer)) {
        compositionBenefitCandidate.answer = benefitCandidate.answer;
      }
    } else {
  ...
  if ((guidance.useReviewIntentFaq || reviewSignals) && reviewSignals) {
    pushCanonicalFaq({
      question: fallback(locale, {
        "ko-KR": `고객 리뷰에서는 ${productName}의 어떤 사용감이 주로 언급되나요?`,
  ...
  const primaryCandidates = [
    ...
    ...faq.map((item) => ({
      item,
      sourceBacked: false,
      preferred: canonicalQuestionKeys.has(normalizeFaqQuestionKey(item.question))
    }))
  ];
...
/**
 * Chooses between two answers covering the same merged FAQ topic. Neither
 * candidate is product- or wording-specific: this only compares how much
 * each answer actually says, so a shorter answer is replaced by a longer one
 * regardless of which branch happened to build it first.
 */
function preferMoreDetailedFaqAnswer(candidate: string, current: string): boolean {
  return cleanSignal(candidate).length > cleanSignal(current).length;
}
...
function rankFaqCandidatesForCitation(
  ...
  const normalized = candidates
    .map((candidate, index) => {
	      ...
	      if (!item
	        || !candidate.preferred && isLowQualityFaqItem(item)
...
function faqCoveragePriority(item: PdpGeoFaqItem): number {
  ...
  const locale: PdpGeoLocale = /[가-힣]/u.test(question) ? "ko-KR" : /[ぁ-んァ-ヶ一-龠]/u.test(question) ? "ja-JP" : "en-US";
  if (isSuitabilityOverviewFaqQuestion(question, locale)) return 600;
  if (/(?:주요\s*효능[·・]?효과[^?？]{0,80}(?:상품\s*)?근거|main\s+benefits?[^?!.]{0,100}(?:product\s+)?evidence|主な効果[^?？]{0,80}商品根拠)/iu.test(question)) return 500;
  if (/(?:주요\s*(?:성분|기술)[^?？]{0,80}(?:역할|명시)|ingredients?\s+or\s+technolog(?:y|ies)[^?!.]{0,100}roles?\s+are\s+explicitly\s+stated|主な成分・技術[^?？]{0,80}役割)/iu.test(question)) return 400;
  if (/^(?:.+은\s*)?어떻게\s*사용하나요[?？]?$|^How\s+should\s+.+\s+be\s+used[?？]?$|どのように使/u.test(question)) return 350;
  if (isExplicitReviewFaqCandidateQuestion(question)) return 300;
  if (/(?:어떤\s*루틴에서|fit\s+into\s+a\s+skincare\s+routine|どのようなルーティン)/iu.test(question)) return 290;
  ...
  if (/(?:사용|루틴|apply|use|routine|使い方|使用)/iu.test(question)) return 200;
  if (/(?:옵션|용량|사이즈|가격|본품|소용량|리필|버전|option|size|price|full[-\s]?size|mini|refill|variant|容量|価格|本品|ミニ|リフィル)/iu.test(question)) return 80;
  return 100;
}
...
function createFaqSemanticDedupeKeys(item: PdpGeoFaqItem, locale: PdpGeoLocale): string[] {
  ...
  if (/(?:임상|시험|평가|측정)[^?？.!]{0,50}(?:결과|수치)|(?:reported|clinical|instrumental)[^?!.]{0,50}(?:assessment|study|results?)|(?:assessment|study)\s+results?|試験[^?？。]{0,40}結果|評価[^?？。]{0,40}結果/iu.test(question)) {
    keys.push("evidence-measurement");
  }
  if (/(?:어떻게\s*사용|사용\s*방법|루틴|how\s+should[^?!.]{0,80}\b(?:use|used)|fit\s+into[^?!.]{0,80}\broutine|how\s+to\s+use|使い方|ルーティン)/iu.test(question)) {
    keys.push("usage-routine");
  }
  if (/실리콘[^?？]{0,100}민감\s*피부[^?？]{0,100}함량|민감\s*피부[^?？]{0,100}실리콘[^?？]{0,100}함량/u.test(question)) {
    keys.push("silicone-sensitive-skin-amount");
  }
  // Must stay last: every differentiating angle above emits its own key, so
  // an empty `keys` is exactly the statement "this question carries no
  // ingredient, audience, evidence, usage, or review angle of its own". A
  // question that only asks what the product does is then a plain benefit
  // overview, and any broader question that already states the finished-
  // product benefits — a composition-and-benefits question — answers it in
  // full, so the two must not both be published.
  if (asksPlainBenefitOverview(question) && (keys.length === 0 || keys.includes("ingredient-benefit-overview"))) {
    keys.push("plain-benefit-overview");
  }
  return keys;
}

/**
 * True when a question asks what the product does in general terms. It only
 * identifies a *plain* overview together with the caller's guard that no
 * other angle key was produced for the same question.
 */
function asksPlainBenefitOverview(question: string): boolean {
  return /(?:효능|효과|장점|benefits?|effects?|効果|メリット)/iu.test(question)
    && /(?:무엇|뭔가요|어떤가요|어떻게\s*되나요|\bwhat\b|何|どんな)/iu.test(question);
}
...
function isLowQualityFaqItem(item: PdpGeoFaqItem): boolean {
  ...
  const qualityText = combined.replace(/다만\s*특정\s*성분이\s*완제품의\s*효능[·・]?효과를\s*단독으로\s*만들었다고\s*단정할\s*근거는\s*제공되지\s*않았습니다[.!?。！？]?/gu, " ");
  return isLowQualityPublicEvidenceText(qualityText)
...
function isRawSourceFaqPairUsable(item: PdpGeoFaqItem): boolean {
  const question = cleanSignal(item.question);
  const answer = cleanSignal(item.answer);
  if (!question || !answer || hasTruncationMarker(question) || hasTruncationMarker(answer) || /\b1\s+(?:weeks|days|hours)\b/i.test(answer) || /["']\s*$/.test(answer)) {
    return false;
  }
...
function normalizeFaqAnswerForDirectVoice(value: string, locale: PdpGeoLocale, productName: string): string {
  ...
  if (locale === "ko-KR") {
    text = text
      ...
      .replace(/제공한다고\s*(?:설명|안내)합니다/gu, "제공하는 것으로 소개됩니다")
      .replace(/도와준다고\s*(?:설명|안내)합니다/gu, "도움을 주는 것으로 소개됩니다")
      .replace(/돕는다고\s*(?:설명|안내)합니다/gu, "돕는 것으로 소개됩니다")
      .replace(/강화해\s*준다고\s*(?:설명|안내)합니다/gu, "강화에 도움을 주는 것으로 소개됩니다")
      ...
      .replace(/제공해\s*준다고\s*확인할\s*수\s*있습니다/gu, "제공하는 것으로 소개됩니다")
      .replace(/도와\s*준다고\s*확인할\s*수\s*있습니다/gu, "도움을 주는 것으로 소개됩니다")
...
/**
 * Reverse-query concerns must come from fields already classified as audience,
 * skin type, benefit, or effect evidence. Raw ingredient and source-text
 * co-occurrence is deliberately excluded because it does not establish who a
 * finished product is suitable for.
 */
function inferKoreanFaqCustomerConcernFromEvidenceRoles(product: PdpProductSignal): string | undefined {
  const recommendedSkinType = inferRecommendedSkinType(product, "ko-KR");
  if (recommendedSkinType) {
    const hasSensitive = /민감|예민/u.test(recommendedSkinType);
    const hasDry = /건조|건성/u.test(recommendedSkinType);
    if (hasSensitive && hasDry) return "민감하고 건조한 피부";
    if (hasSensitive) return "민감한 피부";
    if (hasDry) return "건조한 피부";
  }

  const benefitRoleEvidence = unique([
    ...product.benefits,
    ...product.effects,
    ...(product.semanticFacts?.benefits ?? []),
    ...(product.semanticFacts?.effects ?? [])
  ].map(cleanSignal).filter(Boolean));
  return benefitRoleEvidence.length > 0
    ? inferKoreanFaqCustomerConcern(benefitRoleEvidence.join(". "))
    : undefined;
}
...
function isUsefulSourceFaqQuestion(value: string, answer: string, locale: PdpGeoLocale, productName: string): boolean {
  ...
  // PDP accordions can mix product FAQ with category education. A definition
  // whose answer would stay the same for any product is not a buyer FAQ.
  const genericEducationalQuestion = locale === "ko-KR"
    ? /(?:(?:피부\s*)?장벽|각질층)(?:의\s*(?:기능|역할|의미|정의)(?:은|이|가)?\s*무엇|이란\s*무엇|은\s*무슨\s*역할)/u.test(text)
    : locale === "ja-JP"
      ? /(?:肌の?バリア|角質層)(?:とは(?:何|どのようなもの)|の(?:機能|役割|意味)は(?:何|どのようなもの))/u.test(text)
      : /\b(?:what\s+is|what\s+does|what\s+role\s+does)\b[^?!.]{0,40}\b(?:skin\s+barrier|stratum\s+corneum)\b[^?!.]{0,40}\b(?:mean|do|function|role)?\b/iu.test(text);
  if (genericEducationalQuestion && !containsEntityToken(text, productName)) {
    return false;
  }
...
function isUsefulSourceFaqAnswer(value: string, _locale: PdpGeoLocale): boolean {
  const text = cleanSignal(value);
  if (text.length < 12 || text.length > 520 || hasTruncationMarker(text) || isQuestionLikeText(text) || isBrokenMarketingFragment(text) || isNonCitationEvidenceArtifact(text) || isLowQualityPublicEvidenceText(text)) {
    return false;
  }
...
function createTextureFaqAnswer(
  ...
  if (locale === "ko-KR") {
    const texturePhrase = formatKoreanNaturalList(textureFinish
      .split(/\s*,\s*/u)
      .map(cleanSignal)
      .filter(Boolean)
      .map((signal) => /(?:사용감|흡수감|마무리|제형|텍스처)$/u.test(signal) ? signal : `${signal} 사용감`)) ?? textureFinish;
    return compactSentence([
      `${appendKoreanTopicParticle(productName)} ${appendKoreanSubjectParticle(texturePhrase)} 특징입니다`
    ]);
  }
  const textureSentence = `${productName} is described as having ${indefiniteArticle(textureFinish)} ${textureFinish}`;
...
function selectTextureFinishSignal(product: PdpProductSignal, locale: PdpGeoLocale): string | undefined {
  ...
  const enSignals = [
    ...
    /non[-\s]?sticky|without\s+(?:feeling\s+)?sticky/i.test(combined) ? "non-sticky finish" : undefined,
    /smooth(?:ing)?\s+(?:texture|finish|feel)/i.test(combined) ? "smooth finish" : undefined,
    /absorbs?\s+(?:quickly|well)|quick\s+absorption/i.test(combined) ? "quick absorption" : undefined,
...
function selectRoutineSynergySignal(product: PdpProductSignal, locale: PdpGeoLocale): string | undefined {
  ...
  const hasOrderedRoutineContext = /\b(?:serum|ampoule|essence|toner|cream|skincare)\b.{0,40}\b(?:after|before|routine|layer|step)\b|\b(?:after|before|routine|layer|step)\b.{0,40}\b(?:serum|ampoule|essence|toner)\b/i.test(text);
  const hasExplicitPairingContext = /\b(?:use|used|using|pair|paired|layer|layered|combine|combined)\b.{0,24}\bwith\b.{0,40}\b(?:serum|ampoule|essence|toner|cream|skincare)\b/i.test(text);
  if (hasOrderedRoutineContext || hasExplicitPairingContext) {
...
function createBenefitFaqAnswer(
  ...
  if (locale === "en-US" || locale === "en-GB") {
    ...
    const supportedLink = selectFaqIngredientBenefitLink(product, locale);
    return compactSentence([
      `${productName} is ${englishProductTypeWithArticle(productType)}${targetContext}, with benefits focused on ${benefitPhrase}`,
...
function createIngredientFaqAnswer(
  ...
  supportedLink: { ingredient: string; benefit: string } | undefined,
  ocrContext?: string
): string {
  if (locale === "ko-KR") {
  ...
  return compactSentence([
    fallback(locale, {
      ...
      "en-US": benefit
        ...
        : `${productName} highlights ${ingredient} as ${englishFormulaElement} for product comparison and routine selection`,
      "en-GB": benefit
        ? `${productName} highlights ${ingredient} as formula elements. Product information identifies ${benefit} as a key benefit`
        : `${productName} highlights ${ingredient} as ${englishFormulaElement} for product comparison and routine selection`
    }),
    supportedLink
      ? locale === "ja-JP"
        ? `${supportedLink.ingredient}は${supportedLink.benefit}を支えます`
...
function createReviewIntentFaqAnswer(
  ...
  if (locale === "ko-KR") {
    ...
    return compactSentence([
      `고객 리뷰에서는 ${appendKoreanSubjectParticle(formattedReviewSignals)} 언급되며, ${productName}의 사용감을 판단할 때 참고할 수 있습니다`,
...
function normalizeEnglishSupportObject(value: string): string {
  // Benefit phrases often arrive verb-first ("supports the look of ...");
  // callers prepend their own "supports", so strip a leading support verb to
  // avoid "supports supports ...".
  return cleanSignal(value)
    .replace(/^(?:helps?\s+(?:to\s+)?)?supports?\s+/iu, "")
    .replace(/\bskin barrier support\b/gi, "skin barrier health");
}
...
function createKoreanReverseQuerySuitabilityQuestion(product: PdpProductSignal, productName: string): string {
  ...
  const hasDrySkin = /(?:건조|건성)/u.test(evidenceText);
  if (!hasDeepDryness && hasSensitiveSkin && hasDrySkin) {
    return `건조하고 민감한 피부 고객에게 ${appendKoreanTopicParticle(productName)} 적합한가요?`;
  }
  const concerns = unique([
    hasDeepDryness ? "속건조" : undefined,
    hasBarrierConcern ? "피부 장벽 관리" : undefined,
    hasSensitiveSkin ? "민감한 피부" : undefined,
    hasDrySkin ? "건조한 피부" : undefined,
    /(?:탄력|리프팅)/u.test(evidenceText) ? "탄력 저하" : undefined,
    /(?:주름|잔주름)/u.test(evidenceText) ? "주름" : undefined
  ].filter((value): value is string => Boolean(value))).slice(0, 2);
  const concernPhrase = formatKoreanNaturalList(concerns);
  if (concernPhrase) {
    return `${appendKoreanSubjectParticle(concernPhrase)} 고민인 고객에게 ${appendKoreanTopicParticle(productName)} 적합한가요?`;
  }
  const targetCustomer = inferTargetCustomer(product, "ko-KR");
  return isSpecificTargetCustomer(targetCustomer, "ko-KR")
    ? `${appendKoreanTopicParticle(productName)} ${targetCustomer}에게 적합한가요?`
...
function selectFaqIngredientBenefitLinks(
  product: PdpProductSignal,
  locale: PdpGeoLocale,
  limit: number
): Array<{ ingredient: string; benefit: string }> {
  const results: Array<{ ingredient: string; benefit: string }> = [];
  for (const link of product.semanticFacts?.ingredientBenefitLinks ?? []) {
    ...
    if (!results.some((item) => descriptionFactMatches(item.ingredient, ingredient)
      && descriptionFactMatches(item.benefit, benefit))) {
      results.push({ ingredient, benefit });
    }
...
function localizedSuitabilityBenefitContext(
  ...
  return ingredient
    ? `${ingredient} supports ${normalizeEnglishSupportObject(benefitPhrase)} for shoppers comparing that concern`
...
function createEnglishEvidenceReport(cleanEvidence: string): string {
  ...
  if (assessment?.[1] && assessment[2] && assessment[3]) {
    // Keep the source-stated outcome so the measured value and its reporting
    // modality survive. Topic categories are only a last resort.
    const topics = formatDescriptionList(extractEvidenceTopics(cleanEvidence), "en-US", 3);
    return `In ${assessment[1]} ${assessment[2]}, ${lowercaseFirst(assessment[3] || `the product showed ${topics}`)}`;
  }
...
/**
 * Query parameters that select a variant or carry click tracking. They are
 * stripped from schema entity URLs (@id/url) so one product keeps one stable
 * entity identity; product-identifying parameters (e.g. `prdSeq`) are kept.
 */
const nonCanonicalQueryParamPattern = /^(?:variant|variant_?id|selected_?variant|option_?id|utm_[a-z0-9_]+|gclid|fbclid|igshid|msclkid|mc_[ce]id|ref|ref_src|srsltid)$/i;

function canonicalizeSchemaSourceUrl(sourceUrl: string | undefined): string | undefined {
  if (!sourceUrl) {
    return undefined;
  }
  try {
    ...
    for (const key of Array.from(url.searchParams.keys())) {
      if (nonCanonicalQueryParamPattern.test(key)) {
...
function createSchemaMarkup(input: {
  ...
  // Entity identity must not fragment per variant selection or tracking
  // click: every variant shares one canonical @id/url, so AI engines resolve
  // one product entity regardless of which URL the crawler collected.
  const sourceUrl = canonicalizeSchemaSourceUrl(input.sourceUrl);
  const baseId = sourceUrl ?? `urn:agentic-geo:pdp:${slug(input.productName)}`;
  ...
  // P0 엔티티 계층: 소스가 운영 조직을 제공할 때만 Organization 노드를
  // 발행하고, 판매 주체를 Offer.seller로 연결한다(fail-closed).
  const organizationNode = createOrganizationSchema(input.hints?.organization);
  const organizationId = typeof organizationNode?.["@id"] === "string" ? organizationNode["@id"] : undefined;
  if (organizationNode && organizationId) {
    graph.push(organizationNode);
    for (const offerNode of Array.isArray(offer) ? offer : offer ? [offer] : []) {
      offerNode.seller = { "@id": organizationId };
    }
  }

  // Page-node typing: PDPs are ItemPage (a WebPage subtype); category pages
  // are CollectionPage; brand/about pages are AboutPage. The subtype is
  // emitted alongside WebPage so consumers matching either type keep working.
  const pageSubtype = input.hints?.pageType ?? "ItemPage";
  if (input.targets.includes("WebPage")) {
    graph.push(cleanJson({
      "@type": pageSubtype === "WebPage" ? "WebPage" : ["WebPage", pageSubtype],
      "@id": webpageId,
      url: sourceUrl,
      name: input.productName,
      description: input.webPageDescription,
      // Freshness is an on-page AI-citation gatekeeper (What Gets Cited,
      // SIGIR 2026). Emitted only from a source-provided date — never the
      // generation time (schema-org-product policy §5).
      dateModified: input.product.dateModified,
  ...
  if (input.targets.includes("Product")) {
    graph.push(cleanJson({
      ...
      ...(input.product.gtin && gtinPropertyName(input.product.gtin)
        ? { [gtinPropertyName(input.product.gtin) as string]: input.product.gtin }
        : {}),
      size: extractCurrentVariantSize(input.product),
      description: input.productDescription,
      // Entity-page hardening: official brand URLs (hints.brandSameAs) link the
      // Brand node to dereferenceable identities so the JSON-LD graph works as
      // a connected entity page for agentic retrieval, not isolated markup.
      brand: input.product.brand
        ? cleanJson({
            "@type": "Brand",
            "name": input.product.brand,
            sameAs: dedupeValidBrandSameAsUrls(
              input.hints?.brandSameAs ?? deriveBrandSameAsFromSourceUrl(input.product.brand, sourceUrl)
            )
          })
        : undefined,
      category,
      ...
      // P0 스키마 정책: positiveNotes(Pros&Cons)는 에디토리얼 리뷰 페이지
      // 전용이라 머천트 PDP에서는 부적격 — 벤핏 신호는 additionalProperty와
      // 가시 콘텐츠 섹션으로만 발행한다.
      additionalProperty: createAdditionalProperties(input.product, usageInstructions, input.locale, input.contentPlan, Boolean(variantOffers), Boolean(aggregateRating))
    }));
  }
  ...
  if (input.targets.includes("HowTo") && howToSteps.length > 0) {
    graph.push(cleanJson({
      ...
      step: howToSteps.map((step, index) => ({
        "@type": "HowToStep",
        position: index + 1,
        name: createHowToStepName(input.locale, index),
        text: step
      }))
    }));
  }
...
/**
 * States the step's position, which every step in an ordered procedure
 * genuinely has. Guessing a descriptive label from keywords produced public
 * copy that contradicted the step it labelled — an unanchored substring match
 * turned "Rinse thoroughly with lukewarm water" into "Warm and apply", and
 * "손상된 피부 부위에 얇게 펴 바릅니다" into "손에 덜어 펴 바르기" — and it also
 * mixed positional and descriptive labels inside one procedure.
 */
function createHowToStepName(locale: PdpGeoLocale, index: number): string {
  return fallback(locale, {
    "ko-KR": `${index + 1}단계`,
    "ja-JP": `ステップ${index + 1}`,
    "en-US": `Step ${index + 1}`,
...
function createOfferSchema(product: PdpProductSignal, locale: PdpGeoLocale, market?: string, sourceUrl?: string): JsonObject | undefined {
  ...
  return cleanJson({
    ...
    itemCondition: product.itemCondition ? schemaEnumUrl(product.itemCondition) : undefined,
    priceValidUntil: product.priceValidUntil,
    hasMerchantReturnPolicy: createMerchantReturnPolicySchema(product.returnPolicy),
    shippingDetails: createShippingDetailsSchema(product.shipping),
    url: sourceUrl
  }) as JsonObject;
}

/**
 * P0 엔티티 계층: 운영 조직 노드. 이름과 유효한 URL이 모두 있을 때만
 * 발행하며, sameAs는 유효한 절대 URL만 남긴다(빈 배열 미발행 —
 * 지식그래프 연결 경로가 없는 [] 는 순수 노이즈다).
 */
function createOrganizationSchema(organization?: PdpGeoGenerationHints["organization"]): JsonObject | undefined {
  ...
  let origin: string;
  try {
    origin = new URL(organization.url).origin;
  } catch {
    return undefined;
  }
  return cleanJson({
    ...
    sameAs: dedupeValidBrandSameAsUrls(organization.sameAs)
  }) as JsonObject;
}

/**
 * P0 커머스 신뢰 계층: 소스가 제공한 반품 정책만 구조화 객체로 발행한다.
 * 문자열 반품 정책("45 Days")은 타입 오류이며, Google 머천트 리스팅은
 * category/country가 있는 MerchantReturnPolicy 객체를 요구한다.
 */
function createMerchantReturnPolicySchema(policy: PdpProductSignal["returnPolicy"]): JsonObject | undefined {
  if (!policy) {
    return undefined;
  }
  return cleanJson({
    "@type": "MerchantReturnPolicy",
    returnPolicyCategory: schemaEnumUrl(policy.category),
    merchantReturnDays: policy.merchantReturnDays,
    returnMethod: policy.returnMethod ? schemaEnumUrl(policy.returnMethod) : undefined,
    returnFees: policy.returnFees ? schemaEnumUrl(policy.returnFees) : undefined,
    applicableCountry: policy.applicableCountry,
    returnPolicyCountry: policy.returnPolicyCountry ?? policy.applicableCountry,
    merchantReturnLink: policy.url
  }) as JsonObject;
}

/**
 * P0 커머스 신뢰 계층: 배송 리드타임은 자유 문자열이 아니라
 * ShippingDeliveryTime + QuantitativeValue(day)로만 발행한다.
 */
function createShippingDetailsSchema(shipping: PdpProductSignal["shipping"]): JsonObject | undefined {
  if (!shipping) {
    return undefined;
  }
  const dayRange = (min?: number, max?: number): JsonObject | undefined => (
    min === undefined && max === undefined
      ? undefined
      : cleanJson({ "@type": "QuantitativeValue", minValue: min, maxValue: max, unitCode: "DAY" }) as JsonObject
  );
  return cleanJson({
    "@type": "OfferShippingDetails",
    shippingDestination: { "@type": "DefinedRegion", addressCountry: shipping.destinationCountry },
    deliveryTime: cleanJson({
      "@type": "ShippingDeliveryTime",
      handlingTime: dayRange(shipping.handlingDaysMin, shipping.handlingDaysMax),
      transitTime: dayRange(shipping.transitDaysMin, shipping.transitDaysMax)
    }),
    shippingRate: shipping.rate
      ? { "@type": "MonetaryAmount", value: shipping.rate.amount, currency: shipping.rate.currency }
      : undefined
  }) as JsonObject;
}
 ...
 * to the single current-variant Offer otherwise, so option-specific price
 * and stock are never guessed. Flattened `Options`/`Variant comparison`
 * additionalProperty summaries are suppressed by the caller when this fires
 * (schema-org-product policy: no lossy duplication of variant structure).
 */
function createVariantOfferSchemas(
  product: PdpProductSignal,
  locale: PdpGeoLocale,
  market?: string,
  sourceUrl?: string
  ...
  const fallbackCurrency = normalizePriceCurrency(product.price?.currency) ?? inferCurrencyForMarket(market, locale);
  const offers = variants.flatMap((variant) => {
    const currency = normalizePriceCurrency(variant.currency) ?? fallbackCurrency;
    const amount = variant.price !== undefined ? normalizeOfferPriceAmount(variant.price, undefined, currency) : undefined;
    if (!currency || amount === undefined) {
      return [];
    }
    const label = cleanSignal(variant.title ?? "") || (variant.options.length > 0 ? variant.options.slice(0, 3).join(" / ") : undefined);
    return [cleanJson({
      "@type": "Offer",
      name: label,
      sku: variant.sku,
      ...
      price: amount,
      priceCurrency: currency,
      availability: variant.availability ? schemaEnumUrl(variant.availability) : undefined,
      // 반품·배송·가격 유효기간은 상품 단위 정책이므로 모든 variant Offer가
      // 동일 값을 공유한다(머천트 리스팅은 Offer 단위 필드를 읽는다).
      priceValidUntil: product.priceValidUntil,
      hasMerchantReturnPolicy: createMerchantReturnPolicySchema(product.returnPolicy),
      shippingDetails: createShippingDetailsSchema(product.shipping),
      url: variant.url ?? sourceUrl
    }) as JsonObject];
  });
  if (offers.length < 2) {
    return undefined;
  }
  const commerceSignatures = new Set(offers.map((offer) => `${String(offer.price)}|${String(offer.availability ?? "")}`));
  const identitySignatures = new Set(offers
    .map((offer) => String(offer.sku ?? offer.gtin ?? ""))
    .filter((value) => value.length > 0));
  if (commerceSignatures.size < 2 && identitySignatures.size < 2) {
    return undefined;
  }
  return offers;
}
...
function normalizeOfferPriceAmount(raw: string, amount: number | undefined, currency: string | undefined): number | undefined {
  return normalizeMonetaryAmountForCurrency(raw, amount, currency);
}
...
function createAdditionalProperties(
  ...
  locale: PdpGeoLocale,
  contentPlan?: PdpGeoContentPlan,
  suppressVariantFlattening = false,
  suppressRatingSummary = false
): JsonObject[] {
  const claimSentences = selectOptimizedSourceBackedClaimSentences(product, locale, 7);
  const ingredientEffectDetail = createIngredientEffectDetailProperty(product, locale, claimSentences[0]);
  ...
  // Review-derived Q&A units are unified into FAQPage.mainEntity and
  // diagnostics.inferredSearchQueries. Flat question+answer PropertyValues
  // duplicated the FAQPage role and Q&A-shaped flat fields correlate with
  // lower answer absorption (citation-absorption research).
  const entries: ProductAdditionalPropertyEntry[] = [
    ...
    { name: "Options", value: suppressVariantFlattening ? undefined : product.options.slice(0, 5).join(", ") }
  ];
...
function createModelPlannedCepProperties(contentPlan?: PdpGeoContentPlan): ProductAdditionalPropertyEntry[] {
  ...
  return contentPlan.cep
    ...
    .map((parts) => ({
      ...
      // Semicolon-joined atomic facts parse cleanly for AI consumers; the
      // former " · " bullet separator read as layout noise inside a value.
      value: parts.map((part) => part.replace(/[.。]\s*$/u, "")).join("; ")
    }));
}
...
function selectFunctionalCertificationSignals(product: PdpProductSignal, locale: PdpGeoLocale, limit = 3): string[] {
  ...
  // Atomic facts are mined per source value: a fragment spanning two joined
  // values (page prose + vendor + price) is a statement no source ever made.
  const atomicValueSignals = allProductEvidenceValues(product)
    .flatMap((value) => selectAtomicFunctionalCertificationValues(value, locale));
...
function normalizeFunctionalCertificationSentence(value: string, locale: PdpGeoLocale): string | undefined {
  ...
  // A long source sentence often wraps the completed-test fact in unrelated
  // page copy; publish the atomic fact it contains rather than the wrapper.
  const atomicFact = first(selectAtomicFunctionalCertificationValues(text, locale));
  if (atomicFact) {
    return atomicFact;
  }
  if (containsSerializedMetadata(text)) {
...
function createRoutineSynergyProperty(product: PdpProductSignal, locale: PdpGeoLocale): string | undefined {
  ...
  // Routine synergy must stay a routine-context fact. Appending the usage
  // instruction duplicated the Usage property (field-value blending is
  // forbidden) and produced broken "says to <imperative>" grammar.
  return locale === "ko-KR" ? routine : capitalizeFirst(routine);
}

function createConsumerSatisfactionProperty(product: PdpProductSignal, locale: PdpGeoLocale, omitRatingSummary = false): string | undefined {
  ...
  // When aggregateRating is emitted as structured data, the rating/count text
  // is a lossy duplicate (same pattern as variant Offers suppressing Options).
  const rating = !omitRatingSummary && product.reviews.rating && product.reviews.reviewCount
    ? fallback(locale, {
...
/** Broken extraction/refinement tokens that are never ingredient names. */
const nonIngredientTokenPattern = /^(?:what|which|how|why|when|where|who|it|this|that|these|those|other|others|etc|and|or|more|misc|unknown|n\/a|무엇|어떤|기타|그\s*외|등|외)$/i;

/**
 * List-separator comma that is not part of a number: a comma flanked by
 * digits on both sides is a thousands separator ("10,000ppm") or a chemical
 * locant ("1,2-헥산디올") and must stay inside the list item.
 */
...
function sanitizeProductSchemaPropertyText(name: string, value: string, locale: PdpGeoLocale): string {
  if (name === "Key ingredients") {
    // Drop broken tokens instead of discarding the whole list: entity density
    // (AutoGEO) favors keeping the valid ingredient names.
    return cleanSignal(value)
      .replace(/\\[rn]/g, " ")
      .replace(/\s+([,.])/g, "$1")
      .replace(/\s+/g, " ")
      .split(listSeparatorCommaPattern)
      .map((item) => item.trim())
      .filter((item) => item.length > 0 && !nonIngredientTokenPattern.test(item))
      .join(", ")
      .trim();
  }
  if (/^Variant comparison$/i.test(name)) {
    // Keep the concrete variant values; strip only the meta-label tail so the
    // property never promises attributes ("price, version name") it does not
    // carry. Label-only fallback values become empty and are dropped.
    return cleanSignal(value).replace(variantMetaLabelTailPattern, "").trim();
  }
...
function createSemanticClinicalEvidenceSummary(product: PdpProductSignal, locale: PdpGeoLocale): string | undefined {
  ...
  const formatted = claims
    .map((claim) => formatSemanticMetricClaim(claim, locale))
    .filter((value): value is string => Boolean(value));
  return formatted.length > 0 ? compactRepeatedStudyContextText(formatted.join(" ")) : undefined;
}
...
function formatKoreanSemanticClinicalClaim(claim: PdpSemanticMetricClaim): string | undefined {
  ...
  const timingAlreadyInLabel = Boolean(timing && (label.includes(timing)
    || numericClaimSignature(timing).some((token) => numericClaimSignature(label).includes(token))));
  const timingPrefix = timing && !timingAlreadyInLabel ? `${timing} ` : "";
  const baselinePhrase = baseline
    ? /(?:대비|비교)/u.test(baseline) ? baseline : `${baseline} 대비`
    : "";
  const result = label && value
    ? `${timingPrefix}${appendKoreanTopicParticle(label)} ${baselinePhrase ? `${baselinePhrase} ` : ""}${value}${claim.direction ? ` ${claim.direction}` : ""}`
...
function createReviewDerivedIndirectSearchQueries(product: PdpProductSignal, locale: PdpGeoLocale): ReviewDerivedSearchQuery[] {
  ...
  if (!target || !benefitPhrase || !supportBenefitPhrase || !isSpecificTargetCustomer(target, locale)) {
    return [];
  }
...
function createIngredientBenefitDirectQuery(product: PdpProductSignal, locale: PdpGeoLocale): ReviewDerivedSearchQuery | undefined {
  ...
  if (locale === "ko-KR") {
    ...
    return {
      kind: "direct",
      question: `${directEntity}의 주요 성분이나 기술은 무엇이고 어떤 역할이 명시되어 있나요?`,
  ...
  return {
    kind: "direct",
    question: `Which ingredients or technologies are highlighted for ${directEntity}, and what roles are explicitly stated?`,
    keywords: selectReviewQueryCoreKeywords(product, locale, benefits, directEntity),
...
function createPositiveReviewUseFeelDirectQuery(product: PdpProductSignal, locale: PdpGeoLocale): ReviewDerivedSearchQuery | undefined {
  ...
  if (locale === "ko-KR") {
    return {
      kind: "direct",
      question: `${directEntity} 리뷰에서 언급되는 사용감은 무엇인가요?`,
      keywords: selectReviewQueryCoreKeywords(product, locale, reviewKeywords, directEntity),
      answer: `${productName}의 긍정 리뷰에서는 ${appendKoreanSubjectParticle(formatKoreanListForSentence(reviewPhrase))} 언급됩니다`,
  ...
  return {
    kind: "direct",
    question: `What comfort and finish signals do reviews mention for ${directEntity}?`,
    keywords: selectReviewQueryCoreKeywords(product, locale, reviewKeywords, directEntity),
    answer: `Positive reviews for ${productName} mention ${reviewPhrase}`,
    source: "positive-review-usefeel",
    score: 18 + reviewKeywords.length + benefits.length
  };
}
...
function createModelPlannedSearchQueries(
  ...
  const descriptionAnswer = contentPlan.productDescription.include
    ...
    : contentPlan.webPageDescription.include
      ...
      : "";
  const cepQueries = contentPlan.cep.flatMap((cep, index): ReviewDerivedSearchQuery[] => {
  ...
  const seen = new Set<string>();
  return [...plannedFaqQueries, ...cepQueries].filter((query) => {
...
function normalizeInferencePublicText(value: string, locale: PdpGeoLocale): string {
  return value
    .split("\n")
    .map((line) => {
      let text = normalizePublicFactText(line).replace(/([.!?。！？])\1+/gu, "$1");
      if (locale === "ko-KR") {
        text = text
...
function isUsefulSchemaPropertyValue(name: string, value: string): boolean {
  ...
  // A public PropertyValue must be coherent copy, never a stitched
  // marketing-page dump — whatever population/period/method tokens the dump
  // also contains.
  ...
  if (name === "Key ingredients") {
    return value.split(",").every((item) => {
      ...
      return token.length > 0
        ...
        && !nonIngredientTokenPattern.test(token)
...
function createRecommendations(
  ...
  if (!product.availability) {
    recommendations.push({
      ...
      reason: "Provide availability in the input contract (for example InStock, SoldOut, or a boolean soldOut flag). Commerce surfaces treat price and availability as primary eligibility signals, and this generator never guesses stock state."
    });
  }
  if (guidance.sources.length > 0) {
    recommendations.push({
...
function readTerminologyConcepts(documents: Array<{ name: string; content: string }>): TerminologyConcept[] {
  ...
  const isDefaultMap = (name: string) => normalizeRagDocumentName(name) === pdpGeoGeneratorRagManifest.documents.localeTerminologyMap;
  const isBrandMap = (name: string) => brandTerminologyMaps.has(normalizeRagDocumentName(name));
  // Brand maps first so brand-specific definitions win on concept collisions.
  const orderedMaps = [
    ...documents.filter((item) => isBrandMap(item.name)),
    ...documents.filter((item) => isDefaultMap(item.name)),
    ...documents.filter((item) => !isBrandMap(item.name) && !isDefaultMap(item.name) && /terminology/i.test(item.name))
  ];

  const merged = new Map<string, TerminologyConcept>();
  for (const document of orderedMaps) {
    try {
      const parsed = JSON.parse(document.content) as { concepts?: TerminologyConcept[] };
      for (const concept of parsed.concepts ?? []) {
        if (concept?.concept && !merged.has(concept.concept)) {
          merged.set(concept.concept, concept);
        }
      }
    } catch {
      // Skip malformed maps; remaining sources still contribute.
    }
  }
...
function formatKoreanListForSentence(value: string): string {
  const items = value.split(listSeparatorCommaPattern).map((item) => item.trim()).filter(Boolean);
  if (items.length <= 1) {
    return value.trim();
  }
