...
describe("evidence-bound content planning", () => {
  ...
  it("keeps one polite Korean application direction visible with one-step HowTo schema", async () => {
    ...
    expect(run.result.content.sections.howToUse).toBe(direction.replace(/\.$/, ""));
  });
  ...
  it("does not treat two unrelated actions as an ordered procedure", async () => {
    ...
    // Clear usage content is still published, but unmarked actions must not
    // become an ordered procedure, so they collapse into one step.
    expect(result.plan.howTo.eligible).toBe(true);
    expect(result.plan.howTo.ordered).toBe(false);
    expect(result.plan.howTo.steps).toHaveLength(1);
    expect(result.plan.howTo.steps[0]?.text).toContain("Apply to the face.");
    expect(result.plan.howTo.steps[0]?.text).toContain("Massage the neck.");
  });
  ...
  it("builds an ordered HowTo from an explicitly numbered spray usage sequence", async () => {
    const result = await planPdpGeoContent(planningRequest({
      ...product,
      usage: [
    ...
    expect(result.plan.howTo.eligible).toBe(true);
    expect(result.plan.howTo.ordered).toBe(true);
    expect(result.plan.howTo.steps.map((step) => step.text)).toEqual([
      "연약하고 건조해진 피부 부위에 미세 분사를 합니다.",
      "피부에 건조함이 느껴질 때 수시로 뿌려줍니다."
    ]);
    expect(result.plan.howTo.steps.map((step) => step.position)).toEqual([1, 2]);
  });

  it("keeps an explicitly numbered spray sequence ordered while unmarked routine notes stay out of the procedure", async () => {
    const result = await planPdpGeoContent(planningRequest({
      ...product,
      usage: [
        "1. 연약하고 건조해진 피부 부위에 미세 분사를 합니다.",
    ...
    expect(result.plan.howTo.eligible).toBe(true);
    expect(result.plan.howTo.ordered).toBe(true);
    expect(result.plan.howTo.steps).toHaveLength(2);
    expect(result.plan.howTo.steps[0]?.text).toContain("미세 분사");
    expect(result.plan.howTo.steps[1]?.text).toContain("수시로 뿌려줍니다");
  });

  it("does not promote passive spray-benefit copy into HowTo", async () => {
    const result = await planPdpGeoContent(planningRequest({
      ...product,
      usage: [],
      sourceTexts: ["부드럽게 뿌려져 피부 표면에 보습막을 형성"]
    }), {});
  ...
  it("lets the model add evidence-backed FAQ and decide schema applicability before rendering", async () => {
    ...
    const webPage = graph.find((node) => (Array.isArray(node["@type"]) ? node["@type"].includes("WebPage") : node["@type"] === "WebPage")) as Record<string, unknown>;
  ...
  it("drops a narrower FAQ question whose evidence is a subset of a broader question's, even with low raw word overlap", async () => {
    const effectProduct = { ...product, benefits: ["수분 공급 효과"] };
    const request = planningRequest(effectProduct);
    const identity = request.evidenceLedger.find((item) => item.role === "identity")!;
    const description = request.evidenceLedger.find((item) => item.role === "description")!;
    const ingredient = request.evidenceLedger.find((item) => item.role === "ingredient")!;
    const benefit = request.evidenceLedger.find((item) => item.role === "benefit")!;
    // These two questions share only about 70% of their raw whole-question
    // vocabulary once the shared product name is counted in, which sits
    // under a naive Jaccard/union duplicate threshold and would let both
    // survive. The narrower question's cited evidence (identity+description
    // +benefit) is a strict subset of the broader question's (+ingredient),
    // so it asks the same thing with less detail and should not survive as a
    // second, near-duplicate FAQ entry -- mirroring a real generator run
    // that kept both "구성 성분과 효능·효과는 무엇인가요?" and "주요
    // 효능·효과는 무엇인가요?" as separate FAQ items.
    const result = await planPdpGeoContent(request, {
      customContentPlanner: {
        planContent: () => ({
          plan: planPayload({
            faq: [
              {
                include: true,
                question: "하이드라 세럼의 세라마이드 성분과 수분 공급 효과는 무엇인가요?",
                answer: "하이드라 세럼은 세라마이드와 함께 수분 공급 효과가 있습니다.",
                intent: "composition-benefit",
                cep: "구성 성분과 효능·효과를 함께 확인하는 상황",
                evidenceIds: [identity.id, description.id, ingredient.id, benefit.id],
                confidence: 0.94,
                omitReason: ""
              },
              {
                include: true,
                question: "하이드라 세럼의 수분 공급 효과는 무엇인가요?",
                answer: "하이드라 세럼은 수분 공급 효과가 있습니다.",
    ...
    expect(result.plan.faq).toHaveLength(1);
    expect(result.plan.faq[0]?.question).toBe("하이드라 세럼의 세라마이드 성분과 수분 공급 효과는 무엇인가요?");
  });
...
describe("provider-native structured output", () => {
  ...
  it("protects BestPractice tone guidance alongside GEO, CEP, and E-E-A-T in a small planner context", async () => {
    ...
    request.ragChunks = [
      ...
      { id: "best", source: "brands/examplederma/best-practice_v2.md", kind: "best-practice", text: "Use a calm, assured dermocosmetic voice with natural evidence transitions.", metadata: {}, score: 0.87 }
    ];
    ...
    // Task rules and voice are carried in separate slots: taskGuidance holds
    // the policy families that decide what a field may contain, while
    // BestPractice reaches the model through the tone slot in a longer excerpt.
    // It used to appear in both, the second copy truncated to 240 characters.
    expect(payload.taskGuidance.map((item: Record<string, unknown>) => item.kind)).toEqual(
      expect.arrayContaining(["geo-research", "cep", "eeat"])
    );
    expect(payload.taskGuidance.map((item: Record<string, unknown>) => item.kind)).not.toContain("best-practice");
    expect(payload.bestPracticeToneGuidance).toHaveLength(1);
    expect(payload.bestPracticeToneGuidance[0].source).toBe("brands/examplederma/best-practice_v2.md");
