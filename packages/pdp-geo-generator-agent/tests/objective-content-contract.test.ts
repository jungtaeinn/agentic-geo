...
import {
  ...
  unorderedSingleStepHowToCases,
  japaneseLocalePurityInput,
  sensibleDescriptionInput,
  sparseFaqInput
} from "./fixtures/objective-content-contract-cases";
import { graphOf, isGraphNode, nodeOf, type GraphNode } from "./support/graph";
...
describe("objective GEO content contracts", () => {
  describe("HowTo eligibility", () => {
    ...
    it.each(unorderedSingleStepHowToCases)("publishes one unordered HowTo step for $label", async ({ input }) => {
      const run = await generatePdpGeo(input);
      const howTo = nodeOf(graphOf(run), "HowTo");
      const steps = Array.isArray(howTo?.step) ? howTo.step.filter(isGraphNode) : [];
