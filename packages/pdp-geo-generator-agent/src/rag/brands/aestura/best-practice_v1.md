# AESTURA Best Practice v1

This brand-scoped best practice extends the default `best-practice_v1.md` for AESTURA PDP GEO generation. Use this document only when the normalized product brand, hint, or product name maps to AESTURA / 에스트라. When this document is active, it replaces the default best-practice document for best-practice retrieval; general schema, E-E-A-T, CEP, GEO research, official-docs, locale, and terminology documents still apply.

## Brand-Specific Best Practice Overlay

AESTURA output should preserve the default field evidence contract while adding the brand's distinctive quality bar:
