...
- `Product.description` can synthesize target customer, product identity, ingredients/technology, benefit/effect, citation-ready metric evidence, and high-level usage/comparison/review context in that order when evidence exists. It must not reuse HowTo sentences as if they were benefits or use internal labels such as "usage context", "customer review language", or "source-backed product evidence".
