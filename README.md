...
`0.2.0` 기준의 중요한 변화는 RAG 문서를 단순히 많이 넣는 방식이 아니라, 문서/섹션 단위 metadata와 field evidence contract로 관리한다는 점입니다. `llms.txt` 같은 대표 링크 문서에 의존하지 않고 typed RAG index, section-aware chunking, subquery planning, strategic full document hydration을 조합해 FAQ, HowTo, ingredient, benefit, claim evidence가 각자 맞는 필드로 들어가도록 추론하고 검증합니다.
...
여기에 더해 generator는 retrieval에서 선택되지 않은 문서의 요구사항도 놓치지 않도록, 로드된 모든 RAG 정책 문서를 원자 규칙 단위의 **compiled policy checklist**로 컴파일해 LLM copy refinement 프롬프트에 함께 주입합니다. critical 규칙은 전량 주입이 보장되고, 브랜드 서사 섹션은 `brand-context`로 강등되어 규칙으로 위장하지 못하며, 모델은 `ruleCompliance`로 자가 검증 결과를 보고합니다. 주입률은 `diagnostics.policyCoverage`로 계측됩니다. 상세 플로우는 `packages/pdp-geo-generator-agent/README.md`의 "RAG를 추론에 활용하는 방식" 섹션을 참고하세요.
...
- repair가 필요한 경우 `validationWarnings`, `validationRepairs`, `evidence`에 보정 내역 기록
