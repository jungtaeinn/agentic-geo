...
const localizedCreativeWorkTypes = new Set(["WebPage", "ItemPage", "CollectionPage", "AboutPage", "FAQPage", "HowTo"]);

/** Node @type values as an array (page nodes may be typed ["WebPage", "ItemPage"]). */
function nodeTypeValues(node: SchemaNode): string[] {
...
export function repairPdpSchemaGraphIntegrity(
  ...
  const repairedGraph = retained.map((node) => {
    let next = node;
    const types = nodeTypeValues(node);
    const type = types[0];

    if (types.some((item) => localizedCreativeWorkTypes.has(item)) && node.inLanguage !== locale) {
    ...
    if (nodeTypeValues(next).includes("WebPage") && next.hasPart !== undefined) {
...
function renderHowToText(graph: SchemaNode[]): string {
  ...
  // validated step has no order to show, so it stays a plain sentence.
  const [only] = steps;
  return steps.length === 1 ? (only ?? "") : steps.map((text, index) => `${index + 1}. ${text}`).join("\n");
}
