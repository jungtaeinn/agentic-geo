  ...
  if (isRecord(faqPage) && Array.isArray(faqPage.mainEntity)) {
    faqPage.mainEntity.forEach((item, index) => {
      ...
      const plannedEvidence = plannedMatches && planned
        ...
        : [];
      const faqRoles: PdpGeoAtomicEvidence["role"][] = ["identity", "description", "benefit", "effect", "ingredient", "audience", "usage", "metric", "faq", "review", "source"];
      const plannedQuestion = renderedSentenceEvidence(item.name, plannedEvidence, faqRoles);
      const plannedAnswer = renderedSentenceEvidence(item.acceptedAnswer.text, plannedEvidence, faqRoles);
      // A refined FAQ no longer matches its plan text verbatim; bind it from
      // the full ledger sentence-by-sentence instead of dropping it from
      // final proofreading entirely.
      const fallbackQuestion = !plannedMatches && directIds.length === 0
        ? renderedSentenceEvidence(item.name, input.evidenceLedger, faqRoles)
        : undefined;
      const fallbackAnswer = !plannedMatches && directIds.length === 0
        ? renderedSentenceEvidence(item.acceptedAnswer.text, input.evidenceLedger, faqRoles)
        : undefined;
      ...
      add(
        ...
        item.name,
        plannedMatches ? plannedQuestion.evidenceIds : fallbackQuestion?.evidenceIds ?? ids,
        origin,
        plannedMatches ? plannedQuestion.sentenceEvidenceIds : fallbackQuestion?.sentenceEvidenceIds
      );
      add(
        `FAQPage.mainEntity[${index}].acceptedAnswer.text`,
        item.acceptedAnswer.text,
        plannedMatches ? plannedAnswer.evidenceIds : fallbackAnswer?.evidenceIds ?? ids,
        origin,
        plannedMatches ? plannedAnswer.sentenceEvidenceIds : fallbackAnswer?.sentenceEvidenceIds
      );
    });
  }
...
/**
 * Fraction of the target's substantive tokens (particle/inflection-normalized)
 * that also appear in the reference text — a containment signal for public
 * sentences composed from ledger atoms.
 */
function evidenceTokenCoverage(target: string, reference: string): number {
  const locale: PdpGeoLocale = /[가-힣]/u.test(target) ? "ko-KR" : /[ぁ-んァ-ン一-龯]/u.test(target) ? "ja-JP" : "en-US";
  const targetTokens = substantiveTokenSignature(target, locale);
  if (targetTokens.length === 0) return 0;
  const referenceTokens = new Set(substantiveTokenSignature(reference, locale));
  const covered = targetTokens.filter((token) => referenceTokens.has(token)).length;
  return covered / targetTokens.length;
}

/**
 * True when the evidence carries a scope the sentence dropped: a negation
 * ("do not show"), a hedge or non-human scope ("preliminary", "in vitro"),
 * or an attribution ("one customer review says"). Dropping such a qualifier
 * turns supported text into an unsupported stronger claim, so scope-inverted
 * evidence never counts as semantic support (factual-safety invariant).
 */
function evidenceScopeMismatch(sentence: string, evidenceText: string): boolean {
  const hazardClasses: RegExp[] = [
    /\b(?:not|no|never|cannot|nor)\b|않|없|아니|못\s*하|불가|되지\s*않/iu,
    /\b(?:preliminary|suggests?|may|might|could|in\s+vitro|inconclusive)\b|시험관|가능성|예비\s*(?:연구|결과)|추정/iu,
    /\b(?:review|reviews|reviewer|customer|customers|consumer)\b|리뷰|후기|고객|소비자/iu
  ];
  return hazardClasses.some((hazard) => hazard.test(evidenceText) && !hazard.test(sentence));
}
...
function renderedSentenceEvidence(
  ...
  const sentenceEvidenceIds = splitSentences(text).map((sentence) => {
    ...
    const lexicalMatches = eligible.filter((item) => {
      ...
      if (item.role === "source" || item.role === "description") {
        // Public sentences are COMPOSED from ledger atoms, so an exact-match
        // requirement left almost every rendered field unbound and silently
        // skipped by final proofreading. A long source/description atom may
        // also support a sentence semantically: most of its content tokens
        // appear in the sentence, or the sentence draws almost all of its
        // tokens from the atom. Trivial short atoms (brand tokens, currency
        // codes) stay exact-only; an atom whose negation/hedge/attribution
        // scope the sentence dropped never counts as support; and
        // sentenceEvidenceSupportsClaimFrame still enforces marker/numeric/
        // subject safety on every match.
        if (exactEvidenceSentence) return true;
        if (candidate.length < 24) return false;
        if (evidenceScopeMismatch(sentence, item.text)) return false;
        return evidenceTokenCoverage(item.text, sentence) >= 0.7
          || evidenceTokenCoverage(sentence, item.text) >= 0.7
          || splitSentences(item.text).some((evidenceSentence) => (
            normalizeEvidenceText(evidenceSentence).length >= 24
              && !evidenceScopeMismatch(sentence, evidenceSentence)
              && evidenceTokenCoverage(sentence, evidenceSentence) >= 0.7
          ));
      }
