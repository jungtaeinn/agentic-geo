...
describe("generatePdpGeo", () => {
  it("writes Korean FAQ answers as direct AI-citation-friendly customer answers", async () => {
    ...
    const benefitFaq = faqItems[0] as Record<string, any>;
    const ingredientFaq = faqItems[1] as Record<string, any>;
    const reviewFaq = faqItems.find((item) => /고객\s*리뷰/u.test(String(item.name)))!;
    const benefitAnswer = String(benefitFaq.acceptedAnswer.text);
    const ingredientAnswer = String(ingredientFaq.acceptedAnswer.text);
    const reviewAnswer = String(reviewFaq.acceptedAnswer.text);
    ...
    expect(benefitAnswer).toMatch(/예시더마 모이베리어365 바디로션은[^.]*수분[^.]*피부 장벽[^.]*돕습니다/u);
    expect(benefitAnswer).toContain("피부 장벽");
    expect(benefitAnswer).toContain("바디로션입니다");
    // Task 3 fix: a suitability (product-detail-intent) answer must never
    // carry customer-review language -- the fact stays where it is
    // attributed, in the dedicated review-intent FAQ entry below.
    expect(benefitAnswer).not.toMatch(/고객\s*리뷰/u);
    expect(reviewAnswer).toMatch(/촉촉한 사용감[^.]*흡수감[^.]*언급/u);
  ...
  it("separates Korean target customer, brand science, and actionable usage through evidence reasoning", async () => {
    ...
    const { result } = await generatePdpGeo({
      product: {
        geoProduct: {
          ...
          ingredients: [
            ...
            technologyEvidence,
            "BotanON® 기술은 세라마이드, 콜레스테롤, 지방산을 포함한 독자적인 2-STEP 수분장벽 케어 기술로 소개된다."
          ],
          usage: [
            ...
            "예시더마 모이베리어365 캡슐 토너는 PHA 워터에 고밀도 세라마이드 캡슐을 띄운 특허 출원 하이드로퀄 플로팅 포뮬러 기술을 사용한다",
            "손바닥에 적당량을 덜어줍니다.",
      ...
      customCopyRefiner: {
        refineCopy: () => ({
          schemaDescriptions: {
            webPage: "예시더마 모이베리어365 캡슐 토너 상품 페이지는 건조하고 민감한 피부 고객이 세안 후 첫 단계 장벽 보습 토너를 평가할 수 있도록 PHA 워터, 고밀도 세라마이드 캡슐, 특허 출원 하이드로퀄 플로팅 포뮬러를 핵심 성분/기술로 설명합니다. 손바닥에 적당량을 덜어 얼굴 전체에 펴 바른 뒤 가볍게 두드려 흡수시키는 방법과 캡슐이 워터에 떠 있는 이유, 크림 캡슐 동일 여부를 FAQ와 HowTo에서 확인할 수 있습니다.",
            product: "건조하거나 민감한 피부 고객을 위한 예시더마 모이베리어365 캡슐 토너는 PHA 워터에 고밀도 세라마이드 캡슐을 띄운 특허 출원 하이드로퀄 플로팅 포뮬러의 장벽 보습 캡슐 토너입니다."
          },
          schemaProperties: {
          ...
          contentSections: {
            description: "건조하거나 민감한 피부 고객을 위한 예시더마 모이베리어365 캡슐 토너는 PHA 워터에 고밀도 세라마이드 캡슐을 띄운 특허 출원 하이드로퀄 플로팅 포뮬러의 장벽 보습 캡슐 토너입니다."
          }
        })
      }
    });
    ...
    expect(String(webPage.description)).toContain("예시더마 모이베리어365 캡슐 토너 상품 페이지는 EXAMPLEDERMA가 선보이는");
    expect(String(webPage.description)).toMatch(/예시더마 모이베리어365 캡슐 토너는[^.]*고객을 위한 제품/u);
    expect(String(webPage.description)).toMatch(/[^.]+(?:을|를) 주요 성분·기술로 포함하고[^.]+돕습니다/u);
    expect(String(webPage.description)).toContain("예시더마 모이베리어365 캡슐 토너는 30,000원에 판매되고 있습니다");
  ...
  it("keeps Korean cream mist descriptions and review queries grounded in the specific product type", async () => {
    const { result } = await generatePdpGeo({
      product: {
        geoProduct: {
          ...
          faq: [
            {
              question: "모이베리어 365 크림 미스트는 어떤 피부에 적합한가요?",
      ...
      source: {
        type: "pdp-extractor",
        url: "https://shop.example.com/web/product/view.do?prdSeq=1027"
      },
      hints: {
        locale: "ko-KR",
        market: "KR"
    ...
    const productNameIndex = productDescription.indexOf("모이베리어 365 크림 미스트");
    ...
    expect(description).toContain("모이베리어 365 크림 미스트 상품 페이지는 EXAMPLEDERMA가 선보이는");
    expect(description).toMatch(/모이베리어 365 크림 미스트는[^.]*건조하고 민감한 피부 고객을 위한 제품/u);
    ...
    const { result } = await generatePdpGeo({
      product: {
        geoProduct: {
          ...
          ingredients: ["보태니컴플렉스", "진생펩타이드", "비타민C 유도체"],
          usage: ["아침, 저녁 크림 단계에서 적당량을 취해 피부결을 따라 부드럽게 펴 발라 줍니다."],
          faq: [
            {
              question: "보태니컬크림의 핵심 성분은 무엇인가요?",
              answer: "보태니컴플렉스는 희귀 인삼 사포닌을 농축해 콜라겐 케어를 돕고, 진생펩타이드와 비타민C 유도체는 탄력과 항산화 케어 맥락을 제공합니다."
            },
            {
              question: "보태니컬크림의 효과는 얼마나 지속되나요?",
              answer: "사용 중단 1주 후에도 탄력 및 팔자주름 개선 효과가 지속됩니다 (35~55세 여성 33명 인체 적용 시험)."
            },
            {
              question: "보태니컬크림을 다른 제품과 함께 사용하면 효과가 더 좋아지나요?",
              answer: "에센셜에센스, 보태니컬캡슐세럼, 보태니컬크림 리치와 함께 쓰는 루틴에서 영양, 탄력, 주름 개선 만족도 지표가 함께 제시됩니다."
            },
            {
              question: "보태니컬크림은 어떤 피부 고민에 효과가 있나요?",
              answer: "탄력 저하, 주름, 피부 밀도 감소 등 노화 징후에 효과적입니다. 피부 노화지수 -25%, 이마 주름 -36.6%, 리프팅 +103.5%의 결과가 제시됩니다."
            },
            {
              question: "보태니컬크림과 보태니컬크림 리치의 차이점은 무엇인가요?",
              answer: "보태니컬크림은 산뜻한 고밀도 텍스처와 비타민C 유도체를 강조하고, 보태니컬크림 리치는 리치 텍스처와 진생레티놀을 강조합니다."
            },
            {
              question: "보태니컬크림은 민감한 피부도 사용할 수 있나요?",
              answer: "민감 피부 사용 적합 테스트를 완료해 민감성 피부도 선택 기준으로 참고할 수 있습니다."
            },
            {
              question: "보태니컬크림과 탄력크림 EX 중 어떤 것을 선택해야 하나요?",
              answer: "노화 징후가 신경 쓰이기 시작한 고객은 보태니컬크림, 탄력 기본기와 보습 장벽 케어를 우선하는 고객은 탄력크림 EX를 비교할 수 있습니다."
            },
            {
              question: "보태니컬크림 클래식과 소프트는 어디서 구매할 수 있나요?",
              answer: "2024년 9월 리뉴얼로 단종되었습니다. 기존 클래식 사용자는 보태니컬크림 리치, 소프트 사용자는 보태니컬크림을 대체 옵션으로 비교할 수 있습니다."
            }
          ],
          reviews: {
            rating: 4.9,
          ...
          sourceTexts: [
            "60년 인삼과학의 정수 자생력으로 차오른 고밀도 피부 NEW | 보태니컬크림 피부 탄력이 개선된 느낌 93.5% 피부 자생력이 강화된 느낌 90.3% 피부 결이 부드러워진 느낌 96.7% ExampleLuxe",
            "18개의 노화 신호 케어 비타민C 유도체, NEW, 보태니컬크림",
  ...
  it("does not append benefit or conflicting category terms to a product name that already has a product type", async () => {
    const { result } = await generatePdpGeo({
      product: {
        geoProduct: {
          ...
          images: [
            "https://shop.example.com/cdn/shop/files/SWS_Thumbnail_GCF_1080x1080_200ml.jpg"
          ],
          ...
          sourceExtraction: {
            ocr: {
              textBlocks: [
                "Botanical Ginseng Rejuvenating Serum Mini, Korean travel sized serum, product shot.",
                "Botanical Ginseng Rejuvenating Cream Rich, korean cream, pack shot.",
                "Person applying a skincare product to their hand with text 'Gentle, Non-Stripping Formula' in the corner."
              ],
              imageTexts: [
                {
                  imageUrl: "https://shop.example.com/cdn/shop/files/BRAND.COM_1080x1080_NewCGRSerum_01.Packshot_50ml.jpg",
                  text: "Botanical Ginseng Rejuvenating Serum Mini, Korean travel sized serum, product shot."
                }
              ]
            }
          }
      ...
      source: {
        type: "pdp-extractor",
        url: "https://shop.example.com/products/gentle-cleansing-foam?variant=41663478792237"
      },
      hints: {
        locale: "en-US",
        market: "US"
    ...
    expect(serialized).not.toMatch(/product shot|pack shot|travel sized serum|model applying product|person applying|with text|in the corner|Botanical Ginseng Rejuvenating Serum Mini|Botanical Ginseng Rejuvenating Cream Rich/i);
  ...
  it("derives review-backed CEP recommendation PropertyValues from positive reviews while excluding negative review complaints", async () => {
    ...
    const canonicalIngredientQuestion = faqQuestions.find((question: string) => /보태니컬크림의 구성 성분과 효능[·・]?효과/u.test(question));
    expect(canonicalIngredientQuestion).toBeDefined();
    expect(faqQuestions).not.toContain("예시럭셔리 보태니컬크림은 어떤 피부 고민과 효능에 적합한가요?");
  ...
  it("keeps benefit context free of marketing fragments and clinical sample fragments", async () => {
    ...
    // Pairs, not lists: prose narrows a coordinate run to two items in either
    // language, and the complete set stays in the product properties. The
    // intent — the page names its formula components and its benefits — holds.
    expect(webPage.description).toMatch(/lists Korean Ginseng Actives and Retinol as highlighted formula components.*documents .*product benefits/i);
    expect(webPage.description).toMatch(/Directions place the product in a morning and evening post-toner routine.*One customer review highlights smooth and moisture/i);
    expect(hasUnpredicatedEnumeration(String(webPage.description))).toBe(false);
  ...
  it("keeps essential-care style home-usage OCR evidence grounded and removes weak CEP expansion", async () => {
    const { result } = await generatePdpGeo({
      product: {
        ...
        effects: [
          ...
          "AFTER ONE BOTTLE OF DAILY USE*: 100% users had visible improvement in FINE LINES SKIN ELASTICITY DULLNESS *Instrumental result, 30 subjects, after 8 weeks of daily use FIRST CARE ACTIVATING SERUM VI SÉRUM ACTIVATEUR VI PREMIERS SOINS ExampleLuxe"
        ],
        metrics: [
        ...
        usage: ["Warm 2-3 pumps of Essential Care Activating Serum to the palm of your hands, then apply morning and night."],
        reviews: {
          ...
          faq: [
            {
              ...
              answer: "모이베리어365 캡슐 토너는 PHA 워터에 고밀도 세라마이드 캡슐을 띄운 하이드로겔 플로팅 포뮬러 제품입니다."
            },
            {
