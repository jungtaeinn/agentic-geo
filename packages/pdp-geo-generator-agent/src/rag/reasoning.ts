...
// Field contracts are cross-cutting by construction: the six contracts govern
// descriptions, FAQ, HowTo, schema safety, public wording, and evidence
// routing, so every routing principle can fall back to them.
const crossCuttingRagKinds = ["orchestration", "field-contracts", "eeat", "cep", "geo-research"] as const satisfies readonly PdpGeoRetrievedChunk["kind"][];
...
const ragRoutingByPrinciple: Record<RagRoutingKey, RagRoutingRule> = {
  ...
  review: {
    ...
    fieldTargets: ["FAQPage.mainEntity", "Product.description"],
    fallbackKinds: [...crossCuttingRagKinds, "schema", "best-practice"]
  }
};
...
export function createPdpGeoReasoning(input: CreatePdpGeoReasoningInput): PdpGeoReasoningResult {
  ...
  const decisions: PdpGeoReasoningDecision[] = [
    createDecision({
      ...
      // Effects are outcome evidence of the same class as benefits: the FAQ
      // anchor builder derives its benefit answers from either, so the
      // enablement decision must count both or an effects-only product loses
      // its FAQ despite having answerable outcome facts.
      productEvidence: [...evidence.benefits, ...evidence.effects, ...evidence.ingredients, ...evidence.usage, ...evidence.faq, ...evidence.reviews].slice(0, 8),
      fallbackSources: productEvidenceSources,
      rationale: "FAQ generation should be grounded in selected schema/GEO RAG guidance plus product benefit, effect, ingredient, usage, FAQ, or review evidence. Brand identity can influence wording only, not answer evidence."
    }),
    createDecision({
