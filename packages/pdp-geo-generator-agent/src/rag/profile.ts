...
export async function writePdpGeoGeneratorRagProfile(
  ...
  for (const defaultDocument of defaultPdpGeoGeneratorRagProfile.documents) {
    const incomingDocument = incomingDocuments.get(defaultDocument.name);
    if (incomingDocument) {
      await writeManagedRagFile(directory, defaultDocument.name, incomingDocument.content);
      continue;
    }
    // Never overwrite an existing managed file with the inline fallback snapshot.
    // The fallback in default-profile.ts is a generated verbatim copy of the
    // committed corpus (see default-profile-fallback.generated.ts) that can still
    // be stale relative to disk if a managed file was edited without regenerating
    // it. Restore from the fallback only when the managed file is missing on disk.
    if (!existsSync(join(directory, defaultDocument.name))) {
      await writeManagedRagFile(directory, defaultDocument.name, defaultDocument.content);
    }
  }
...
/**
 * Restores the default analysis prompt, restores missing managed RAG files from
 * the inline fallback, and clears custom attachments.
 *
 * Existing managed files on disk are intentionally preserved: the committed
 * package documents are the authoritative corpus, and the inline fallback in
 * default-profile.ts is only a generated verbatim snapshot of that corpus
 * (see default-profile-fallback.generated.ts), which can lag behind disk if
 * a managed file changed without regenerating it. Passing the full default
 * document list here would risk overwriting newer on-disk edits with a
...
export async function resetPdpGeoGeneratorRagProfile(directory = ragDirectory): Promise<StoredPdpGeoGeneratorRagProfile> {
  ...
  return writePdpGeoGeneratorRagProfile({ analysisPrompt: defaultPdpGeoGeneratorRagProfile.analysisPrompt }, directory);
}
...
async function readManagedDocument(directory: string, document: PdpGeoGeneratorRagDocument): Promise<StoredPdpGeoGeneratorRagDocument> {
  ...
  return {
    ...document,
    // The filename version suffix (e.g. `_v2`) is authoritative so chunk and
    // diagnostics metadata cannot drift from the document actually loaded.
    version: extractVersion(document.name) ?? document.version,
    content,
    managed: true,
    path: document.name,
    ...metadata
...
function extractVersion(name: string): string {
  const match = name.match(/_v(\d+)\.[^.]+$/i)?.[1];
  return match ? `v${match}` : "v1";
}
