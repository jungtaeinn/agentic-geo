...
describe("post-validation schema graph integrity", () => {
  it("removes empty FAQPage and HowTo nodes, their local references, and their visible panels", () => {
    ...
    const webPage = graph.find((node) => (Array.isArray(node["@type"]) ? node["@type"].includes("WebPage") : node["@type"] === "WebPage"));
  ...
  it("keeps valid and external hasPart references while removing a missing local fragment", () => {
    ...
    const webPage = graphOf(result).find((node) => (Array.isArray(node["@type"]) ? node["@type"].includes("WebPage") : node["@type"] === "WebPage"));
  ...
  it("rebuilds visible FAQ and retains a valid single-step HowTo with matching visible usage", () => {
    ...
    const result = validate({
      ...
      // A single validated step has no order to show, so the matching
      // visible copy is a plain sentence rather than an always-"1."-prefixed
      // one-item list.
      howToUse: validStep
    });

    expect(result.content.sections.faq).toBe(`Q. ${validQuestion}\nA. ${validAnswer}`);
    expect(result.content.sections.howToUse).toBe(validStep);
    expect(result.content.html).toBe("");
