...
describe("evidence-rich GEO regression contracts", () => {
  ...
  it("adopts an evidence-gated CEP WebPage narrative instead of rebuilding it from fixed sentence templates", async () => {
    ...
    const webPage = graph
      .filter((node): node is Record<string, JsonValue> => typeof node === "object" && node !== null && !Array.isArray(node))
      .find((node) => (Array.isArray(node["@type"]) ? node["@type"].includes("WebPage") : node["@type"] === "WebPage"))!;
    ...
    expect(description).toMatch(new RegExp(`${productName} 상품 페이지는 EXAMPLEDERMA가 선보이는 장벽 수분 크림의 특징과 제품 선택에 필요한 정보를 한데 담고 있습니다`, "u"));
    expect(description).not.toMatch(/EXAMPLEDERMA의[^.]*크림 상품을 소개합니다/u);
  ...
  it("removes stiff certification and source-report sentences during final description validation", () => {
    ...
    const serializedDescriptions = graph
      .filter((node): node is Record<string, JsonValue> => typeof node === "object" && node !== null && !Array.isArray(node))
      .filter((node) => (Array.isArray(node["@type"]) ? node["@type"].includes("WebPage") : node["@type"] === "WebPage") || node["@type"] === "Product")
  ...
  it("requires an explicit semantic link before relating an ingredient to a benefit", async () => {
    ...
    const webPageNode = unlinkedNodes.find((node) => (Array.isArray(node["@type"]) ? node["@type"].includes("WebPage") : node["@type"] === "WebPage"))!;
    ...
    const faqQuestions = (faqNode.mainEntity as JsonValue[])
      .filter((item): item is Record<string, JsonValue> => typeof item === "object" && item !== null && !Array.isArray(item))
      .map((item) => String(item.name ?? ""));
    // With no clinical/assessment evidence to anchor a distinct question, the
    // plain "what are the benefits" question merges into the composition
    // question rather than publishing both as near-duplicate FAQ entries.
    expect(faqQuestions).toContain("What are the key ingredients and supported benefits of Clear Serum?");
  ...
  it("renders ExampleLuxe US composition, clinical timelines, and benefit FAQ as natural attributable English", async () => {
    const { result } = await generatePdpGeo({
      product: {
        name: "ExampleLuxe Hydro Balance Cream",
        brand: "ExampleLuxe",
    ...
    expect(description.match(/ExampleLuxe Hydro Balance Cream/gu)?.length ?? 0).toBe(2);
    expect(description).toMatch(/ExampleLuxe Hydro Balance Cream (?:includes|combines|uses) Ginseng Peptide and Hyaluronic Acid/iu);
