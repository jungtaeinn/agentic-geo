...
import { deriveCepSituationsForPlanning } from "../review-cep";
import type {
...
export function createPlanningPrompt(request: PdpGeoContentPlanningRequest, maxEvidenceItems: number, maxRagChunks: number): { system: string; user: string } {
  ...
  return {
    system: [
      ...
      "Build FAQ backwards from natural recommendation/comparison queries and answers forwards from cited product evidence. Prefer concern + skin type/category, ingredient + supported role, source-backed CEP, or attributed use-feel. Cooling plus a water-cream formula may support a hot-condition or after-sweating refreshing-feel question, but never sweat control, heat treatment, or another physiological effect. Product-specific questions must name the exact product instead of using a deictic subject that only points at the product, such as 이 제품, 이 크림, this product, or this cream. Seasonal, gift, and life-stage contexts require matching current-product evidence. FAQ has no target or minimum count, and must never be padded toward one; zero questions is valid when the source does not support a useful, direct answer. Select only the distinct customer intents that materially help a buying or usage decision; schema presence and question volume are not citation guarantees. Build suitability answers as target/concern -> finished-product benefit -> most relevant result as proof -> explicitly linked ingredient role -> bounded recommendation -> individual-results qualifier. State qualifiers directly to the customer (for example, '개인 차가 있을 수 있습니다' or 'Individual results may vary'); never narrate that a disclaimer, caveat, condition, or note is attached. Keep unlinked ingredient and finished-product effects separate. Reviews cannot prove efficacy or suitability. Ask each question in the words a buyer would use, colloquial phrasing included, never in specification-sheet or internal-classification vocabulary; topic wording is the first thing an answer engine matches. Make the opening sentence of every answer a complete answer that stands alone, and let the rest qualify or support it, because an answer engine lifts one sentence. Prefer the specific over the general in question and answer alike: a stated figure, condition, or population can be cited; a statement that would read the same for a competing product cannot.",
      "For a benefits FAQ, write what a customer would type, never an audit phrase. Name the product and the concern or outcome the buyer is deciding about; mention a result only when finished-product human-application evidence exists. Never ask for the internal evidence record ('what product evidence supports them?'). When the question does name the method, use the source's own: 'clinical study' only for clinical evidence, 'instrumental assessment' for instrumental, otherwise the neutral 'reported assessment results'. A PDP-reported study is not necessarily published or peer-reviewed, so reserve those terms, and 공개된, for an actual bibliographic citation. Never copy a question wording out of these instructions; the properties are the rule.",
      ...
      // Stages 1-2 of the review-to-CEP pipeline run in code and arrive as
      // `reviewSituations`; stages 3-5 are the model's. Stages 4-5 (query
      // reverse-engineering, answer ordering) are already stated by the FAQ
      // instruction above, so only the new input and its boundary are added.
      "`reviewSituations` lists buying/use contexts the source states, each with its clause, its evidence IDs, and an `origin`: `product` for the brand's own copy and `review` for a customer's own words. Only recommending reviews are read, because a FAQ built on a situation recommends the product for it. Treat the list as the primary material for `cep` and the FAQ built on it: a stated situation is a query buyers actually make, while a product field restated as a situation is not. Prefer a `product` origin over a `review` one for the same situation, since official copy is the stronger citation. Pair each situation with the need it implies, and take every product claim only from official benefit, effect, ingredient, metric, usage, or commerce evidence — a review cannot establish what a product does or whom it suits. Drop a situation no official evidence answers instead of answering it from category knowledge.",
      "When a formula or technology is worth a FAQ of its own, ask it as the decision it changes for the buyer, not as a specification lookup: what the buyer can now do, or stop worrying about, is the question they type. A question that only asks what a named process is reads as a spec sheet and competes with the composition question already published.",
    ...
    user: JSON.stringify({
      ...
      reviewSituations: selectPlanningReviewSituations(request),
...
/**
 * 5단계 파이프라인의 1~2단계 결과를 프롬프트 입력으로 만듭니다.
 * 리뷰에서 추출한 구매·사용 상황에 그 상황을 진술한 리뷰 근거 ID를 붙여,
 * 모델이 3단계(CEP 도출)에서 곧바로 인용할 수 있게 합니다.
 */
function selectPlanningReviewSituations(request: PdpGeoContentPlanningRequest): Array<{
  situation: string;
  origin: string;
  clause: string;
  support: number;
  ...
  const reviewEvidenceByIndex = new Map<number, string>();
  for (const item of request.evidenceLedger) {
    const match = /product\.reviews\.items\[(\d+)\]/u.exec(item.sourcePath);
    if (match?.[1]) reviewEvidenceByIndex.set(Number(match[1]), item.id);
  }
  return deriveCepSituationsForPlanning(request.product, request.locale)
    .map((signal) => ({
      situation: signal.situation,
      origin: signal.origin,
      clause: truncate(signal.clause, 110),
      support: signal.support,
      evidenceIds: signal.origin === "review"
        ? signal.reviewIndexes
          .map((index) => reviewEvidenceByIndex.get(index))
          .filter((id): id is string => Boolean(id))
        : matchingLedgerEvidenceIds(signal.clause, request)
    }))
    // A situation whose source is absent from the ledger cannot be cited, and
    // an uncitable CEP is dropped by the evidence gate anyway.
    .filter((entry) => entry.evidenceIds.length > 0)
    .slice(0, 5);
}

/** Ledger atoms whose text carries the clause a product-stated situation came from. */
function matchingLedgerEvidenceIds(clause: string, request: PdpGeoContentPlanningRequest): string[] {
  const needle = cleanText(clause).slice(0, 40);
  if (!needle) return [];
  return request.evidenceLedger
    .filter((item) => cleanText(item.text).includes(needle))
    .map((item) => item.id)
    .slice(0, 2);
}
