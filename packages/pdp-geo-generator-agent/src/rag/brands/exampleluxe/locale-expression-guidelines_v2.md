...
- FAQ question shape is defined once in the ExampleLuxe best-practice document; this overlay adds only the US English surface. A question that points at the product with `this product`, `this cream`, or `this serum` instead of naming it cannot be attributed to this product in an AI answer, and `published`/`peer-reviewed` overstates a PDP-reported study.
