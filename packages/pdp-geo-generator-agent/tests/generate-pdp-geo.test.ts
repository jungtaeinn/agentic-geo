...
describe("generatePdpGeo", () => {
  ...
  it("writes Korean FAQ answers as direct AI-citation-friendly customer answers", async () => {
    ...
    expect(benefitAnswer).toMatch(/에스트라 아토베리어365 바디로션은[^.]*수분[^.]*피부 장벽[^.]*돕습니다/u);
  ...
  it("separates Korean target customer, brand science, and actionable usage through evidence reasoning", async () => {
    ...
    expect(String(webPage.description)).toContain("에스트라 아토베리어365 캡슐 토너 상품 페이지는 AESTURA가 선보이는");
    expect(String(webPage.description)).toMatch(/에스트라 아토베리어365 캡슐 토너는[^.]*고객을 위한 제품/u);
    expect(String(webPage.description)).toMatch(/[^.]+(?:을|를) 주요 성분·기술로 포함하고[^.]+돕습니다/u);
    expect(String(webPage.description)).toContain("에스트라 아토베리어365 캡슐 토너는 30,000원에 판매되고 있습니다");
  ...
  it("elevates SKU-heavy Korean PDP evidence into BestPractice-style GEO content", async () => {
    ...
    expect(String(webPage.description)).toMatch(/설화수 자음생크림은[^.]*고객을 위한 제품/u);
    expect(String(webPage.description)).toMatch(/진세노믹스[^.]*주요 성분·기술로 포함하고/u);
    expect(String(webPage.description)).toMatch(/설화수 자음생크림은 소용량 자음생크림 30ml, 자음생크림 50ml 옵션으로 구성되어 있으며, 270,000원에 판매되고 있습니다.*고객 리뷰에서 고객들은/u);
    expect(String(webPage.description)).not.toMatch(/페이지 본문에서는|페이지에서 확인할 수 있는|페이지에 공개된/u);
  ...
  it("keeps Korean cream mist descriptions and review queries grounded in the specific product type", async () => {
    ...
    const productNameIndex = productDescription.indexOf("아토베리어 365 크림 미스트");
    ...
    expect(description).toContain("아토베리어 365 크림 미스트 상품 페이지는 AESTURA가 선보이는");
    expect(description).toMatch(/아토베리어 365 크림 미스트는[^.]*건조하고 민감한 피부 고객을 위한 제품/u);
    ...
    expect(propertyText).not.toMatch(/고객에게[^?]*\s크림은 무엇인가요\?/);
  ...
  it("keeps generated Korean FAQ questions aligned with target audience and available evidence", async () => {
    ...
    expect(concernAnswer).not.toMatch(/민감 피부에 적합하며,\s*수분감을 중심|NEW\s*\||Sulwhasoo|상품 상세 테스트|성분 설명입니다|확인 키워드/);
  });
  ...
  it("keeps strategic GEO guidance in the final inference context when maxChunks is constrained", async () => {
    const cases = [
      {
        product: {
          name: "ATOBARRIER 365 Cream",
    ...
    for (const testCase of cases) {
      ...
      expect(result.diagnostics.selectedRagChunks.some((chunk) => chunk.kind === "geo-research")).toBe(true);
      ...
      expect(result.diagnostics.reasoning?.selectedSources ?? [])
        .toEqual(expect.arrayContaining([expect.stringContaining(pdpGeoGeneratorRagManifest.documents.geoResearch)]));
    }
  });
  ...
  it("sends GEO, CEP, and E-E-A-T strategic guidance to model-backed copy refinement", async () => {
    ...
    try {
      ...
      await refiner.refineCopy({
        ...
        ragChunks: [
          ...
          {
            id: "best-practice-1",
            source: "brands/sulwhasoo/best-practice_v1.md",
            title: "Sulwhasoo US BestPractice Tone",
    ...
    expect(capturedBody?.instructions).toContain("one connected buyer-answer narrative that covers this semantic arc");
    expect(capturedBody?.instructions).toContain("compact page-level counterpart");
  ...
  it("sends CEP narrative, volume isolation, and generative FAQ intent guidance to copy refinement", async () => {
    ...
    try {
      ...
      expect(instructions).toContain("one connected buyer-answer narrative that covers this semantic arc");
      expect(instructions).toContain("compact page-level counterpart");
  ...
  it("selects package GEO, CEP, and E-E-A-T RAG chunks during generation", async () => {
    ...
    try {
      ...
      expect(JSON.stringify(capturedBody).length).toBeLessThan(50_000);
  ...
  it("derives review-backed CEP recommendation PropertyValues from positive reviews while excluding negative review complaints", async () => {
    ...
    const indirectProperty = reviewQueryProperties.find((item) =>
      item.propertyID === "indirectCustomerQuestion"
    );
    ...
    const canonicalIngredientQuestion = faqQuestions.find((question: string) => /자음생크림의 구성 성분과 효능[·・]?효과/u.test(question));
    ...
    expect(result.diagnostics.inferredSearchQueries?.some((query) =>
      query.kind === "direct"
      && query.question === ingredientQuestion
  ...
  it("does not broaden the recommended skin type from an individual review-like FAQ", async () => {
    const { result } = await generatePdpGeo({
      ...
      source: {
        type: "manual-json",
        url: "https://example.com/products/atobarrier365-cream"
      },
      hints: {
        locale: "ko-KR",
        market: "KR"
  ...
  it("cleans Korean Aestura-style OCR, review typos, and property chunks before schema generation", async () => {
    ...
    expect(webPage.description).toContain("에스트라 아토베리어365 하이드로 수딩크림 상품 페이지는 AESTURA가 선보이는");
    expect(webPage.description).toMatch(/에스트라 아토베리어365 하이드로 수딩크림은[^.]*민감 피부 고객을 위한 제품/u);
    ...
    expect(result.content.sections.faq).toMatch(/추천할 수 있는 크림|성분\/기술이 포함되어 있습니다|주요 효능/);
			  ...
			  it("repairs Korean FAQ comparison answers that mix patent identifiers with formula technology", () => {
			    ...
			    expect(answer).toBe("공개된 상품 정보만으로는 아토베리어365 크림 캡슐과 동일하다고 단정하기 어렵습니다. 에스트라 아토베리어365 캡슐 토너의 캡슐은 PHA 워터에 띄워진 고밀도 세라마이드 캡슐로 설명됩니다.");
				  ...
				  it("keeps distinct Korean concern-benefit and ingredient-benefit FAQ questions", () => {
				    ...
				    expect(questions).toContain("에스트라 아토베리어365 젠틀 포밍클렌저는 어떤 피부 고민과 효능에 적합한가요?");
				    ...
				    expect(repaired.content.sections.faq).toContain("에스트라 아토베리어365 젠틀 포밍클렌저는 어떤 피부 고민과 효능에 적합한가요?");
