...
export const defaultPdpGeoGeneratorAnalysisPrompt = [
  ...
  "Product.description structure: product introduction/type -> target customer and concrete concern/CEP -> ingredient/technology composition -> supported finished-product benefit/effect -> source-stated research/article citation -> attributed positive or neutral review keywords last. Keep usage in Usage/HowTo.",
  ...
  "After classification, build only source-supported relations among target concern, ingredient/technology, benefit/effect, measured evidence, and review preference. Co-occurrence on the same PDP is not proof of an ingredient-to-effect relationship.",
  "Use complete relations to infer distinct suitability, concern/effect, ingredient-role, evidence, routine, and review query intents. Broader semantic associations may remain diagnostic query hypotheses, but they cannot become public product facts without product or review evidence.",
  "Field values must stay separated: HowTo.step uses only actionable source directions; ingredients uses only ingredients, formula technologies, ingredient-role explanations, and full ingredient lists; benefits uses only source-backed finished-product outcomes, effects, and concise evidence topics. Review-only experience stays explicitly review-attributed.",
  ...
  "Separate schema descriptions by entity role while keeping a shared evidence order: introduction -> target customer -> composition -> benefit/effect -> source-stated research/article citation -> attributed review keywords last. WebPage.description uses compact page-scope language; Product.description uses detailed product-entity language. Preserve cited dates and numbers in natural prose without inventing metadata. Brand history or science requires separate current-source brand evidence.",
  ...
  "Use classified OCR sentences as source-backed evidence for the matching field: detailed product facts may support Product.description, Key ingredients, Ingredient/effect detail, benefits, HowTo, and full ingredients; WebPage.description uses only high-level page coverage and separately supported brand context. Do not create separate OCR-only benefit, ingredient, or FAQ content when broader product evidence is available.",
  ...
  "FAQPage has no target or minimum count. Include only distinct questions whose direct answers are supported by product evidence; zero FAQ is valid. Do not manufacture questions from RAG guidance, raw reviews, or quota pressure.",
  "When FAQ evidence exists, prioritize buyer-decision questions for target customer/suitability, core benefit/effect, concern-to-ingredient or technology support, usage, and review/test/metric substantiation before lower-value support or storage questions.",
  "Build FAQ questions backwards from natural recommendation/comparison intents, then write answers forwards from evidence: product and target concern first, finished-product effect next, and an ingredient role only when an explicit ingredient-benefit link exists. Product-specific questions must name the exact product instead of using a deictic subject such as 이 제품, 이 크림, this product, or this cream. Do not strengthen neutral evidence into an efficacy or recommendation claim.",
  ...
  "HowTo requires a concrete goal and at least one direct source-backed customer action. Preserve source cardinality: one instruction becomes one step; multiple steps require explicit source order. Customer-review anecdotes and unordered notes cannot create steps.",
  ...
  documents: [
    {
      ...
      content: [
        ...
        "",
        "Use schema.org JSON-LD to represent PDP entities and grounded facts consistently with visible content. Valid markup cannot guarantee retrieval, citation, or display in a generative answer.",
        "",
        ...
        "- Generate an `@graph` with required product/page entities and only the optional `FAQPage`, `HowTo`, or `BreadcrumbList` nodes that the source actually supports.",
        "- Keep `WebPage.description` and `Product.description` distinct by entity role while following introduction, target customer, composition, benefit/effect, source-stated research/article citation, and attributed review keywords. WebPage is compact page-scope copy; Product is detailed entity copy.",
        ...
        "- Use `additionalProperty` for objective product characteristics such as key ingredients, skin type, size, texture, usage timing, target concern, technology, and review-derived recommendation context. Keep `PropertyValue.name` as a stable property label, not a full customer situation phrase or full question.",
        ...
        "- Use `HowTo.step` for a concrete goal with at least one direct source action. One instruction becomes one step; multiple steps require explicit source order and preserve count/order. Do not convert reviews, warnings, tests, or unordered notes into steps.",
    ...
    },
    {
      ...
      content: [
        ...
        "## Source Scope",
        "",
        "- Official sources checked on 2026-07-11: Google Search Central helpful content guidance, AI features guidance, structured-data policies, and Product structured data guidance.",
        ...
        "- Use positive or neutral reviews to shape attributed review summaries, review-derived recommendation `additionalProperty`, and review-intent FAQ when the observed language supports a reusable customer situation. One review is one observation, not a repeated pattern or product-efficacy proof. Exclude negative review complaints, scent complaints, ratings, and raw reviewer snippets.",
    ...
    },
    {
      ...
      content: [
        ...
        "## Source Scope",
        "",
        "- Sources checked on 2026-07-11: Ehrenberg-Bass Institute Research Services, CEP research service overview, and homepage.",
        ...
        "4. Classify every atomic source unit before combining it. Keep ingredients/technologies, benefits, measured effects, audiences/concerns, usage moments, reviews, and evidence conditions in separate roles.",
        "5. Build evidence-backed causal paths only where the source supports each edge: target concern -> desired outcome, ingredient/technology -> supported benefit/effect, effect -> measurement/study context, and review pattern -> customer preference.",
        "6. Generate CEP candidates and natural query/expression variants from the strongest causal paths without repeating the product name in every sentence.",
        "7. Keep broader semantic associations such as generic dryness -> winter as query hypotheses only unless product source, usage, or reviews support that occasion; never publish them as product facts or efficacy claims.",
        "8. Filter unsupported, generic, risky, duplicated, semantically redundant, or unrelated candidates.",
        "9. Prioritize candidates by source support, causal completeness, specificity, strength of review support, commerce usefulness, locale fit, and schema field usefulness.",
        "",
        "## Causal Path Completeness",
        "",
        "- Prefer product introduction/type -> target customer/concern -> composition -> supported benefit/effect -> source-stated research/article citation -> attributed positive/neutral review keywords last, omitting missing nodes rather than filling them from category knowledge.",
        "- If no explicit ingredient-to-outcome relation exists, keep formula composition and product benefit as separate supported facts instead of inventing causality.",
        "- Produce multiple search expressions only for distinct intent surfaces such as suitability, concern/effect, ingredient role, measured result, routine, or review experience; synonymous questions count as one intent.",
        "- Introduce the product once, then use a short reference or omitted subject and connect composition to benefit/effect instead of restarting every sentence with the full product name.",
        "",
        "## PDP Field Mapping",
        "",
        ...
        "- Product.description: use product-specific CEPs in order: product introduction/type, target customer and concern, ingredient composition, supported benefit/effect, source-stated research/article citation, then attributed representative review preference.",
        ...
        "- HowTo.step: require at least one direct source-backed action. One source instruction becomes one step; multiple steps require explicit source order. CEP and review context cannot invent actions.",
        "",
        "## Partial Update Query Planning",
        "",
    ...
    },
    {
      ...
      content: [
        ...
        "",
        "- Compose Product.description from product introduction/type, target customer and concern, composition, supported benefit/effect, source-stated research/article citation with natural date/number parsing, then attributed positive or neutral review keywords last.",
        ...
        "- Keep public field values separated: `HowTo.step` is a concrete source action and may retain source order/amount/timing/body area inside that action; standalone warning, compatibility, frequency, timing, or amount notes are not steps. `ingredients` is ingredient/formula/full-INCI content; `benefits` is source-backed finished-product outcome/effect/evidence-topic content; review-only experience stays explicitly review-attributed.",
        ...
        "- `WebPage.description` should introduce the product page and source-backed brand, name concrete supported target/formula/benefit information as parallel page facts when available, and summarize the remaining page coverage without copying the detailed product narrative.",
        "- `Product.description` should explain what the product is, who it is for, which ingredients/technologies compose it, which benefit/effect is supported, which source-stated research/article applies, and what attributed review keywords customers mention.",
        "",
        "## Entity and Intent Rules",
        "",
        "- Do not reuse the same text for WebPage.description and Product.description. Both follow introduction, target, composition, effect, research/article citation, and attributed review order; WebPage is compact page-scope copy and Product is detailed entity copy. Keep raw metric strings in dedicated properties.",
        ...
        "- Keep usage source-faithful: HowTo requires at least one direct action. One instruction becomes one step; multiple steps require explicit source order and retain count/order. Remove labels but do not split, merge, invent, or use review anecdotes as actions.",
        "",
    ...
    },
    {
      ...
      content: [
        ...
        "## Source Scope",
        "",
        "- Sources checked on 2026-07-11: GEO paper, GEO project page, C-SEO Bench, E-GEO, citation selection/absorption research, Google AI-features guidance, Google helpful content guidance, and Google Product structured data guidance.",
        ...
        "- FAQ and HowTo answerability: FAQ has no minimum and requires a direct evidence-backed answer; HowTo requires a concrete goal plus at least one direct source action, preserving one source instruction as one step and explicit multi-step order.",
    ...
    },
    {
      ...
      content: [
        ...
        "",
        "- Official sources checked on 2026-07-11: OpenAI retrieval, file search, embeddings, OAI-SearchBot and ChatGPT Search docs; Google Product structured data, AI features, and structured-data updates; Gemini embeddings and grounding; Perplexity Search API docs.",
        ...
        "- Google stopped showing FAQ rich results on 2026-05-07. FAQPage remains schema.org-valid visible Q/A semantics, but it is not a current Google FAQ rich-result tactic or a citation guarantee.",
        "- Google says AI Overviews and AI Mode require no special schema.org markup. Structured data must match visible content and must not be described as a direct citation trigger.",
        "",
