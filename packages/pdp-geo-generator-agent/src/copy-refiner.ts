...
function createCopyRefinementPrompt(request: PdpGeoCopyRefinementRequest): { system: string; user: string } {
  return {
    system: [
      ...
      "For Product.description and contentSections.description, compose one connected buyer-answer narrative that covers this semantic arc when evidence exists: product introduction and type -> target customer and concrete concern/CEP -> detailed main ingredients, named technology, and supported formula structure -> supported finished-product benefits/effects, including only explicitly linked ingredient roles -> one deduplicated source-stated research or related-article citation with naturally parsed publisher/title/date/numbers/finding -> one attributed positive or neutral customer-review keyword summary last. Treat the arc as coverage and reasoning guidance, not six fixed sentences; let CEP determine transitions and clause grouping. Keep exact completed safety tests inside the benefit/evidence block. Product.description may be materially more detailed than WebPage.description, but every detail must come from productEvidence. Never invent bibliographic metadata or turn review/commerce copy into a citation. Select ingredients and technologies by routed role and explicit relationship strength rather than an allowlist; do not turn an educational FAQ/category statement into a current-product ingredient. Use wording that naturally answers likely generative-search intents such as who it is for, which concern it addresses, which ingredient matters, which result is officially supported, and what customers say. Product.description describes the product itself, not the PDP or page resource; never use page-level wording in Product.description.",
      ...
      "Keep usage out of Product.description and contentSections.description so the six-part order remains stable. Routine placement and concrete application steps belong in Usage and HowTo.",
      ...
      "For WebPage.description, write a compact page-level counterpart rather than a second Product.description. Follow the evidence order product-page introduction -> supported target customer -> composition -> benefits/effects -> source-stated research/article citation -> attributed review keywords last, using page-scope language and less detail. Keep ingredients and benefits parallel unless explicit source evidence links them. Add brand history or expertise only when separate current-source brand evidence states it. Do not copy exact HowTo actions or raw report-like metric blocks.",
...
function containsBrokenKoreanCopyFragment(value: string): boolean {
  ...
  return /(?:설명된다|설명됩니다|제시된다|제시됩니다)\s+사용법을\s+다룹니다/u.test(text)
    ...
    || /(?:실제\s*)?고객\s*리뷰에서는[^.!?。！？]{0,180}(?:언급|표시|제시)됩니다/u.test(text);
}
