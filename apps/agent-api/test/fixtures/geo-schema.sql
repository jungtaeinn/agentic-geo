...
comment on table neo.geo_generation is 'GEO 생성 생애주기(상태 머신). upstream API가 접수 시 생성하고, 이후 전이는 neo-batch·agent-api가 상태 가드 UPDATE로 수행한다.';
...
comment on column neo.geo_interface.received_at is 'upstream API가 요청을 수신한 시각.';
...
comment on table neo.geo_result is 'GEO 생성 산출물. agent-api가 소유·기록한다(upstream API는 스키마만 관리, write 안 함).';
...
-- 정작 채널에 보내야 할 내용이 사라진다. upstream API가 INSERT(PENDING)하고 이후 전이는 neo-batch deliver 전담.
...
comment on table neo.geo_delivery is '채널 통보(전송 축) 생애주기. 접수 건당 1행이며 성공·실패와 무관하게 모든 건이 이 축을 갖는다. upstream API가 PENDING으로 생성하고 neo-batch deliver Job이 전이를 전담한다.';
