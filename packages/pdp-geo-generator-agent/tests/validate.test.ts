...
import { applySafePublicCopyRepairs, validateAndRepairPdpGeoArtifacts, validatePdpGeoArtifacts } from "../src/validate";
...
describe("validateAndRepairPdpGeoArtifacts FAQ and public text QA", () => {
  ...
  it("repairs question-period and comma-period punctuation artifacts in public values", () => {
    ...
    expect(values.some((value) => value.includes("무엇인가요? 예시더마"))).toBe(true);
  ...
  it("removes fully redundant duplicated metric clauses from WebPage descriptions", () => {
    ...
    const webPage = graphOf(repaired).find((node) => (Array.isArray(node["@type"]) ? node["@type"].includes("WebPage") : node["@type"] === "WebPage")) as Record<string, any>;
...
describe("applySafePublicCopyRepairs", () => {
  function buildInput(overrides?: {
    productDescription?: string;
    usageValue?: string;
    keyIngredientsValue?: string;
    faqAnswer?: string;
  }) {
    const productDescription = overrides?.productDescription
      ?? "Test Serum is a lightweight serum described for dry skin.";
    const faqAnswer = overrides?.faqAnswer
      ?? "Test Serum is described as suitable for dry skin.";
    const sections = {
      ...
      "@graph": [
        {
          ...
          name: "Test Serum",
          description: "Test Serum product page presents the serum's formula and usage.",
        ...
        },
        {
          ...
          name: "Test Serum",
          description: productDescription,
          mainEntityOfPage: { "@id": "https://example.com/p#webpage" },
          additionalProperty: [
            {
              "@type": "PropertyValue",
              name: "Usage",
              value: overrides?.usageValue ?? "Gently pat 2-3 pumps onto skin morning and night."
            },
            {
              "@type": "PropertyValue",
              name: "Key ingredients",
              value: overrides?.keyIngredientsValue ?? "Ginseng Extract, Niacinamide"
            }
          ]
        },
        {
          ...
          isPartOf: { "@id": "https://example.com/p#webpage" },
          about: { "@id": "https://example.com/p#product" },
          mainEntity: [{
            "@type": "Question",
  ...
  function graphNode(result: { schemaMarkup: { jsonLd: JsonObject } }, type: string): Record<string, unknown> {
    const graph = result.schemaMarkup.jsonLd["@graph"] as Array<Record<string, unknown>>;
    return graph.find((node) => node["@type"] === type
      || (Array.isArray(node["@type"]) && (node["@type"] as string[]).includes(type))) as Record<string, unknown>;
  }

  it("applies deterministic sentence-QA normalization to public copy and records the applied repair", () => {
    const broken = "Test Serum is a serum ,  described for dry skin.\\nIt includes ginseng.";
    const result = applySafePublicCopyRepairs(buildInput({ productDescription: broken }));

    const product = graphNode(result, "Product");
    expect(product.description).toBe("Test Serum is a serum, described for dry skin. It includes ginseng.");
    expect(result.content.sections.description).toBe("Test Serum is a serum, described for dry skin. It includes ginseng.");
    expect(result.appliedRepairs.some((repair) =>
      repair.field === "Product.description" && repair.source === "sentence-qa" && repair.before === broken.trim()
    )).toBe(true);
    // The scriptTag must serialize the repaired JSON-LD, not the original.
    expect(result.schemaMarkup.scriptTag).toContain("It includes ginseng.");

    // Re-validating the repaired artifacts must not re-report the applied
    // sentence-qa repair as a pending finding.
    const revalidated = validatePdpGeoArtifacts({
      schemaMarkup: result.schemaMarkup,
      content: result.content,
      fallbackProductName: result.content.sections.productName,
      fallbackDescription: result.content.sections.description,
      locale: "en-US"
    });
    expect(revalidated.validationFindings.filter((finding) =>
      finding.source === "sentence-qa" && /Product\.description|content\.sections\.description/.test(finding.field)
    )).toEqual([]);
  });

  it("deduplicates the Usage additionalProperty steps as an applied sentence-qa repair", () => {
    const duplicated = "Gently pat 2-3 pumps onto skin morning and night; Gently pat 2-3 pumps onto skin morning and night.";
    const result = applySafePublicCopyRepairs(buildInput({ usageValue: duplicated }));

    const product = graphNode(result, "Product");
    const usage = (product.additionalProperty as Array<Record<string, unknown>>).find((property) => property.name === "Usage");
    expect(usage?.value).toBe("Gently pat 2-3 pumps onto skin morning and night");
    expect(result.appliedRepairs.some((repair) =>
      repair.field === "Product.additionalProperty.Usage" && repair.source === "sentence-qa"
    )).toBe(true);
  });

  it("normalizes FAQ answers while keeping the question-answer pair intact", () => {
    const brokenAnswer = "Test Serum is described as suitable for dry skin ,  and layers easily.";
    const result = applySafePublicCopyRepairs(buildInput({ faqAnswer: brokenAnswer }));

    const faq = graphNode(result, "FAQPage");
    const item = (faq.mainEntity as Array<Record<string, unknown>>)[0] as Record<string, unknown>;
    expect(item.name).toBe("Is Test Serum suitable for dry skin?");
    expect((item.acceptedAnswer as Record<string, unknown>).text)
      .toBe("Test Serum is described as suitable for dry skin, and layers easily.");
  });

  it("leaves trust and structural issues unapplied so they remain findings", () => {
    const contaminated = "Ginseng Extract, What, Niacinamide";
    const input = buildInput({ keyIngredientsValue: contaminated });
    const result = applySafePublicCopyRepairs(input);

    const product = graphNode(result, "Product");
    const keyIngredients = (product.additionalProperty as Array<Record<string, unknown>>).find((property) => property.name === "Key ingredients");
    // The safe pass must not rewrite claim content — the broken token stays,
    // and the read-only validator still reports it for review.
    expect(keyIngredients?.value).toBe(contaminated);
    const revalidated = validatePdpGeoArtifacts({
      schemaMarkup: result.schemaMarkup,
      content: result.content,
      fallbackProductName: result.content.sections.productName,
    ...
    expect(revalidated.validationFindings.some((finding) =>
      finding.source === "trust-field-validator" && /non-ingredient token/i.test(finding.issue)
    )).toBe(true);
  });

  it("returns artifacts unchanged and no repairs for clean copy", () => {
    const input = buildInput();
    const result = applySafePublicCopyRepairs(input);
    expect(result.appliedRepairs).toEqual([]);
    expect(result.schemaMarkup.jsonLd).toEqual(input.schemaMarkup.jsonLd);
  });

  it("removes a URL-only value without leaving domain-path remainders", () => {
    // A URL-only value used to survive sentence QA as "com/upload/product/usage."
    // because sentence-punctuation spacing split the URL before URL-artifact
    // sentence removal ran.
    const result = applySafePublicCopyRepairs(buildInput({
      usageValue: "https://example.com/upload/product/usage.png"
    }));

    const product = graphNode(result, "Product");
    const properties = product.additionalProperty as Array<Record<string, unknown>>;
    expect(properties.some((property) => property.name === "Usage")).toBe(false);
    expect(JSON.stringify(properties)).not.toMatch(/com\/upload/);
  });

  it("drops only the URL sentence from mixed public copy and keeps the real sentences intact", () => {
    const mixed = "Gently pat 2-3 pumps onto skin morning and night. https://example.com/upload/product/usage.png Reapply whenever skin feels dry.";
    const result = applySafePublicCopyRepairs(buildInput({ usageValue: mixed }));

    const product = graphNode(result, "Product");
    const usage = (product.additionalProperty as Array<Record<string, unknown>>).find((property) => property.name === "Usage");
    expect(usage?.value).toContain("Gently pat 2-3 pumps onto skin morning and night");
    expect(usage?.value).not.toMatch(/https?|com\/upload|png/);
  });

  it("drops an additionalProperty whose value collapses to empty during sentence QA", () => {
    // Live examplederma run (2026-08-11): "Reported details" was published as
    // value: "" after its refined text was emptied by sentence-level repair.
    const result = applySafePublicCopyRepairs(buildInput({ usageValue: "→" }));

    const product = graphNode(result, "Product");
    const properties = product.additionalProperty as Array<Record<string, unknown>>;
    expect(properties.some((property) => property.name === "Usage")).toBe(false);
    expect(properties.every((property) => typeof property.value === "string" && property.value.trim().length > 0)).toBe(true);
    expect(result.appliedRepairs.some((repair) =>
      repair.field === "Product.additionalProperty.Usage" && /empty/i.test(repair.issue)
    )).toBe(true);
  });
});
